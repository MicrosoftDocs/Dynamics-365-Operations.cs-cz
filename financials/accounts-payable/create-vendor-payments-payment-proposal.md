...
Dotaz na návrh platby obsahuje různé karty, z nichž každá má různé možnosti výběru faktur k platbě. **Parametr** karta obsahuje možnosti, které většina organizací používá nejčastěji. V **záznamy mají být zahrnuty** s náhledem můžete určit, které faktury nebo včetně definováním oblastí pro různé ukazatele pro platby dodavatelů. Například pokud chcete zaplatit pouze určitou oblast dodavatelů, můžete definovat filtr pro oblast dodavatele. Tato funkce se často používá k výběru faktur pro konkrétní způsob platby. Například pokud definujete filtr kde **způsob platby** = **kontrola**, vybrány pouze faktury, které mají tento způsob platby pro platbu, za předpokladu, že splňují ostatní kritéria, které jsou zadány v dotazu. karta **Rozšířené parametry** obsahuje další možnosti, které nemusí být vztahující se k vaší organizaci. Na této kartě například naleznete možnosti pro úhradu faktur pro centralizované platby.

## <a name="parameters"></a>Parametry
-   **Výběr faktur podle** – faktury v rozsahu dat, který je určen **ode** a **datum** polí lze vybrat podle data splatnosti, data platební slevy nebo obojí. Pokud použijete datum platební slevy, systém nejprve hledá pro faktury, které mají datum platební slevy mezi od data a do data. Systém pak určuje, zda má faktura nárok na platební slevu použitím data relace pro ujištění se, že datum platební slevy již neuplynulo.
...
-   **Minimální datum platby** – zadejte minimální datum platby. Například **ode** a **datum** polí určete rozsah ze dne 1. září 10 a datum minimální platba je dne 5. V tomto případě všechny faktury, které mají datum splatnosti od 1 dne 5. září mají datum platby ze dne 5. Všechny faktury, které mají datum splatnosti od 5 dne 10 září však mají datum platby, která se shoduje s datem splatnosti každé faktury.
-   **Limit částky** – zadejte maximální celkovou částku u všech plateb.
-   **Vytvoření platby bez náhledu faktury** -Pokud je tato možnost nastavena na **Ano**, platby budou vytvořeny okamžitě na **platby dodavatele** stránky. **Návrh platby** stránka bude přeskočena. Z toho vyplývá, že platby se vytvoří rychleji. Platby lze měnit i nadále ze stránky **Platby dodavatele**. Alternativně se můžete vrátit na stránku **Návrh platby** pomocí tlačítka **Upravit faktury k vybrané platbě**.

## <a name="advanced-options"></a>Rozšířené možnosti
-   **Kontrola zůstatku dodavatele** – je-li tato možnost je nastavena na **Ano**, systém potvrdí, že dodavatel nemá záporný zůstatek před zaplacením jakékoli faktury. Pokud dodavatel debetní zůstatek, je vytvořena žádná platba. Dodavatele může mít například dobropisy a platby, které byly zaúčtovány, ale nebyly dosud vyrovnány. V takovém případě by neměla být platba dodavateli odeslána. Místo toho by měly být dobropisy nebo platby vyrovnány s ohledem na nevyřízené faktury.
...
-   **Měna platby** – toto pole určuje všechny platby, které jsou vytvořeny v měně. Pokud není definována v měně, každá faktura zaplacena v měně faktury.
...
-   **Typ protiúčtu** a **protiúčet** – nastavte tato pole definovat určitý typ účtu (například **knihy** nebo **banky**) a protiúčet (například zvláštní bankovní účet). Metoda platby pro fakturu definuje výchozí typ protiúčtu a protiúčet, ale můžete přepsat výchozí hodnoty těchto polí.
-   **Další filtry** – na **záznamy mají být zahrnuty** s náhledem můžete definovat další rozsahy kritérií. Například pokud chcete zaplatit pouze řadu dodavatelů, můžete definovat filtr pro oblast dodavatele. Tato funkce se často používá k výběru faktur pro konkrétní způsob platby. Například pokud definujete filtr kde **způsob platby** = **kontrola**, vybrány pouze faktury, které mají tento způsob platby pro platbu, za předpokladu, že splňují ostatní kritéria, které jsou zadány v dotazu.
...
April vybere **Platební sleva** jako typ návrhu. Zadává datum rozsahu ze dne 26 dne 10. Tyto faktury jsou zahrnuty v návrhu:
...
April vybere **Dle data splatnosti** jako typ návrhu. Zadává datum rozsahu ze dne 26 dne 10. Tyto faktury jsou zahrnuty v návrhu:
...
April vybere **Datum splatnosti a platební sleva** jako typ návrhu. Zadává datum rozsahu ze dne 26 dne 10. Tyto faktury jsou zahrnuty v návrhu:
...
-   Pole **Kontrola dimenzí** je aktivováno a jsou povoleny další dimenze. Nyní můžete definovat, jak dimenze budou zkopírovány do deníku. Například: • vyberte **organizační** políčko, chcete-li vytvořit návrh platby podle organizační jednotky pro metodu platby • vyberte **CostCenter** políčko, chcete-li vytvořit návrh platby na nákladové středisko pro metodu platby
...
Můžete definovat standardní debetní účet plateb podle metody platby bez ohledu na kontext země. To bude nastaveno na řádcích platby generovaných návrhem. S funkcí bankovního účtu lze definovat více debetní bankovních účtů spravovaných dimenzí a měnou nebo jejich kombinací k použití různých debetní bankovních účtů, v závislosti na jednotlivých kombinacích. Můžete nastavit tyto kombinace v **metody platby** stránku pomocí **bankovní účty** tlačítko je k dispozici pro každý způsob platby s **typem účtu účtování** = **banky**.
