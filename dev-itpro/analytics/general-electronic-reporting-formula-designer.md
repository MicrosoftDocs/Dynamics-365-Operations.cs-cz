...
description: "Toto téma popisuje, jak lze používat návrháře receptur v elektronickém výkaznictví. Při vytváření formátu pro určitý elektronický dokument pro elektronické výkaznictví můžete použít vzorce podobné vzorcům aplikace Microsoft Excel pro transformaci dat za účelem plnění požadavků na plnění a formátování daného dokumentu. Jsou podporovány různé druhy funkcí – text, datum a čas, matematické logické, informace, konverze datového typu a dalších (podnikové domény specifické funkce)."
author: kfend
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
-   transformace dat, která jsou přijatá z databáze Microsoft Dynamics 365 for Operations a která mají být vyplněna v datovém modelu elektronického výkaznictví, který je určený jako zdroj dat pro formáty elektronického výkaznictví (filtrování, seskupení, převod typů dat atd.);
...
-   ze zdrojů dat aplikace Dynamics 365 for Operations a parametrů spuštění do datového modelu elektronického výkaznictví,
-   z datového modelu elektronického výkaznictví do formátu elektronického výkaznictví,
-   ze zdrojů dat aplikace Dynamics 365 for Operations a parametrů spuštění do datového modelu elektronického výkaznictví.

Následující obrázek znázorňuje návrh výraz tohoto typu. V tomto příkladu výraz vrací hodnotu pole **Intrastat.AmountMST** tabulky aplikace Dynamics 365 for Operations **Intrastat** po zaokrouhlení hodnoty na dvě desetinná místa. [![Obrázek výraz vazby](./media/picture-expression-binding.jpg)](./media/picture-expression-binding.jpg) následující obrázek ukazuje, jak lze použít výraz tohoto typu. V tomto příkladu je výsledek výrazu navržené osazeny **Transaction.InvoicedAmount** systému **model pro vykazování daně** datového modelu. [![Obrázek výraz binding2](./media/picture-expression-binding2.jpg)](./media/picture-expression-binding2.jpg) za běhu, navržený vzorec **ROUND (Intrastat.AmountMST 2)**, zaokrouhlí hodnotu **AmountMST** pole pro každý záznam **Intrastat** tabulky na dvě desetinná místa a naplnit Zaokrouhlená hodnota **Transaction.InvoicedAmount** systému **vykazování daně** datový model.

### <a name="data-formatting"></a>Formátování dat

Návrháře receptur elektronického výkaznictví lze použít k definování výrazu, který naformátuje data přijatá ze zdrojů dat, aby tato data bylo možné odeslat jako součást generovaného elektronického dokumentu: Pokud máte formátování, které je třeba použít jako typické pravidlo, které by mělo být znovu využito pro formát, můžete použít toto formátování jednou v konfiguraci formátu jako pojmenovanou transformaci, která má výraz formátování. Tuto pojmenovanou transformaci lze potom propojit s mnoha komponentami formátu, jejichž výstup musí být formátován podle vytvořeného výrazu. Následující obrázek znázorňuje návrh transformace tohoto typu. V tomto příkladu přijme transformace **TrimmedString** vstupní data typu dat **řetězec** a ořeže počáteční a koncové mezery při vrácení hodnoty řetězce. [![Obrázek transformace návrh](./media/picture-transformation-design.jpg)](./media/picture-transformation-design.jpg) následující obrázek ukazuje, jak lze použít transformaci tohoto typu. V tomto příkladu několik součástí formátu, které v době spuštění odeslaly text jako výstup do generovaného elektronického dokumentu, odkazuje na transformaci **TrimmedString** názvem. [![Obrázek transformace využití](./media/picture-transformation-usage.jpg)](./media/picture-transformation-usage.jpg) při formátu součásti se odkazují ** TrimmedString ** transformace (například **partyName** komponenty na předchozím obrázku) to odešle text jako výstup pro generování dokumentu. Text nezahrnuje počáteční a koncové mezery. Pokud máte formátování, které je nutné použít jednotlivě, můžete toto formátování použít jako jednotlivý výraz vazby konkrétní součásti formátu. Následující obrázek znázorňuje výraz tohoto typu. V tomto příkladu je součást formátu **partyType** vázána na zdroj dat pomocí výrazu, který převede příchozí data z pole **Model.Company.RegistrationType** ve zdroji dat na text s velkými písmeny a odešle tento text jako výstup do elektronického dokumentu. [![Obrázek vazby pomocí vzorce](./media/picture-binding-with-formula.jpg)](./media/picture-binding-with-formula.jpg)
...
[![ověřovací obrázek](./media/picture-validation.jpg)](./media/picture-validation.jpg) The ER návrháře vzorců lze také určit název souboru pro generování elektronického dokumentu a řídí proces vytváření souboru. Následující obrázek znázorňuje návrh kontroly procesního toku tohoto typu. Zde je vysvětlení konfigurace v tomto příkladu:
...
[![ovládací prvek obrázek soubor](./media/picture-file-control.jpg)](./media/picture-file-control.jpg)
...
Text a numerické konstanty (hodnoty, které nejsou vypočteny) lze použít při tvorbě výrazů. Například výraz ** hodnota ("100") + 20 ** používá řetězec konstanta "100" a numerická konstanta 20 a vrátí číselnou hodnotu **120**. Návrhář receptur elektronického výkaznictví podporuje řídicí sekvence, které tak můžete zadat jako část řetězce výrazu, s níž by se mělo zacházet jinak. Například výraz **"Lev Tolstoj ""Vojna a mir"" Svazek 1"** vrátí textový řetězec **Lev Tolstoj "Vojna a mir" Svazek 1**.
...
Operátory na stejném řádku mají stejnou prioritu. Pokud výraz obsahuje více těchto operátorů, bude vyhodnocen zleva doprava. Například výraz **1 + 6 / 2 \*3 &gt;5** vrátí **true**. Doporučujeme vám pomocí závorek explicitně určit požadované pořadí vyhodnocení výrazů, usnadní se tím čtení a správa výrazů.

#### <a name="references"></a>Odkazy

Všechny zdroje dat aktuální součásti elektronického výkaznictví (model nebo formát), které jsou k dispozici během návrhu výrazu, lze použít jako pojmenované odkazy. Například aktuální datový model elektronického výkaznictví obsahuje zdroj dat **ReportingDate**, který vrací hodnotu datového typu **DATETIME**. Chcete-li správně formátovat hodnotu do generování dokumentu, můžete odkazovat na zdroj dat ve výrazu takto: **DATETIMEFORMAT (ReportingDate, "dd-MM-rrrr")** všech znaků názvu referenční zdroj dat, které není představují písmena abecedy musí předcházet apostrof ('). Pokud název zdroje dat odkazující obsahuje nejméně jeden symbol, která nereprezentuje písmeno abecedy (například interpunkční znaménka nebo jiné písemné symbolů), název musí být vložen do jednoduchých uvozovek. Několik příkladů:
...
| DATETODATETIME (datum)                      | Převede zadanou hodnotu data na hodnotu data a času.                                                                                                                                                                                                                                                                                                            | **DATETODATETIME (CompInfo. "getCurrentDate()')** vrací aktuální 365 Dynamics pro operace data relace, 24/12/2015 a jako **24/12/2015 00:00:00 dop**. V tomto příkladu **CompInfo** představuje zdroj dat elektronického výkaznictví typu **Dynamics 365 for Operations/Table**, který odkazuje na tabulku CompanyInfo. |
| NOW ()                                     | Vrátí aktuální datum relace Dynamics 365 for Operations jako hodnotu data a času.                                                                                                                                                                                                                                                             |                                                                                                                                                                                                                                                                                                       |
| TODAY ()                                   | Vrátí aktuální datum relace Dynamics 365 for Operations jako hodnotu data.                                                                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                                                                       |
| NULLDATE ()                                | Vrátí hodnotu data **null**.                                                                                                                                                                                                                                                                                                                                    |                                                                                                                                                                                                                                                                                                       |
| NULLDATETIME ()                            | Vrátí hodnotu data a času **null**.                                                                                                                                                                                                                                                                                                                                |                                                                                                                                                                                                                                                                                                       |
| DATETIMEFORMAT (datum a čas, formát)          | Převede zadanou hodnotu data a času na řetězec v zadaném formátu. (Informace o podporovaných formátech: [standardní](https://msdn.microsoft.com/en-us/library/az4se3k1(v=vs.110).aspx) a [vlastní](https://msdn.microsoft.com/en-us/library/8kb3ddd4(v=vs.110).aspx).)                                                                        | **DATETIMEFORMAT (NOW(), "dd-MM-rrrr")** vrátí jako aktuální 365 Dynamics pro operace datum aplikaci serveru, 12/24/2015 **"24-12-2015"**, podle zadaného vlastního formátu.                                                                                                          |
| DATETIMEFORMAT (datum a čas, jazyková verze) | Převede zadanou hodnotu data a času na řetězec v zadaném formátu [jazykové verzi](https://msdn.microsoft.com/en-us/goglobal/bb896001.aspx). (Informace o podporovaných formátech viz [standardní](https://msdn.microsoft.com/en-us/library/az4se3k1(v=vs.110).aspx) a [vlastní](https://msdn.microsoft.com/en-us/library/8kb3ddd4(v=vs.110).aspx).) | **DATETIMEFORMAT (NOW(), "d", "de")** vrátí aktuální datum aplikačního serveru Dynamics 365 for Operations, 12/24/2015, jako **"24.12.2015"**, podle vybraného německého prostředí.                                                                                                             |
| SESSIONTODAY ()                            | Vrátí aktuální datum relace Dynamics 365 for Operations jako hodnotu data.                                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                                       |
| SESSIONNOW ()                              | Vrátí aktuální datum a čas relace Dynamics 365 for Operations jako hodnotu data a času.                                                                                                                                                                                                                                                                         |                                                                                                                                                                                                                                                                                                       |
| DATEFORMAT (datum, formát)                  | Vrátí řetězcovou reprezentaci data v zadaném formátu.                                                                                                                                                                                                                                                                                                    | **DATEFORMAT (SESSIONTODAY (), "dd-MM-rrrr")** vrátí aktuální datum relace Dynamics 365 for Operations jako 12/24/2015 “**24-12-2015**” podle zadaného vlastního formátu.                                                                                                                      |
| DATEFORMAT (datum, formát, jazyková verze)         | Převede zadanou hodnotu data na řetězec v zadaném formátu [jazykové verzi](https://msdn.microsoft.com/en-us/goglobal/bb896001.aspx). (Informace o podporovaných formátech viz [standardní](https://msdn.microsoft.com/en-us/library/az4se3k1(v=vs.110).aspx) a [vlastní](https://msdn.microsoft.com/en-us/library/8kb3ddd4(v=vs.110).aspx).)     | **DATETIMEFORMAT (SESSIONNOW (), "d", "de")** vrátí aktuální datum aplikačního serveru Dynamics 365 for Operations, 12/24/2015 jako **“24.12.2015”** podle vybrané německé jazykové verze.                                                                                                                       |
...
<td><strong>ROZDĚLIT (&quot;abcd&quot;, 3)</strong> vrátí nový seznam, který obsahuje dva záznamy, které mají <strong>řetězec</strong> pole. Pole v prvním záznamu obsahuje text, <strong>&quot;abc&quot;</strong>, a pole v druhém záznamu obsahuje text, <strong>&quot;d&quot;</strong>.</td>
</tr>
<tr class="even">
...
<td>V následujícím příkladu je zdroj dat <strong>Lines</strong> vytvořen jako seznamu tří záznamů, který je rozdělený na dávky, z nichž každá obsahuje až dva záznamy. <a href="./media/picture-splitlist-datasource.jpg"><img src="./media/picture-splitlist-datasource.jpg" alt="Data source that is divided into batches" class="alignnone wp-image-290681 size-full" width="397" height="136" /></a>Zobrazuje rozložení navržený formát, kde vazby <strong>řádky</strong> zdroj dat lze generovat výstup ve formátu XML, který představuje jednotlivé uzly pro každou dávku a záznamy v ní. <a href="./media/picture-splitlist-format.jpg"><img src="./media/picture-splitlist-format.jpg" alt="Format layout that has bindings to a data source" class="alignnone wp-image-290691 size-full" width="374" height="161" /></a>Tímto je výsledkem spuštění navržený formát. <a href="./media/picture-splitlist-result.jpg"><img src="./media/picture-splitlist-result.jpg" alt="Result of running the format" class="alignnone wp-image-290701 size-full" width="358" height="191" /></a></td>
</tr>
<tr class="odd">
...
<td><strong>LISTJOIN (rozdělení (&quot;abc&quot;, 1), ROZDĚLENÉ (&quot;def&quot;, 1))</strong> vrátí seznam záznamů, pokud jeden z pole <strong>řetězec</strong> datový typ obsahuje zadání jednoho písmena.</td>
</tr>
<tr class="odd">
...
<td><strong>EMPTYLIST (rozdělení (&quot;abc&quot;, 1))</strong> vrátí nový prázdný seznam, který má stejnou strukturu jako seznam, který je vrácený <strong>rozdělení</strong> funkce.</td>
</tr>
<tr class="odd">
<td>FIRST (seznam)</td>
...
<td>Pokud zadáte <strong>ROZDĚLIT (&quot;abcdef&quot;, 2)</strong> jako zdroj dat (DS), <strong>počet (ALLITEMS (DS. Hodnota))</strong> vrátí <strong>3</strong>.</td>
</tr>
<tr class="odd">
...
<td>Při <strong>dodavatele</strong> nakonfigurován jako zdroj dat ER, který odkazuje na tabulku VendTable <strong>kde (dodavatelé, Vendors.VendGroup = &quot;40&quot;)</strong> vrátí seznam dodavatelů, kteří patří do skupiny dodavatele 40.</td>
</tr>
<tr class="even">
...
<td>V následujícím příkladu je zdroj dat <strong>Enumerated</strong> vytvořen jako výčtový seznam záznamů dodavatelů ze zdroje dat <strong>Vendors</strong>, který odkazuje na tabulku <strong>VendTable</strong>. <a href="./media/picture-enumerate-datasource.jpg"><img src="./media/picture-enumerate-datasource.jpg" alt="Enumerated data source" class="alignnone wp-image-290711 size-full" width="387" height="136" /></a>Zde je formát, kde jsou vytvořeny vazby dat ke generování výstupu ve formátu XML, který představuje jednotlivé dodavatele jako výčet uzly. <a href="./media/picture-enumerate-format.jpg"><img src="./media/picture-enumerate-format.jpg" alt="Format that has data bindings" class="alignnone wp-image-290721 size-full" width="414" height="138" /></a>To je výsledkem spuštění navržený formát. <a href="./media/picture-enumerate-result.jpg"><img src="./media/picture-enumerate-result.jpg" alt="Result of running the format" class="alignnone wp-image-290731 size-full" width="567" height="176" /></a></td>
</tr>
<tr class="odd">
<td>COUNT (seznam)</td>
...
<td><strong>POČET (rozdělení (&quot;abcd&quot;, 3))</strong> vrátí <strong>2</strong>, protože <strong>rozdělení</strong> funkce vytvoří seznam, který obsahuje dva záznamy.</td>
</tr>
<tr class="even">
...
<td>Následující příklad ukazuje výčet zavedený v datovém modelu. <a href="./media/ger-listoffields-function-model-enumeration.png"><img src="./media/ger-listoffields-function-model-enumeration-e1474545790761.png" alt="GER LISTOFFIELDS function - model enumeration" class="alignnone wp-image-1203943 size-full" width="514" height="155" /></a>V následujícím příkladu:
<ul>
...
< href="./media/ger-listoffields-function-in-format-expression.png" ></ph><img src="./media/ger-listoffields-function-in-format-expression-e1474546110395.png" alt="GER LISTOFFIELDS function - in format expression" class="alignnone wp-image-1204033 size-full" width="549" height="318" /></a> v následujícím příkladu ER formátovat prvky, které jsou vázány na zdroj dat typu seznamu záznamů, který byl vytvořen pomocí funkce LISTOFFIELDS. <a href="./media/ger-listoffields-function-format-design.png"><img src="./media/ger-listoffields-function-format-design.png" alt="GER LISTOFFIELDS function - format design" class="alignnone size-full wp-image-1204043" width="466" height="221" /></a>To je výsledkem spuštění navržený formát. <a href="./media/ger-listoffields-function-format-output.png"><img src="./media/ger-listoffields-function-format-output.png" alt="GER LISTOFFIELDS function - format output" class="alignnone size-full wp-image-1204053" width="585" height="158" /></a><strong>Poznámka:</strong> ER formát výstupu podle jazykového nastavení nakonfigurována pro nadřazený soubor a SLOŽKU formátování prvků je naplněn překládané textu pro popisy a popisy.</td>
</tr>
<tr class="odd">
...
<td>V následujícím příkladu je uveden vzorový formát použití datových zdrojů. <a href="./media/ger-splitlistbylimit-format.png"><img src="./media/ger-splitlistbylimit-format.png" alt="GER SPLITLISTBYLIMIT - format" class="alignnone size-full wp-image-1204063" width="396" height="195" /></a><a href="./media/ger-splitlistbylimit-datasources.png"><img src="./media/ger-splitlistbylimit-datasources.png" alt="GER SPLITLISTBYLIMIT - datasources" class="alignnone size-full wp-image-1204073" width="320" height="208" /></a>To je výsledek spuštění formát, zobrazující seznam bez stromové struktury položek zboží. <a href="./media/ger-splitlistbylimit-output.png"><img src="./media/ger-splitlistbylimit-output.png" alt="GER SPLITLISTBYLIMIT - output" class="alignnone size-full wp-image-1204083" width="462" height="204" /></a>Následující příklad ukazuje stejný formát, který byl upraven po jedné dávce musí obsahovat komodity s celkovou hmotnost, která by neměla překročit limit 9 prezentovat seznam položek zboží v dávkách. <a href="./media/ger-splitlistbylimit-format-1.png"><img src="./media/ger-splitlistbylimit-format-1.png" alt="GER SPLITLISTBYLIMIT - format 1" class="alignnone size-full wp-image-1204103" width="466" height="438" /></a><a href="./media/ger-splitlistbylimit-datasources-1.png"><img src="./media/ger-splitlistbylimit-datasources-1.png" alt="GER SPLITLISTBYLIMIT - datasources 1" class="alignnone size-full wp-image-1204093" width="645" height="507" /></a>To je výsledkem spuštění upravené formátu. <a href="./media/ger-splitlistbylimit-output-1.png"><img src="./media/ger-splitlistbylimit-output-1.png" alt="GER SPLITLISTBYLIMIT - output 1" class="alignnone size-full wp-image-1204113" width="676" height="611" /></a><strong>Poznámka:</strong> limit není vyrovnána na poslední položku seznamu Původ hodnota překročila limit zdroje (hmotnosti) (11) překračuje limit definovaný (9). Použijte funkci <strong>WHERE</strong> nebo výraz <strong>Enabled</strong> odpovídajícího prvku formátu k ignorování (přeskočení) dílčích seznamů během generování sestavy (pokud je třeba).</td>
</tr>
<tr class="odd">
...
<td>Filtr (dodavatelé, Vendors.VendGroup = &quot;40&quot;) vrátí seznam pouze dodavatele, skupiny dodavatelů "40" při <strong>dodavatele</strong> nakonfigurován jako odkazující na zdroj dat ER <strong>VendTable</strong> tabulka</td>
</tr>
</tbody>
</table>
...
| Funkce                                                                                | popis                                                                                                                                                                                                                                                                     | Příklad                                                                                                                                                                                                                                                      |
|-----------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| PŘÍPAD (1 výsledek výrazu, možnost 1, \[, možnost 2, vést 2\] ... \[, výchozí výsledek\]) | Vyhodnotí zadanou hodnotu výrazu s ohledem na zadané alternativní možnosti. Vrátí se výsledek možnosti, která je rovna hodnotě výrazu. V opačném případě se vrátí volitelně zadaný výchozí výsledek (poslední parametr, který nepředchází dané možnosti). | **CASE( DATETIMEFORMAT( NOW(), "MM"), "10", "WINTER", "11", "WINTER", "12", "WINTER", "")** vrátí řetězec **"WINTER"**, když je aktuální datum relace Dynamics 365 for Operations mezi říjnem a prosincem. Jinak bude vrácen prázdný řetězec. |
| IF (podmínka, hodnota 1, hodnota 2)                                                        | Při splnění dané podmínky bude vrácena zadaná hodnota 1. V ostatních případech vrátí hodnotu 2. Jsou-li hodnoty 1 a 2 záznamy nebo seznamech záznamů, výsledek bude mít pouze pole, která existují v obou seznamech.                                                                     | **IF (1=2, "podmínka je splněna", "podmínka není splněna")** vrátí řetězec **"podmínka není splněna"**.                                                                                                                                                      |
| NOT (podmínka)                                                                         | Vrátí obrácenou logickou hodnotu zadané podmínky.                                                                                                                                                                                                                   | **NOT (TRUE)** vrátí **FALSE**.                                                                                                                                                                                                                            |
| A (Podmínka 1\[, Podmínka 2... \])                                                 | Vrátí **TRUE**, pokud jsou *všechny *zadané podmínky pravda. V opačném případě vrátí hodnotu **FALSE**.                                                                                                                                                                                            | **AND (1=1, "a"="a")** vrátí **TRUE**. **AND (1=2, "a"="a")** vrátí **FALSE**.                                                                                                                                                                           |
| NEBO (Podmínka 1\[, Podmínka 2... \])                                                  | Vrátí **FALSE**, pokud jsou *všechny *zadané podmínky nepravda. Vrátí **TRUE**, pokud je *jakákoli *zadaná podmínka pravda.                                                                                                                                                                 | **OR (1=2, "a"="a")** vrátí **TRUE**.                                                                                                                                                                                                                      |
...
<td><strong>HODNOTA (&quot;1 234,56&quot;)</strong> vyvolá výjimku.</td>
</tr>
<tr class="odd">
...
| EMPTYRECORD (záznam) | Vrátí záznam **null**, který má stejnou strukturu jako zadaný seznam záznamů nebo záznam. **Poznámka:** A **null** je záznam, kde mají všechna pole prázdná hodnota (**0**\[0\] pro čísla, prázdný řetězec pro řetězce a podobně). | **EMPTYRECORD (SPLIT ("abc", 1))** vrátí nový prázdný záznam, který má stejnou strukturu jako seznam vrácený funkcí **SPLIT**.   |
...
<td><strong>HORNÍ (&quot;vzorek&quot;)</strong> vrátí <strong>&quot;vzorek&quot;</strong>.</td>
</tr>
<tr class="even">
<td>LOWER (řetězec)</td>
<td>Vrátí zadaný řetězec, který je převeden na malá písmena.</td>
<td><strong>NIŽŠÍ (&quot;vzorek&quot;)</strong> vrátí <strong>&quot;vzorek&quot;</strong>.</td>
</tr>
<tr class="odd">
<td>LEFT (řetězec, počet znaků)</td>
<td>Vrátí zadaný počet znaků od začátku zadaného řetězce.</td>
<td><strong>VLEVO (&quot;vzorek&quot;, 3)</strong> vrátí <strong>&quot;Sam&quot;</strong>.</td>
</tr>
<tr class="even">
<td>RIGHT (řetězec, počet znaků)</td>
<td>Vrátí zadaný počet znaků od konce zadaného řetězce.</td>
<td><strong>DOPRAVA (&quot;vzorek&quot;, 3)</strong> vrátí <strong>&quot;Náhled&quot;</strong>.</td>
</tr>
<tr class="odd">
...
<td><strong>MID (&quot;vzorek&quot;, 2, 3)</strong> vrátí <strong>&quot;amp&quot;</strong>.</td>
</tr>
<tr class="even">
<td>LEN (řetězec)</td>
<td>Vrátí počet znaků v zadaném řetězci.</td>
<td><strong>Délka (&quot;vzorek&quot;)</strong> vrátí <strong>6</strong>.</td>
</tr>
<tr class="odd">
<td>CHAR (číslo)</td>
...
<td><strong>ZŘETĚZENÍ (&quot;abc&quot;, &quot;def&quot;)</strong> vrátí <strong>&quot;abcdef&quot;</strong>. <strong>Poznámka:</strong> výraz <strong>&quot;abc&quot;&amp;&quot;HD&quot;</strong> také vrátí <strong>&quot;abcdef&quot;</strong>.</td>
</tr>
<tr class="odd">
...
<td><strong>PŘELOŽIT (&quot;abcdef&quot;, &quot;cd&quot;, &quot;gv&quot;)</strong> nahrazuje vzorek <strong>&quot;cd&quot;</strong> s řetězcem <strong>&quot;gv&quot;</strong> a vrátí <strong>&quot;abGHef&quot;</strong>.</td>
</tr>
<tr class="even">
...
<td>  <strong>NAHRADIT (&quot;+1 923 456 4971&quot;, &quot;[^ 0-9]&quot;, &quot;&quot;, PRAVDA)</strong> použije regulární výraz, který odebere všechny matematických symbolů a vrátí <strong>&quot;19234564971&quot;</strong>. <strong>NAHRADIT (&quot;abcdef&quot;, &quot;cd&quot;, &quot;gv&quot;, false)</strong> nahrazuje vzorek <strong>&quot;cd&quot;</strong> s řetězcem <strong>&quot;gv&quot;</strong> a vrátí <strong>&quot;abGHef&quot;</strong>.</td>
</tr>
<tr class="odd">
<td>TEXT (vstup)</td>
<td>Vrátí zadaný vstup, který je převeden na textový řetězec naformátovaný podle nastavení národního prostředí serveru aktuální instance aplikace Dynamics 365 for Operations. Co se týká hodnot typu <strong>real</strong>, převod řetězce je omezen na dvě desetinná místa.</td>
<td>Pokud 365 Dynamics pro národní prostředí serveru instance operace je definována jako <strong>EN-US</strong>, <strong>TEXT (NOW ())</strong> vrátí aktuální 365 Dynamics pro operace data relace, 17/12/2015 a jako textový řetězec <strong>&quot;17/12/2015 07:59:23 AM&quot;</strong>. <strong>TEXT (1/3)</strong> vrátí <strong>&quot;0.33&quot;</strong>.</td>
</tr>
<tr class="even">
<td>FORMAT (řetězec 1 řetězce 2[, řetězec 3...])</td>
<td>Vrátí zadaný řetězec, který je formátován nahrazením všech výskytů <strong>%N</strong> argumentem <em>n</em>. Argumenty jsou řetězce. Pokud není argument pro parametr, parametr je vrácena jako <strong>&quot;%N&quot;</strong> v řetězci. Co se týká hodnot typu <strong>real</strong>, převod řetězce je omezen na dvě desetinná místa.</td>
<td>V tomto příkladu vrátí zdroj dat <strong>PaymentModel</strong> seznam záznamů odběratelů prostřednictvím součásti <strong>Customer</strong> a datum zpracování prostřednictvím pole <strong>ProcessingDate</strong>. <a href="./media/picture-format-datasource.jpg"><img src="./media/picture-format-datasource.jpg" alt="PaymentModel data source" class="alignnone wp-image-290751 size-full" width="293" height="143" /></a>Ve ER formátu, který je určen ke generování elektronického souboru pro vybrané zákazníky <strong>PaymentModel</strong> je vybrána jako zdroj dat a řídí tok procesu. Jestliže je vybraný odběratel zastaven u data zpracování sestavy, je vyvolána výjimka pro koncové uživatele. Vzorec, který je určen pro tento typ ovládacího prvku pro zpracování, může využít následující zdroje:
<ul>
<li>Popisek aplikace Dynamics 365 for Operations SYS70894, který má následující text:
<ul>
<li><strong>Pro jazyk EN-US:</strong>&quot;nic k vytištění&quot;</li>
<li><strong>Pro jazyk DE:</strong>&quot;Nichts zu drucken&quot;</li>
</ul></li>
<li>Popisek aplikace Dynamics 365 for Operations SYS18389, který má následující text:
<ul>
<li><strong>Pro jazyk EN-US:</strong>&quot;zastavení odběratele %1 pro %2.&quot;</li>
<li><strong>Pro jazyk DE:</strong>&quot;gesperrt für %2 wird Debitor '%1'.&quot;</li>
</ul></li>
</ul>
Zde je vzorec, který může být určen: formát (CONCATENATE (@&quot;SYS70894&quot;,&quot;. &quot;@&quot;SYS18389&quot;), model.Customer.Name, DATETIMEFORMAT (model. ProcessingDate, &quot;d&quot;)) Pokud zpracování sestavy <strong>Litware maloobchodní zákazník</strong> na 17 prosince 2015 v <strong>EN-US</strong> kultura a <strong>EN-US</strong> jazyk, tento vzorec vrátí následující text, který může být předloženo jako zpráva o výjimce pro koncového uživatele: &quot;nic k vytištění. Maloobchodní zákazník Litware je zastaven pro 17/12/2015.&quot; Pokud stejnou sestavu zpracování<strong> Litware maloobchodní zákazník</strong> na 17. prosince 2015 v <strong>DE</strong> kultura a <strong>DE</strong> jazyk, tento vzorec vrátí následující text, který používá jiný formát data: &quot;Nichts zu drucken. Gesperrt debitor "Litware maloobchodní" wird für 17.12.2015.&quot; <strong>Poznámka: </strong>Následující syntaxe je použita ve vzorcích elektronického výkaznictví pro popisky:
<ul>
<li><strong>Pro popisky z Dynamics 365 pro operace zdroje:</strong> <strong>@&quot;X&quot;</strong>, kde X je ID popisku ve stromu aplikačních objektů (AOT)</li>
<li><strong>Pro popisky, které se nacházejí v konfiguracích ER:</strong> <strong>@&quot;ER_LABEL:X&quot;</strong>, kde X je ID popisku v konfiguraci ER</li>
</ul></td>
</tr>
<tr class="odd">
...
Vrátí název elementu aktuálního formátu. Vrací prázdný řetězec, když je příznak **Podrobnosti výstupu shromažďování** aktuálních souborů vypnut.

Další informace o použití těchto funkcí najdete v části **Elektronické výkaznictví - zdroj dat formát výstupu pro inventuru a souhrn** (část obchodního procesu **Získání/vývoj komponent služby/řešení** Průvodce záznamem úloh.

SUMIFS (klíč řetězce pro sčítání, řetězec Oblast1 kritéria, kritéria hodnota1 řetězec \[, řetězec oblast2 kritéria, kritéria řetězec hodnota2,... \])

Vrátí součet hodnot uzlů (s názvem definovaným jako klíčem) XML získaným během provedení tohoto formátu, který splňuje zadané podmínky (dvojice rozsah a hodnota). Vrací nulovou hodnotu, když je příznak **Podrobnosti výstupu shromažďování** aktuálních souborů vypnut.

SUMIF (řetězec klíče pro sčítání, řetězec kritéria rozsahu, řetězec kritéria hodnoty)

Vrátí součet hodnot uzlů (s názvem definovaným jako klíčem) XML získaným během provedení tohoto formátu, který splňuje zadanou podmínku (rozsah a hodnota). Vrací nulovou hodnotu, když je příznak **Podrobnosti výstupu shromažďování** aktuálních souborů vypnut.

COUNTIFS (řetězec Oblast1 kritéria, kritéria hodnota1 řetězec \[, řetězec oblast2 kritéria, kritéria řetězec hodnota2,... \])

Vrátí počet uzlů (s názvem definovaným jako klíčem) XML získaným během provedení tohoto formátu, který splňuje zadané podmínky. Vrací nulovou hodnotu, když je příznak **Podrobnosti výstupu shromažďování** aktuálních souborů vypnut.

COUNTIF (řetězec rozsahu kritérií, řetězec hodnoty kritérií)

Vrátí počet uzlů XML získaný během provedení tohoto formátu, který splňuje zadanou podmínku (rozsah a hodnota). Vrací nulovou hodnotu, když je příznak **Podrobnosti výstupu shromažďování** aktuálních souborů vypnut.

COLLECTEDLIST (řetězec Oblast1 kritéria, kritéria hodnota1 řetězec \[, řetězec oblast2 kritéria, kritéria řetězec hodnota2,... \])

Vrátí počet hodnot uzlů XML získaný během provedení tohoto formátu, který splňuje zadané podmínky. Vrací prázdný seznam, když je příznak **Podrobnosti výstupu shromažďování **aktuálních souborů vypnut.

### <a name="other-business-domainspecific-functions"></a>Další funkce (konkrétní pro obchodní domény)

| Funkce                                                                         | popis                                                                                                                                                                                                                                                        | Příklad                                                                                                                                                                                                                                                                                                       |
|----------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CONVERTCURRENCY (částka, zdrojová měna, cílová měna, datum, společnost)        | Převede zadanou peněžní částku ze zdrojové měny na cílovou měnu za použití nastavení zadané společnosti aplikace Dynamics 365 for Operations k zadanému datu                                                                            | **CONVERTCURRENCY (1, "EUR", "USD", TODAY(), "DEMF")** vrátí ekvivalent jednoho eura v amerických dolarech v aktuální den relace podle nastavení společnosti DEMF.                                                                                                                                  |
| ROUNDAMOUNT (číslo, desetinná místa, pravidlo zaokrouhlování)                                       | Zaokrouhlí zadanou částku podle zadaného pravidla zaokrouhlování a zadaného počtu desetinných míst. **Poznámka:** Pravidlo zaokrouhlování musí být zadáno jako hodnota výčtu **RoundOffType** aplikace Dynamics 365 for Operations.                          | Pokud **modelu. Vznikajícím** parametr nastaven na *** Downward *** **ROUNDAMOUNT (1000.787, 2, model. Vznikajícím)** vrátí hodnotu **1000.78**. Pokud je parametr **model.RoundOff** nastaven na hodnotu **Normal** nebo **Rounding-up**, **ROUNDAMOUNT (1000.787, 2, model.RoundOff)** vrátí hodnotu **1000.79**. |
...
| KN\_GBT\_AdditionalDimensionID (řetězec, číslo)                                  | Získá ID další finanční dimenze. Dimenze reprezentují v tomto řetězci ID oddělená čárkou. Čísla definují požadovaný kód číselné řady dimenze v tomto řetězci.                                                                            | KN\_GBT\_AdditionalDimensionID ("AA, BB, CC, DD, EE, FF, GG, HH", 3) vrátí "CC"                                                                                                                                                                                                                                      |
| GetCurrentCompany ()                                                             | Vrátí kód aktuální protokolované společnosti.                                                                                                                                                                                                                    |                                                                                                                                                                                                                                                                                                               |
| CH\_banky\_MOD\_10 (číslice)                                                       | Vrátí referenční údaj věřitele jako výraz MOD10 na základě číslic daného čísla faktury.                                                                                                                                                                      | CH\_banky\_MOD\_vrátí hodnotu 3, 10 ("VEND-200002")                                                                                                                                                                                                                                                                   |
| IM\_SUM (dlouhodobý majetek kód, kód modelu hodnotu, počáteční datum, koncové datum)               | Vrátí připravený datový kontejner částek dlouhodobého majetku za období.                                                                                                                                                                                         | IM\_SUM ("COMP-000001", "Aktuální", datum1, datum2) vrátí kontejner připravených dat dlouhodobého majetku "COMP-000001" s oceňovacím modelem "Aktuální" na období od Datum1 Datum2.                                                                                                                        |
| IM\_(dlouhodobého majetku kód, kód hodnoty modelu vykazovaný rok, datum oznámení) | Vrátí připravený datový kontejner zůstatků dlouhodobého majetku. Rok vykazování je nutné zadat jako výčet hodnot for Operations **AssetYear**.                                                                                           | IM\_SUM ("COMP-000001", "Aktuální", AxEnumAssetYear.ThisYear, SESSIONTODAY () vrátí kontejner připravená data zůstatků dlouhodobého majetku "COMP-000001" s oceňovacím modelem "Aktuální" na aktuální 365 pro operace data relace.                                                                |
