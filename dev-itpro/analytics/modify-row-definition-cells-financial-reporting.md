...
-   Popisek musí začínat abecedním znakem (a až z nebo A až Z) a může být libovolnou kombinací číslic a písmen do maximálně 16 znaků. **Poznámka:** Popisek může zahrnovat znak podtržítka (\_), ale nejsou povoleny žádné jiné speciální znaky.
...
3.  V dialogovém okně **Přečíslovat řádky** zadejte nové hodnoty pro počáteční kód řádku a přírůstek kódu řádku. Můžete obnovit číselné kódy řádků na rovnoměrně rozložené hodnoty. Návrhář sestav však přečísluje pouze kódy řádků začínající číslicemi (například 130 nebo 246). Nepřečísluje kódy řádků, které začínají písmeny (například INCOME\_93 nebo TP0693) **Poznámka:** Při přečíslování kódů řádků návrhář sestav automaticky aktualizuje odkazy **TOT** a **CAL**. Pokud například řádek **TOT** odkazuje na rozmezí, které začíná kódem řádku 100, a přečíslujete řádky počínaje od 90, počáteční odkaz **TOT** se změní ze 100 na 90.
...
3.  V seznamu vyberte kód formátu. Následující tabulka popisuje kódy formátu a jejich akce.
    ...
    | SORT ASORT SORTDESC ASORTDESC | Seřadit                               | Seřadí výdaje nebo výnosy, seřadí sestavu skutečné nebo rozpočtové odchylky podle největší odchylky, nebo abecedně seřadí popisy řádků.                                                                   |
...
Typ by měl představovat kladnou hodnotu a poté zápornou hodnotu. Obvykle zadáte podobný formát, který odlišuje kladné a záporné hodnoty. Pokud chcete například určit, že kladné i záporné hodnoty mají mít dvě desetinná místa, ale záporné hodnoty se zobrazí v závorkách, zadejte hodnotu **0,00;(0,00)**. Následující tabulka obsahuje vlastní formáty, které můžete použít ke kontrole formátu hodnot. Všechny příklady začínají hodnotou 1234,56.
...
Ačkoli můžete upravovat účty přímo, můžete použít také dialogové okno **Dimenze** k aplikaci správného formátování na vaše odkazy na finanční údaje. Všechny hodnoty mohou obsahovat zástupné znaky (? nebo \*). Návrhář sestav však nemůže vybrat účty, dimenze nebo pole ze systému Microsoft Dynamics ERP, které obsahují kterékoli z následujících vyhrazených znaků: : &, \*, \[, \], {, or }. **Poznámka:** K odečtení hodnot musíte tyto hodnoty umístit do závorek. Zadáte-li například hodnotu **450?-(4509)**, bude zobrazena jako **+Účet==\[4509\]-Účet=\[450?\]** a říká návrháři sestav, aby odečetl částku segmentu účtu 4509 od částky jakéhokoli segmentu účtu, který má na začátku 450.
...
| Položka                | Popis                                                                                                                                                                                                                                                                                             |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Jednotlivec nebo rozsah | Do pole **Od** zadejte název účtu nebo klikněte na tlačítko **Procházet** ![Procházet](https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "Procházet") k vyhledání účtu. K výběru rozsahu zadejte nebo vyhledejte hodnotu pro pole **Do**.                                             |
| Sada hodnot dimenze | Do pole **Název** zadejte název sady hodnot dimenze. Chcete-li vytvořit, upravit, kopírovat nebo odstranit sadu, klikněte na tlačítko **Správa sad hodnot dimenzí**. Pole **Vzorec** je vyplněno vzorcem z buňky **Odkaz na finanční dimenze** pro tuto sadu hodnot dimenze v definici řádku. |
