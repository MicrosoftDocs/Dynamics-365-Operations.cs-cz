...
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | Potlačit podtržení sloupce, pokud se provedení příkazu podtržítka (**---**) nebo příkazu dvojité podtržítko (**===**) je zadán v definici řádku. Nemusí být vhodné například podtrhovat částky, které jsou vytvářeny podle výpočtu procentuální hodnoty.                                                                        |
...
| Datum zahájení (@StartDate)                 | Vytiskne počáteční datum pro sloupec.                                                                                                                                                                                                                                                             |
...
-   Pokud zadáte záhlaví sloupců, které se budou překrývat, budou se překrývat i při tisku sestavy. Sestava je vygenerována, ale zobrazí se následující upozornění v **zprávy o stavu fronty** pole: "záhlaví sloupců pomocí Base průsečík s další záhlaví sloupců a může způsobit překrývající se text." Například definice záhlaví sloupce B má základní + 1 B a definice záhlaví ve sloupci D je základní + 1 F. V tomto případě záhlaví jsou vytištěny na sebe a nečitelné. Vždy při použití funkce BASE v definici **Pokrýt od / Pokrýt k** si prohlédněte vygenerovanou sestavu a zkontrolujte, zda se záhlaví nepřekrývají.
...
-   Podmíněný tisk sloupců (**P&lt;B**, **P&gt;= B**), podmíněné spanning záhlaví chovat jako definice jakékoliv pravidelné sloupec záhlaví. Například pokud podmínka není splněna, všechny následné sloupce splňující podmínku pokrytí zahájí tisk záhlaví.
...
3.  V seznamu vyberte typ sloupce. Následující tabulka popisuje různé typy sloupců.
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>Pokud váš účetní systém podporuje atributy, zobrazí ve sloupci atribut účtu nebo transakce. Atribut, který musí platit pro jeden úplný účet, získá související informace o účtu nebo transakci z finančních dat. Atributy na úrovni účtu se zobrazí data z účtu a atributy na úrovni transakce zobrazení dat, k níž došlo v době, která byla transakce zaúčtována. Vyberete-li <strong>ATTR</strong> typ sloupce, určete atribut kategorie v <strong>kód/atribut kategorie knihy</strong> řádek podrobností definice sloupce.</td>
    </tr>
    </tbody>
    </table>
...
| ZÁKLADNÍ +\# | Rok, který je pomocí \#let za základní rok. Například pro použití třetího roku po základním roku zadejte hodnotu **BASE+3**. |
| BASE-\# | Rok, který je pomocí \#let před základní rok. Chcete-li například použít předchozí rok, zadejte hodnotu **BASE-1**.                 |
...
| ZÁKLADNÍ +\#         | Období, pro které je použití \#období po základním období. Například pro použití třetího období po základním období zadejte hodnotu **BASE+3**.                                                                                               |
| BASE-\#         | Období, pro které je použití \#období před základním období. Chcete-li například použít předchozí období, zadejte hodnotu **BASE-1**.                                                                                                                 |
...
| ZÁKLAD: ZÁKLAD +\#    | Použije více období, od základního období až po několik období po základním období. Například pro použití základního období a dvou následujících období zadejte hodnotu **BASE:BASE+2**.                                                  |
| BASE -\#: základní +\# | Použije více období, od několika období před základním obdobím až po několik období po základním období. Například pro použití tři předchozích období, základního období a dvou následujících období zadejte hodnotu **BASE-3:BASE+2**. |
...
Můžete jít za hranice fiskálního roku v kterémkoli určení období a můžete kombinovat roky v rozmezí období. Například určit období jako **BASE-5** (představující posledních šest období) a spustit sestavu, která má základní období 2. V tomto případě sestava zobrazuje data pro první dvě období určené fiskálního roku a poslední čtyři období předchozího fiskálního roku.
...
3.  Na řádku vzorce nad navigačním podoknem nebo v buňce **Období** zadejte vzorec. Nahradit libovolný znak (\#) s odpovídající hodnotou.
...
Filtr dimenzí slouží k omezení sloupce **FD** na konkrétní hodnoty dimenzí. Filtr může zahrnovat jednu dimenzi, rozsah dimenzí nebo skupinu dimenzí. Filtr může také obsahovat sady hodnot dimenzí. Protože můžete měnit hodnoty dimenze,... \financial-dimensions\dimension-Based systém nemusí odpovídat přesnou délku. Filtr se použije bez ohledu na to, zda sestava obsahuje strom výkaznictví, či nikoli. Můžete použít zástupný znak (\* nebo?) v jakékoliv poloze. Pokud chcete zadat více účtů, nevložili čárku mezi účty, jako v následujícím příkladu: + účet =\[1200\], + účet =\[1100\], oddělení =\[01?\] Pokud chcete získat všechna oddělení pro určitý účet, můžete vyloučit dimenzi Oddělení z filtru dimenzí. Například oba následující filtry dimenzí jsou zpracovány stejným způsobem:

-   + Účet =\[1100\], oddělení
-   + Účet =\[1100\]

Můžete také použít jakoukoli kombinaci alfanumerických znaků pro přesnou shodu a můžete definovat částečné dimenze. Například **umístění = \[10\*\]** zahrnuje všechna umístění hodnoty dimenze, které začínají s 10.
...
-   **Zobrazení měny** – určete typ měny (fyzické, funkční nebo vykazování) zobrazená v transakcích. Tato funkce je někdy označovány jako převod měny. Převod měny je možnost vykazovat částky hlavní knihy v měně, která nemusí být funkční měnou společnosti nebo měnou, ve které byla zadána transakce.
...
**Poznámka:** Možnosti zde popsané se mohou lišit v závislosti na systému ERP. Další informace naleznete v [dokumentace systému Microsoft ERP](https://www.microsoft.com/en-us/download/details.aspx?id=5916).
...
| Funkční měna z informací o společnosti | Prázdné                | **$2,310\*\*** – výsledek zobrazuje všechna data ve funkční měně, který je určen v informacích společnosti. **Poznámka:** Tato částka je součtem všech transakcí ve funkční měně. |
...
Můžete vytvořit sloupec zobrazující všechny hodnoty ve určeném sloupci jako procento základního čísla. Proto je možné zobrazit vztahy mezi řádky, například procento řádku prodeje nebo procento řádku celkových výdajů. Násobení nebo dělení každý řádek v určitého sloupce v řádku základní, zadejte sloupce, které chcete použít ve výpočtech a potom zadejte **\*BASEROW** nebo **/BASEROW**. Například zadejte **C\*BASEROW** nebo **C/BASEROW**. ** Poznámka:** Když použijete výpočet základního řádku v definici sloupce, ujistěte se, že každá definice řádku použitá s touto definicí sloupce obsahuje alespoň jeden základní řádek pro výpočty.
