...
description: "Tento článek poskytuje přehled výrobních procesů. Popisuje různé fáze výrobních zakázek, dávkové objednávky a kanbany, od vytvoření objednávky k uzavření finančního období."
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
Tento článek poskytuje přehled výrobních procesů. Popisuje různé fáze výrobních zakázek, dávkové objednávky a kanbany, od vytvoření objednávky k uzavření finančního období. 
...
**Řízení výroby** modul je propojen s jinými moduly, jako **řízení informací o produktech**, **řízení zásob**, **financí**, **řízení skladu**, **účetnictví projektu**, a **Správa organizace**. Tato integrace podporuje tok informací, který je požadován k dokončení výroby dokončené položky.  

Výrobní proces je obvykle ovlivněn nákladovým účetnictvím a metodami ocenění zásob, které byly vybrány pro konkrétní výrobní proces. Dynamics 365 pro operace podporuje obě skutečné náklady (první v první out \[FIFO\]; poslední, první ven \[LIFO\]; klouzavý průměr a pravidelné vážený průměr) a standardní nákladové metody. Štíhlá výroba je implementována na základě pravidla pro zpětné účtování nákladů.  
...
Jiné produkty a topologie výroby vyžadují použití různých typů objednávek. Dynamics 365 pro operace můžete použít různé typy zakázek v kombinovaném režimu. Jinými slovy ke všem objednávkám může dojít od začátku do konce při výrobě jednoho konečného produktu.
...
2.  **Odhadováno** – Můžete vypočítat odhady spotřeby materiálu a zdrojů. Odhad generuje skladové transakce pro suroviny, které mají stav **Na objednávce**. Když výrobní zakázky se generují příjmy pro hlavní produkty, druhotné produkty a vedlejší produkty nebo se odhadují dávkové objednávky. Pokud Kusovník obsahuje řádky **doloženo zásobování** zadejte nákupní objednávky pro materiály nebo služby pro subdodavatelské operace jsou generovány a doloženo dávkovou objednávku nebo výrobní zakázku. Položky nebo objednávky se rezervují podle strategie rezervace výrobní zakázky a cena dokončeného zboží je založena na nastavení parametrů.
3.  **Plánováno** – K plánování výroby můžete použít operace, jednotlivé úkoly nebo obojí.
