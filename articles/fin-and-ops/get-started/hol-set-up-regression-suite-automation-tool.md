...
- Vysvětlení, jak funkce Záznamník úloh v Microsoft Dynamics 365 for Finance and Operations spolu s funkcí Microsoft Dynamics Lifecycle Services (LCS) a Microsoft Azure DevOps umožňují vytvářet, konfigurovat, spouštět, zkoumat a vykazovat testovací případy.
- Pomocí záznamníku úkolů v aplikaci Finance and Operations můžete funkčním uživatelům umožnit záznam obchodních úkolů a poté převést záznamy úkolů na sadu automatizovaných testů, aniž by bylo nutné vytvářet zdrojový kód.

## <a name="before-you-begin"></a>Než začnete

### <a name="prerequisites"></a>Předpoklady

- V tomto kurzu je vyžadováno prostředí Finance and Operations s aplikací Microsoft Dynamics 365 for Finance and Operations verze 10.0 (duben 2019) nebo novější. Pro zákazníky, kteří používají Microsoft Dynamics 365 for Finance and Operations Enterprise edition 7.3, je podporována také Platform update 20 (PU20) nebo vyšší.
- Uživatel musí mít oprávnění správce prostředí Finance and Operations.
...
## <a name="finance-and-operations-environment"></a>Prostředí Finance and Operations

### <a name="version-requirement"></a>Požadavek na verzi

Je požadován test Finance and Operations nebo prostředí sandbox, na kterém běží verze 10. Pro zákazníky, kteří používají verzi 7.3, je podporována také Platform update 20 nebo vyšší.

> [!NOTE]
> Nástroj RSAT musí mít přístup k testovacímu prostředí Finance and Operations prostřednictvím webového prohlížeče.
...
Tento krok je nutný pro připojení s LCS, aby bylo možné ukládat záznamy úkolů do příslušné knihovny BPM v systému LCS prostřednictvím klienta Finance and Operations.

1. Otevrete klienta Finance and Operations.
...
> [!NOTE]
> Zkontrolujte, zda jsou všechny záznamy úkolů v hlavním řídicím panelu modulu Finance and Operations zahájeny. Jednotlivé nahrávky uchovávejte tak, aby byly krátké, a zaměřujte se na jeden obchodní úkol, jako je vytvoření prodejní objednávky.
...
RSAT lze nainstalovat do libovolného počítače se systémem Windows 10, který se může připojit k prostředí Finance and Operations pomocí webového prohlížeče (Internet Explorer nebo webu Google Chrome).
...
> [!NOTE]
> Pokud je prostředí Finance and Operations prostředím úrovně 2 +, kryptografický otisk certifikátu musí být aktualizován v souboru WIF. config pro **všechny** počítače AOS pro dané prostředí.
...
4. Aktualizujte **soubor wif.config** přidáním nového záznamu autority pro váš certifikát a název úřadu, jak je uvedeno v následujícím příkladu.
    ...
    > [!NOTE]
    > Pokud více uživatelů používá stejnou aplikaci Finance and Operations, musí každý uživatel generovat samostatné kryptografické otisky a každý z těchto kryptografických otisků musí být přidán v části **\<klíče\>**.
...
Pokud se používá vrstva 2 + (standardní prostředí sandbox pro příjem testů nebo vyšší) prostředí Finance and Operations, ověřte nebo nastavte následující nastavení registru v počítači, ve kterém je nainstalována sada RSAT. Tento krok je požadován, protože pomáhá předcházet chybám ověřování a RSAT.
...
3. Vyberte tlačítko **Nastavení** (symbol ozubeného kola) v pravém horním rohu a potom v zobrazeném dialogovém okně vyplňte následující pole:
    ...
    - **Název hostitele** – zadejte název hostitele testovacího prostředí Finance and Operations, například **\<myaos\>. cloudax.dynamics.com.** Nezahrnujte předponu **https://** nebo **http://**.
    - **Název hostitele SOAP** – zadejte název hostitele SOAP pro testovací prostředí Finance and Operations. Název hostitele SOAP se obvykle shoduje s názvem hostitele, ale má příponu **SOAP**. Zde je příklad: **\<myaos\>soap.cloudax.dynamics.com**. Nezahrnujte předponu **https://** nebo **http://**.
    ...
    - **Uživatelské jméno správce** – zadejte e-mailovou adresu uživatele správce v testovacím prostředí Finance and Operations.
    ...
    - **Název společnosti** – zadejte název společnosti, který má být použit jako výchozí společnost Finance and Operations při vytváření souborů parametrů aplikace Excel. Společnost lze později změnit úpravou souboru parametrů aplikace Excel.
...
3. Chcete-li pro každý běh generovat jedinečné číslo produktu bez nutnosti znovu otevřít soubor parametrů aplikace Excel a ručně aktualizovat číslo produktu, použijte následující vzorec aplikace Excel.
    ...
    > [!NOTE]
    > Kromě karty **obecné** obsahuje soubor parametrů aplikace Excel kartu data pro každou stránku formuláře Finance and Operations pro návštěvy testovacího případu.
