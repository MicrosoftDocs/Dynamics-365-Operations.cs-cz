# [Dynamics 365 for Finance and Operations](index.md)

## Začínáme
### [Před zakoupením](get-started/before-you-buy.md)
#### [Registrace ukázkového předplatného](../dev-itpro/dev-tools/sign-up-preview-subscription.md)
#### [Nasazení ukázkového prostředí](../dev-itpro/deployment/deploy-demo-environment.md)
#### [Srovnání funkce cloudu a on-premises](get-started/cloud-prem-comparison.md)
#### [Výběr možnosti nasazení](../dev-itpro/deployment/choose-deployment-type.md)
##### [Nasazení cloudu](../dev-itpro/deployment/cloud-deployment-overview.md)
##### [Systémové požadavky pro nasazení cloudu](get-started/system-requirements.md)
##### [Nasazení on-premises](../dev-itpro/deployment/on-premises-deployment-landing-page.md)
##### [Systémové požadavky pro místní nasazení](get-started/system-requirements-on-prem.md)

#### Zakoupení aplikace Finance and Operations
##### [Microsoft FastTrack for Dynamics 365](get-started/fasttrack-dynamics-365-overview.md)
##### [Zakoupení aplikace Finance and Operations (on-premises)](get-started/purchase-on-premises.md)

### [Co je nového a co se změnilo](get-started/whats-new-changed.md)
#### [Zastaralé funkce](../dev-itpro/migration-upgrade/deprecated-features.md)
#### [Dosud neimplementované funkce aplikace AX 2012](get-started/ax-2012-features-not-implemented-but-not-deprecated.md)
#### [Funkce neimplementované v místních nasazeních](get-started/features-not-implemented-on-prem.md)
#### [Zásady životního cyklu](../dev-itpro/migration-upgrade/versions-update-policy.md)
#### [Platform update 11 (říjen 2017)](get-started/whats-new-platform-update-11.md)
#### [Platform update 10 (srpen 2017)](get-started/whats-new-platform-update-10.md)
#### [Finance and Operations, Enterprise Edition, (červenec 2017)](get-started/whats-new-application-July-2017-update.md)
#### [Platform update 9 (červenec 2017)](get-started/whats-new-platform-update-9.md)
#### [Platform update 8 (červen 2017)](get-started/whats-new-platform-update-8.md)
#### [Platform update 7 (květen 2017)](get-started/whats-new-platform-update-7.md)
#### [Platform update 6 (duben 2017)](get-started/whats-new-platform-update-6.md)
#### [Platform update 5 (březen 2017)](get-started/whats-new-platform-update-5.md)
#### [Platform update 4 (únor 2017)](get-started/whats-new-platform-update-4.md)
#### [Operations, verze 1611 (listopad 2016)](get-started/whats-new-dynamics-365-operations-1611.md)
#### [Platform update 3 (listopad 2016)](get-started/whats-new-platform-update-3.md)
#### [Platform update 2 (srpen 2016)](get-started/whats-new-platform-update-2.md) 
#### [Aplikace Dynamics AX, verze 7.0.1 (květen 2016)](get-started/whats-new-changed-application-version-7-0-1-may-2016.md)
#### [Platform update 1 (květen 2016)](get-started/whats-new-changed-platform-version-7-1-may-2016.md)
#### [Dynamics AX 7.0 (únor 2016)](get-started/whats-new-changed-7-0-february-2016.md)
#### Aktualizovaní průvodci záznamem úloh
##### [Průvodci záznamem úloh - listopad 2016](get-started/new-task-guides-november-2016.md)
##### [Průvodci záznamem úloh - srpen 2016](get-started/new-updated-task-guides-available-august-2016.md)
##### [Průvodci záznamem úloh - květen 2016](get-started/new-updated-task-guides-available-may-2016.md)
##### [Průvodci záznamem úloh - únor 2016](get-started/new-task-guides-available-february-2016.md)

### Prozkoumání uživatelského rozhraní
#### [Časté dotazy týkající se klientského uživatelského rozhraní](get-started/client-faq.md)
#### [Vyhledání akce](get-started/action-search.md)
#### [Pokročilé filtrování a syntaxe dotazu](get-started/advanced-filtering-query-options.md)
#### [Konfigurace a filtrování pracovních prostorů](get-started/configure-filter-workspaces.md)
#### [Zobrazení stránek vedle sebe pomocí ikony Otevřít v novém okně](get-started/display-pages-side-by-side.md)
#### [Klávesové zkratky](get-started/shortcut-keys.md)
#### [Změna nápisu nebo loga](get-started/tasks/change-banner-or-logo.md)
#### [Hledání za účelem navigace](get-started/navigation-search.md)
#### [Přizpůsobení uživatelského prostředí](get-started/personalize-user-experience.md)
#### [Použití vyhledávání pro nalezení informace](get-started/use-lookups-to-find-information.md)
#### [Změna data pro relaci](organization-administration/tasks/change-date-session.md)
#### [Nastavení upřednostňovaného časového pásma uživatele](organization-administration/tasks/set-users-preferred-time-zone.md)
#### [Vysvětlení služeb Lifecycle Services](../dev-itpro/lifecycle-services/lcs-works-lcs.md)

### Získání nápovědy a podpory
#### [Přehled nápovědy](get-started/help-overview.md)
#### [Připojení nápovědy](get-started/help-connect.md)
#### [Zobrazení a export popisů polí](get-started/view-export-field-descriptions.md)
#### Průvodci záznamem úloh a záznamník úloh
##### [Záznamník úloh](../dev-itpro/user-interface/task-recorder.md)
##### [Vytvoření dokumentace nebo školení](../dev-itpro/user-interface/task-recorder-training-docs.md)
#### [Glosář](get-started/glossary.md)

## Shoda s předpisy
### [Pravidla zásad auditu](../financials/general-ledger/audit-policy-rules.md)
### [Porušení a případy zásad auditu](../financials/general-ledger/audit-policy-violations-cases.md)

## Správa nákladů
### Balíček obsahu Power BI
#### [Balíček obsahu správy nákladů](../dev-itpro/analytics/cost-management-content-pack.md)
### Předem stanované náklady
#### [Nákladový formulář](../supply-chain/cost-management/costing-sheets.md)
#### [Nákladová verze](../supply-chain/cost-management/costing-versions.md)
#### [Simulace změny nákladů pomocí nákladové verze pro plánované náklady](../supply-chain/cost-management/simulate-cost-changes-costing-version-planned-costs.md)

### Výpočet nákladů zásob
#### [Úprava hodnoty nákladové ceny množství na skladě](../supply-chain/cost-management/adjust-hand-inventory-cost-values.md)
#### [Objekt nákladů](../supply-chain/cost-management/cost-object.md)
#### [Záznam nákladů](../supply-chain/cost-management/cost-entries.md)
#### [Nákladová skupina](../supply-chain/cost-management/cost-groups.md)
#### [Metoda FIFO s fyzickou hodnotou a označením](../supply-chain/cost-management/fifo-physical-value-marking.md)
#### [Zahrnutí fyzické hodnoty](../supply-chain/cost-management/include-physical-value.md)
#### [Hodnota objektu](../supply-chain/cost-management/physical-quantity.md)
#### [Uzavřít zásoby](../supply-chain/cost-management/inventory-close.md)
#### [Datum LIFO s fyzickou hodnotou a označením](../supply-chain/cost-management/lifo-date-physical-value-marking.md)
#### [Metoda LIFO s fyzickou hodnotou a označením](../supply-chain/cost-management/lifo-physical-value-marking.md)
#### [Klouzavý průměr](../supply-chain/cost-management/moving-average.md)
#### [Fyzické a finanční aktualizace](../supply-chain/cost-management/physical-financial-updates.md)
#### [Průběžná průměrná nákladová cena](../supply-chain/cost-management/running-average-cost-price.md)
#### [Sledování průběžných průměrných nákladů pro dimenzi zboží](../supply-chain/cost-management/track-running-average-cost-per-inventory-dimension.md)
#### [Datum váženého průměru](../supply-chain/cost-management/weighted-average-date.md)
#### [Vážený průměr s fyzickou hodnotou a označením](../supply-chain/cost-management/weighted-average-physical-value-marking.md)
### Výpočet nákladů výroby
#### [Amortizace konstantních nákladů na vyráběnou položku](../supply-chain/cost-management/amortize-constant-costs-manufactured-item.md)
#### [Zpětné účtování nákladů](../supply-chain/cost-management/backflush-costing.md)
#### [Výpočty kusovníku](../supply-chain/cost-management/bom-calculations.md)
#### [Skupina výpočtů kusovníku](../supply-chain/cost-management/bom-calculation-groups.md)
#### [Běžné zdroje výrobní odchylky](../supply-chain/cost-management/common-sources-of-production-variances.md)
#### [Nákladová kategorie použitá ve výrobních postupech](../supply-chain/cost-management/cost-categories-used-production-routings.md)
#### [Nákladová kategorie, která se používá v modulu Řízení výroby a Řízení a účetnictví projektů](../supply-chain/cost-management/cost-categories-used-production-control-project-management-accounting.md)
#### [Zobrazení nákladů na vyráběnou položku](../supply-chain/cost-management/charges-manufactured-item.md)
#### [Analýza nákladů ve výrobní zakázce](../supply-chain/cost-management/production-order-cost-analysis.md)
#### [Odhad nákladů výrobní zakázky](../supply-chain/cost-management/production-order-cost-estimation.md)
#### [Zaúčtování výroby](../supply-chain/cost-management/production-posting.md)
#### [Nastavení produktu, který může být vyroben nebo pořízen](../supply-chain/cost-management/manufactured-items-treated-as-purchased-items.md)
#### [Metoda přidělení celkových nákladů](../supply-chain/cost-management/methodology-total-cost-allocation.md)
### Standardní náklady
#### [Kalkulace kusovníků se standardními náklady](../supply-chain/cost-management/information-used-bom-calculations-standard-costs.md)
#### [Správa aktualizací standardních nákladů](../supply-chain/cost-management/manage-standard-cost-updates.md)
#### [Předpoklad pro převod standardních nákladů](../supply-chain/cost-management/prerequisites-standard-cost-conversion.md)
#### [Přehled převodu standardních nákladů](../supply-chain/cost-management/standard-cost-conversion-overview.md)
#### [Aktualizace standardních nákladů ve výrobním prostředí](../supply-chain/cost-management/update-standard-costs-manufacturing-environment.md)
#### [Aktualizace standardních nákladů pro nově vyrobené položky](../supply-chain/cost-management/update-standard-costs-new-manufactured-item.md)
#### [Aktualizace standardních nákladů v nevýrobním prostředí](../supply-chain/cost-management/update-standard-costs-non-manufacturing-environment.md)

### Úkoly správy nákladů
#### [Zobrazení nákladových položek pro objekt nákladů](../supply-chain/cost-management/tasks/view-cost-entries-cost-object.md)
#### [Zobrazení zůstatku objektu nákladů](../supply-chain/cost-management/tasks/view-cost-object-balance.md)
#### [Zobrazení aktuálního stavu NV u výrobní zakázky](../supply-chain/cost-management/tasks/view-current-wip-status-production-order.md)
#### [Zobrazení výkazu zásob a ukazatele KPI podle objektů nákladů](../supply-chain/cost-management/tasks/view-inventory-statement-kpi-cost-object.md)
#### [Porovnání aktivních, odhadovaných a realizovaných nákladů u výrobní zakázky](../supply-chain/cost-management/tasks/compare-active-estimated-realized-costs.md)
#### [Vytvoření nákupní objednávky řízené rozpočtem](../supply-chain/cost-management/tasks/create-purchase-order-governed-by-budget.md)
#### Výpočty kusovníku pomocí nákladového formuláře
##### [Vytvoření dokončeného produktu (jen ve verzi z února 2016)](../supply-chain/pim/tasks/create-finished-product-2016-02.md)
##### [Vytvoření polotovaru produktu (jen ve verzi z února 2016)](../supply-chain/pim/tasks/create-semi-finished-product-2016-02.md)
##### [Vytváření surovin (je ve verzi z února 2016)](../supply-chain/pim/tasks/create-raw-materials-2016-02.md)
##### [Vytváření kusovníků (jen ve verzi z února 2016)](../supply-chain/pim/tasks/create-boms-2016-02.md)
##### [Vytváření postupů (jen ve verzi z února 2016)](../supply-chain/pim/tasks/create-routes-2016-02.md)
##### [Výpočet kusovníku pomocí jednoúrovňové struktury (jen ve verzi z února 2016)](../supply-chain/cost-management/tasks/calculate-bom-single-level-structure-2016-02.md)
##### [Výpočet kusovníku pomocí víceúrovňové struktury (jen ve verzi z února 2016)](../supply-chain/cost-management/tasks/calculate-bom-multilevel-structure-2016-02.md)


## Integrace a správa dat

### [Datové entity, projekty a balíčky](../dev-itpro/data-entities/data-entities.md)
#### [Architektura datových entit a balíčků](../dev-itpro/data-entities/data-entities-data-packages.md)
##### [Projekty dat konfigurace](../dev-itpro/data-entities/configuration-data-projects.md)
##### [Balíčky dat konfigurace](../dev-itpro/data-entities/configuration-data-packages.md)
##### [Zkopírování dat konfigurace mezi společnostmi](../dev-itpro/data-entities/copy-configuration.md)
##### [Šablony dat konfigurací](../dev-itpro/data-entities/configuration-data-templates.md)
##### [Sledování změn entity](../dev-itpro/data-entities/entity-change-track.md)

#### Standardní datové entity
##### [Sestava datových entit](../dev-itpro/data-entities/data-entities-report.md)
##### [Datové entity: Závazky a daně](../dev-itpro/data-entities/data-entities-accounts-payable-taxes.md)
##### [Datové entity: Pohledávky](../dev-itpro/data-entities/data-entities-accounts-receivable.md)
##### [Datové entity: Banka](../dev-itpro/data-entities/data-entities-bank.md)
##### [Datové entity: Počáteční zůstatky](../dev-itpro/data-entities/data-entities-beginning-balances.md)
##### [Datové entity: Plánování rozpočtu](../dev-itpro/data-entities/data-entities-budget-plan.md)
##### [Datové entity: Rozpočtování](../dev-itpro/data-entities/data-entities-budgeting.md)
##### [Datové entity: Výpočet nákladů](../dev-itpro/data-entities/data-entities-costing.md)
##### [Datové entity: Správa výdajů](../dev-itpro/data-entities/data-entities-expense-management.md)
##### [Datové entity: Dlouhodobý majetek](../dev-itpro/data-entities/data-entities-fixed-assets.md)
##### [Datové entity: Hlavní kniha](../dev-itpro/data-entities/data-entities-general-ledger.md)
##### [Datové entity: Globální adresář](../dev-itpro/data-entities/data-entities-global-address-book.md)
##### [Datové entity: Lidské zdroje](../dev-itpro/data-entities/data-entities-human-resources.md)
##### [Datové entity: Řízení zásob](../dev-itpro/data-entities/data-entities-inventory-management.md)
##### [Datové entity: Hlavní plánování](../dev-itpro/data-entities/data-entities-master-plan.md)
##### [Datové entity: Zásobování a zdroje](../dev-itpro/data-entities/data-entities-procurement-sourcing.md)
##### [Datové entity: Řízení informací o produktu](../dev-itpro/data-entities/data-entities-product-information-management.md)
##### [Datové entity: Řízení výroby](../dev-itpro/data-entities/data-entities-production-control.md)
##### [Datové entity: Projektové účetnictví](../dev-itpro/data-entities/data-entities-project-accounting.md)
##### [Datové entity: Veřejný sektor](../dev-itpro/data-entities/data-entities-public-sector.md)
##### [Datové entity: Maloobchod](../dev-itpro/data-entities/data-entities-retail.md)
##### [Datové entity: Provádění prodeje a marketingu](../dev-itpro/data-entities/data-entities-sales-marketing.md)
##### [Datové entity: Předprodej při prodeji a marketingu](../dev-itpro/data-entities/data-entities-sales-marketing-pre-sales.md)
##### [Datové entity: Správa systému](../dev-itpro/data-entities/data-entities-system-administration.md)
##### [Datové entity: Workflow](../dev-itpro/data-entities/data-entities-workflow.md)

#### [Úlohy importu a exportu dat](../dev-itpro/data-entities/data-import-export-job.md)

##### [Osvědčené postupy pro import dokladů pomocí entity hlavního deníku](../dev-itpro/data-entities/tips-tricks-import-general-journal-entity.md)

#### Vývoj pro datové entity
##### [Vytváření nových datových entit](../dev-itpro/data-entities/build-consuming-data-entities.md)
##### [Vlastnosti datových entit](../dev-itpro/data-entities/behavioral-properties-data-entities.md)
##### [Vytváření výpočtu sloupců a virtuálních polí](../dev-itpro/data-entities/data-entity-computed-columns-virtual-fields.md)
##### [Chování napříč společnostmi](../dev-itpro/data-entities/cross-company-behavior.md)
##### [Zpracování kódů zemí/oblastí](../dev-itpro/data-entities/countryregion-codes-configuration-keys.md)
##### [Vzory dědičnosti](../dev-itpro/data-entities/support-super-type-sub-type.md)
##### [Pravidla průvodce datovými entitami](../dev-itpro/data-entities/data-entity-wizard-rules.md)
##### [Vlastnosti metadat](../dev-itpro/data-entities/behavioral-properties-data-entities.md)
##### [Ověření, výchozí hodnoty a nemapovaná pole](../dev-itpro/data-entities/validations-defaults-unmapped-fields.md)
##### [Zabezpečení a datové entity](../dev-itpro/data-entities/security-data-entities.md)

### Integrace dat
#### [Integrace dat s použitím datových entit](../dev-itpro/data-entities/data-management-integration-data-entity.md)
##### [Vývoj entity pro migraci dat](../dev-itpro/data-entities/develop-entity-for-data-migration.md)
##### [Vývoj kombinované datové entity](../dev-itpro/data-entities/develop-composite-data-entities.md)
##### [Konfigurace sdílení finančních dat mezi společnostmi](../dev-itpro/data-entities/tasks/configure-financial-cross-company-data-sharing.md)
##### [Vytvoření šablony záznamu pro usnadnění zadávání dat](../dev-itpro/data-entities/tasks/create-record-template-facilitate-data-entry.md)
##### [Vytvoření nového záznamu s použitím šablony záznamu](../dev-itpro/data-entities/tasks/use-record-template-new-record.md)

#### API integrace dat
##### [API balíčku správy dat](../dev-itpro/data-entities/data-management-api.md)

#### Vzory integrace
##### [Koncové body služby](../dev-itpro/data-entities/services-home-page.md)
###### [Řešení problému s ověřením služby](../dev-itpro/data-entities/troubleshoot-service-authentication.md)

##### [OData](../dev-itpro/data-entities/odata.md)
##### [Opakované integrace](../dev-itpro/data-entities/recurring-integrations.md)

#### [Integrace dat s použitím služby Common Data Service](../dev-itpro/data-entities/data-integration-cds.md) 
##### [Konfigurace scénáře zpeněžení potenciálního zákazníka](../supply-chain/sales-marketing/prospect-to-cash.md)
###### [Synchronizace produktů z aplikace Finance and Operations na produkty v aplikaci Sales](../supply-chain/sales-marketing/products-template-mapping.md)
###### [Synchronizace obchodních vztahů z aplikace Sales na odběratele v aplikaci Finance and Operations](../supply-chain/sales-marketing/accounts-template-mapping.md)
###### [Synchronizace kontaktů z aplikace Sales na kontakty nebo odběratele v aplikaci Finance and Operations](../supply-chain/sales-marketing/contacts-template-mapping.md)
###### [Synchronizace hlaviček a řádků prodejních nabídek z aplikace Sales do aplikace Finance and Operations](../supply-chain/sales-marketing/sales-quotation-template-mapping.md)
###### [Synchronizace hlaviček a řádků prodejní objednávky z aplikace Finance and Operations do Sales](../supply-chain/sales-marketing/sales-order-template-mapping.md)
###### [Synchronizace hlaviček a řádků prodejní faktury z aplikace Finance and Operations do Sales](../supply-chain/sales-marketing/sales-invoice-template-mapping.md)

## [Správa financí](../financials/index.md)

### [Hlavní kniha](../financials/general-ledger/general-ledger.md)
#### Účty
##### [Přehled časově rozlišených částek](../financials/general-ledger/accruals-overview.md)
##### [Pravidla eliminace](../financials/general-ledger/elimination-rules.md)
##### [Kombinace dimenzí a účtů](../financials/general-ledger/enter-account-dimension-combinations-segmented-entry-control.md)
##### [Vyrovnané deníky pro mezijednotkové účetnictví](../financials/general-ledger/example-balanced-journals-interunit-accounting.md)
##### [Definice účtování](../financials/general-ledger/posting-definitions.md)
###### [Příklady definice účtování ](../financials/general-ledger/example-posting-definitions.md)
##### [Účetní zůstatky hlavní knihy](../financials/general-ledger/general-ledger-account-balances.md)
##### [Typy hlavního účtu](../financials/general-ledger/main-account-types.md)
##### [Plánování účetní osnovy](../financials/general-ledger/plan-chart-of-accounts.md)
##### [Průzkumník zdroje účetnictví](../financials/accounts-payable/accounting-source-explorer.md)
##### [Rozúčtování](../financials/accounts-payable/accounting-distributions.md)
##### [Zpracování přidělení](../financials/general-ledger/process-allocations.md)
##### [Finanční dimenze](../financials/general-ledger/financial-dimensions.md)
##### [Převod měn účetnictví nebo vykazování](../financials/general-ledger/convert-accounting-reporting-currencies.md)
##### [Konsolidace skupin účtů a účtů](../financials/budgeting/consolidation-account-groups-consolidation-accounts.md)
##### [Přehled konsolidace a vyloučení](../financials/budgeting/consolidation-elimination-overview.md)
##### [Fiskální kalendáře, fiskální roky a období](../financials/budgeting/fiscal-calendars-fiscal-years-periods.md)
#### Deníky a hlavní knihy
##### [Zpracování hlavního deníku](../financials/general-ledger/general-journal-processing.md)
##### [Nastavení mezipodnikového účetnictví](../financials/general-ledger/intercompany-accounting-setup.md)
##### [Pravidla přidělení hlavní knihy](../financials/general-ledger/ledger-allocation-rules.md)
##### [Typy deníku hlavní knihy](../financials/general-ledger/ledger-journal-types.md)
##### [Publikování řádek deníku a dokumentů z Excelu](../financials/general-ledger/open-lines-excel-journals-documents.md)
##### [Zobrazení položek a transakcí](../financials/general-ledger/view-journal-entries-transactions.md)
##### [Přecenění měny v konsolidační společnosti](../financials/general-ledger/currency-revaluation-consolidation-company.md)
##### [Přecenění cizí měny pro hlavní knihu](../financials/general-ledger/foreign-currency-revaluation-general-ledger.md)
##### [Import směnných kurzů měn](../financials/general-ledger/import-currency-exchange-rates.md)
#### Závěrka období
##### [Zavření hlavní knihy na konci období](../financials/general-ledger/close-general-ledger-at-period-end.md)
##### [Pracovní prostor uzavření finančního období](../financials/general-ledger/financial-period-close-workspace.md)
##### [Roční uzávěrka](../financials/general-ledger/year-end-close.md)
#### DPH
##### [Přehled DPH](../financials/general-ledger/indirect-taxes-overview.md)
##### [Sazby daně z prodeje na základě základu marže a metod výpočtu](../financials/general-ledger/marginal-base-field.md)
##### [Platby DPH a pravidla zaokrouhlení](../financials/general-ledger/round-sales-tax-payments.md)
##### [Metody výpočtu daně z prodeje v poli Zdroj](../financials/general-ledger/sales-tax-calculation-methods-origin-field.md)
##### [Možnosti výpočtu celé částky a intervalu pro kódy DPH](../financials/general-ledger/whole-amount-interval-options-sales-tax-codes.md)

#### Úkoly hlavní knihy
##### [Auditování faktur a klíčových dat pro závazky](../financials/general-ledger/tasks/audit-invoices-key-data-ap-system.md)
##### [Výpočet a úprava DPH na faktuře dodavatele](../financials/general-ledger/tasks/calculate-adjust-sales-tax-vendor-invoice.md)
##### [Uzavření fiskálního roku](../financials/general-ledger/tasks/close-fiscal-year.md)
##### [Vytvoření účetních struktur](../financials/general-ledger/tasks/create-account-structures.md)
##### [Vytvoření schémat časového rozlišení](../financials/general-ledger/tasks/create-accrual-schemes.md)
##### [Vytvoření rozšířených pravidel pro deníky](../financials/general-ledger/tasks/create-advanced-rules-journals.md)
##### [Vytvoření a přiřazení struktur rozšířeného pravidla](../financials/general-ledger/tasks/create-assign-advanced-rule-structures.md)
##### [Vytvoření konsolidačních skupin a dalších konsolidačních účtů](../financials/general-ledger/tasks/create-consolidation-groups.md)
##### [Vytvoření položky deníku pomocí šablony](../financials/general-ledger/tasks/create-journal-entry-template.md)
##### [Vytvoření transakcí časového rozlišení hlavní knihy](../financials/general-ledger/tasks/create-ledger-accrual-transactions.md)
##### [Vytvoření hlavního účtu](../financials/general-ledger/tasks/create-main-account.md)
##### [Vytvoření platby DPH](../financials/general-ledger/tasks/create-sales-tax-payment.md)
##### [Vytváření transakcí DPH v dokladech](../financials/general-ledger/tasks/create-sales-tax-transactions-documents.md)
##### [Vytváření a ověřování deníků](../financials/general-ledger/tasks/create-validate-journals.md)
##### [Definování zásad auditu pro zdrojové dokumenty](../financials/general-ledger/tasks/define-audit-policies-source-documents.md)
##### [Definování finančních dimenzí](../financials/general-ledger/tasks/define-financial-dimensions.md)
##### [Stanovení platebních podmínek odběratele](../financials/general-ledger/tasks/establish-customer-payment-terms.md)
##### [Zapsání zaúčtovaných položek deníku do deníku](../financials/general-ledger/tasks/journalize-posted-journal-entries.md)
##### [Udržování výchozích šablon finančních dimenzí](../financials/general-ledger/tasks/maintain-financial-dimension-default-templates.md)
##### [Hromadné uzavření finančního období](../financials/general-ledger/tasks/mass-financial-period-close.md)
##### [Účtování periodických deníků](../financials/general-ledger/tasks/post-periodic-journals.md)
##### [Zpracování deníku přidělení hlavní knihy](../financials/general-ledger/tasks/process-ledger-allocation-journal.md)
##### [Vyrovnání transakcí mezi účty hlavní knihy](../financials/general-ledger/tasks/settle-transactions-between-ledger-accounts.md)
##### [Nastavení aliasu účtu hlavní knihy](../financials/general-ledger/tasks/set-up-ledger-account-alias.md)
##### [Nastavení účetních skupin pro DPH](../financials/general-ledger/tasks/set-up-ledger-posting-groups-sales-tax.md)
##### [Nastavení kategorií hlavního účtu](../financials/general-ledger/tasks/set-up-main-account-categories.md)
##### [Nastavení finančních úřadů](../financials/general-ledger/tasks/set-up-sales-tax-authorities.md)
##### [Nastavení kódů DPH](../financials/general-ledger/tasks/set-up-sales-tax-codes.md)
##### [Nastavení skupin DPH a skupin DPH položky](../financials/general-ledger/tasks/set-up-sales-tax-groups-item-sales-tax-groups.md)
##### [Nastavení kódů vykazování DPH](../financials/general-ledger/tasks/set-up-sales-tax-reporting-codes.md)
##### [Nastavení období vyrovnání DPH](../financials/general-ledger/tasks/set-up-sales-tax-settlement-periods.md)
##### [Nastavení srážkové daně](../financials/general-ledger/tasks/set-up-withholding-tax.md)
##### [Zobrazení položek nebo transakcí deníku](../financials/general-ledger/tasks/view-journal-entries-or-transactions.md)
##### [Zobrazení zaúčtovaných transakcí DPH](../financials/general-ledger/tasks/view-posted-sales-tax-transactions.md)
##### [Zobrazení souvisejícího mezipodnikového dokladu z deníku](../financials/general-ledger/tasks/view-related-intercompany-voucher-journal.md)

### [Závazky](../financials/accounts-payable/accounts-payable.md)
#### [Přehled faktur dodavatele](../financials/accounts-payable/vendor-invoices-overview.md)
#### [Konfigurace modulu Závazky](../financials/accounts-payable/accounts-payable-overview.md)
#### [Zásady třícestného párování](../financials/accounts-payable/three-way-matching-policies.md)
#### [Porovnání faktur závazků](../financials/accounts-payable/accounts-payable-invoice-matching.md)
#### [Účetní distribuce a účetní položky dílčí hlavní knihy pro faktury dodavatele](../financials/accounts-payable/accounting-distributions-subledger-journal-entries-vendor-invoices.md)
#### [Párování faktur a mezipodnikové nákupní objednávky](../financials/accounts-payable/invoice-matching-intercompany-purchase-orders.md)
#### [Přehled platné platby](../financials/accounts-payable/positive-pay-overview.md)
#### [Nastavení a generování souborů kladné platby](../financials/accounts-payable/set-up-generate-positive-pay-files.md)
#### [Výchozí protiúčty pro deníky faktur dodavatele a deníky schvalování faktur](../financials/accounts-payable/default-offset-accounts-vendor-invoice-journals.md)
#### [Řešení nesrovnalostí během párování součtů faktur](../financials/accounts-payable/resolve-invoice-totals-invoice-matching-discrepancies.md)
#### [Účetní profily dodavatele](../financials/accounts-payable/vendor-posting-profiles.md)
#### [Přecenění cizí měny pro závazky a pohledávky](../financials/cash-bank-management/foreign-currency-revaluation-accounts-payable-accounts-receivable.md)
#### [Mobilní schvalování faktur](../financials/accounts-payable/mobile-invoice-approvals.md)
#### [Pracovní prostor fakturace dodavatelské spolupráce](../financials/accounts-payable/vendor-portal-invoicing-workspace.md)
#### [Automatizace faktur dodavatele](../financials/accounts-payable/vendor-invoice-automation.md) 
#### Platby a vyrovnání
##### [Centralizované platby pro závazky](../financials/accounts-payable/centralized-payments-accounts-payable.md)
##### [Konfigurace vyrovnání](../financials/cash-bank-management/configure-settlement.md)
##### [Pracovní prostor plateb dodavatelů](../financials/accounts-payable/vendor-payments-workspace.md)
##### [Vytvoření plateb dodavatelů pomocí návrhu platby](../financials/accounts-payable/create-vendor-payments-payment-proposal.md)
##### [Stornování platby dodavatele](../financials/accounts-payable/reverse-vendor-payment.md)
##### [Elektronické výkaznictví pro šeky dodavatele](../financials/accounts-payable/electronic-reporting-sample-vendor-checks.md)
##### [Přehled zálohových faktur a záloh](../financials/accounts-payable/prepayments-invoices-vs-prepayments.md)
##### [Vyrovnání částečné platby dodavatele před datem slevy](../financials/accounts-payable/settle-partial-vendor-payment-before-discount-or-final-payment-after.md)
##### [Vyrovnání částečné platby dodavatele, u níž jsou slevy pro dobropisy dodavatele](../financials/accounts-payable/settle-partial-vendor-payment-discounts-vendor-credit-notes.md)
##### [Vyrovnání částečné platby dodavatele, u níž je více období slev](../financials/accounts-payable/settle-partial-vendor-payment-multiple-discount-periods.md)
##### [Vyrovnání částečné platby dodavatele nebo konečné platby před slevou](../financials/accounts-payable/settle-partial-vendor-payment-or-final-payment-before-discount.md)
##### [Jeden doklad se záznamy několika odběratelů nebo dodavatelů](../financials/accounts-payable/single-voucher-multiple-customer-vendor-records.md)
##### [Provedení platební slevy mimo období platební slevy](../financials/accounts-payable/take-cash-discount-outside-cash-discount-timeframe.md)
##### [Použití slevy, která je větší, než vypočítaná sleva pro platbu dodavatele](../financials/accounts-payable/take-discount-more-calculated-discount-vendor-payment.md)
##### [Platby částečných částek dodavatelem](../financials/accounts-payable/vendor-payments-partial-amount.md)

#### Úkoly závazků
##### [Definování platebních poplatků dodavatelů](../financials/accounts-payable/tasks/define-vendor-payment-fees.md)
##### [Definování platebních podmínek dodavatelů](../financials/accounts-payable/tasks/define-vendor-payment-terms.md)
##### [Zadání dat faktury do závazků s použitím faktury dodavatele](../financials/accounts-payable/tasks/key-invoice-data-ap-system-vendor-invoice.md)
##### [Zadání dat faktury do závazků s použitím deníku schválení](../financials/accounts-payable/tasks/key-invoice-data-into-ap-system-approval-journal.md)
##### [Zadání dat faktury do systému závazků s použitím evidence faktur](../financials/accounts-payable/tasks/key-invoice-data-into-ap-system-invoice-pool.md)
##### [Zaznamenání faktury dodavatele do deníku faktur](../financials/accounts-payable/tasks/record-vendor-invoice-invoice-journal.md)
##### [Prozkoumání nebo vyřešení výjimek](../financials/accounts-payable/tasks/research-resolve-exceptions.md)
##### [Nastavení ověření párování faktur závazků](../financials/accounts-payable/tasks/set-up-accounts-payable-invoice-matching-validation.md)

### [Pohledávky](../financials/accounts-receivable/accounts-receivable.md)
#### [Úvěr a inkasa v modulu Pohledávky](../financials/accounts-receivable/collections-credit-accounts-receivable.md)
#### [Konfigurace modulu Pohledávky a kredit a inkasa](../financials/accounts-receivable/accounts-receivables-set-up-overview.md)
#### [Nastavení kreditu a inkas](../financials/accounts-receivable/set-up-collections.md)
#### [Nastavení a zpracování opakovaných faktur](../financials/accounts-receivable/set-up-process-recurring-invoices.md)
#### [Rozúčtování a položky dílčích hlavních knih deníku pro textové faktury](../financials/accounts-receivable/accounting-distributions-subledger-journal-entries-free-text-invoices.md)
#### [Účetní profily odběratele](../financials/accounts-receivable/customer-posting-profiles.md)
#### [Přehled přímého inkasa SEPA](../financials/accounts-receivable/sepa-direct-debit-overview.md)
#### [Souhlas s inkasem SEPA](../financials/accounts-receivable/sepa-direct-debit-mandate.md)
#### [Nastavení platební karty, autorizace a záznam](../financials/accounts-receivable/credit-card-authorizations.md)
#### [Oprava volné faktury](../financials/accounts-receivable/correct-free-text-invoice.md)
#### [Nastavení úrokových sazeb pro kód úroku](../financials/accounts-receivable/set-up-interest-rates-interest-code.md)
#### [Vzdát, obnovit nebo změnit úrokové poplatky](../financials/accounts-receivable/waive-reinstate-reverse-interest-fees.md)
#### [Vytvoření faktury odběratele](../financials/accounts-receivable/configure-customer-invoices.md)
#### [Nastavení cizích směnek](../financials/accounts-receivable/set-up-bills-exchange.md)
#### [Uzavření modulu Pohledávky](../financials/accounts-receivable/close-accounts-receivable.md)

#### Úkoly pohledávek
##### [Přiřazení šablony volné faktury odběrateli](../financials/accounts-receivable/tasks/assign-free-text-invoice-template-customer.md)
##### [Vytvoření posloupnosti upomínek](../financials/accounts-receivable/tasks/create-collection-letter-sequence.md)
##### [Vytvoření zmocnění k přímému debetu pro odběratele](../financials/accounts-receivable/tasks/create-direct-debit-mandate-customer.md)
##### [Vytvoření volné faktury](../financials/accounts-receivable/tasks/create-free-text-invoice.md)
##### [Vytvoření šablony volné faktury](../financials/accounts-receivable/tasks/create-free-text-invoice-template.md)
##### [Vytvoření kódu úroků s rozsahem](../financials/accounts-receivable/tasks/create-interest-code-range.md)
##### [Vytváření faktur prodejních objednávek](../financials/accounts-receivable/tasks/create-sales-order-invoices.md)
##### [Vytvoření odpisového deníku pro odběratele](../financials/accounts-receivable/tasks/create-write-off-journal-customer.md)
##### [Vklad platby odběratele](../financials/accounts-receivable/tasks/deposit-customer-payments.md)
##### [Stanovení způsobu platby odběratele](../financials/accounts-receivable/tasks/establish-customer-method-payment.md)
##### [Stanovení platebních poplatků odběratele](../financials/accounts-receivable/tasks/establish-customer-payment-fees.md)
##### [Generování a zaúčtování opakovaných volných faktur](../financials/accounts-receivable/tasks/post-recurring-free-text-invoices.md)
##### [Zpracování upomínek](../financials/accounts-receivable/tasks/process-collection-letters.md)
##### [Zpracování úroků](../financials/accounts-receivable/tasks/process-interest.md)
##### [Zaznamenání faktury dodavatele a spárování s přijatým množstvím](../financials/accounts-receivable/tasks/record-vendor-invoice-match-against-received-quantity.md)
##### [Kontrola inkasních informací](../financials/accounts-receivable/tasks/review-collections-information.md)
##### [Nastavení a generování informací o splatnosti pohledávek](../financials/accounts-receivable/tasks/set-up-accounts-receivable-aging-information.md)
##### [Nastavení zásad faktur dodavatele](../financials/accounts-receivable/tasks/set-up-vendor-invoice-policies.md)

#### Platby a vyrovnání
##### [Použití jedné platby odběratele na úhradu několika faktur, které zasahují do více období slevy](../financials/accounts-receivable/customer-payment-settle-multiple-invoices-multiple-discount-periods.md)
##### [Centralizované platby pro pohledávky](../financials/accounts-receivable/centralized-payments-accounts-receivable.md)
##### [Vyrovnání částečné platby zákazníka nebo konečné platby před slevou](../financials/accounts-payable/settle-partial-customer-payment-or-final-payment-before-discount.md) 
##### [Vyrovnání částečné platby odběratele před datem slevy s konečnou platbou po datu slevy](../financials/accounts-receivable/settle-partial-customer-payment-before-discount-or-final-payment-after.md)
##### [Vyrovnání částečné platby zákazníka, u níž jsou slevy pro dobropisy dodavatele](../financials/accounts-receivable/settle-partial-customer-payment-discounts-credit-notes.md)
##### [Vyrovnání částečné platby zákazníka, u níž je více období slev](../financials/accounts-receivable/settle-partial-customer-payment-multiple-discount-periods.md)
##### [Refundace odběratelům](../financials/accounts-receivable/reimburse-customers.md)
##### [Platby částečných částek zákazníkům](../financials/accounts-receivable/customer-payments-partial-amount.md)

### [Pokladna a banka](../financials/cash-bank-management/cash-bank-management.md)
#### [Prognóza cashflow](../financials/cash-bank-management/cash-flow-forecasting.md)
#### [Pracovní prostor správy banky](../financials/cash-bank-management/Bank-management-workspace.md)
#### [Platební slevy](../financials/cash-bank-management/cash-discounts.md)
#### [Hotovostní slevy u přeplatků](../financials/cash-bank-management/cash-discount-handling-overpayments.md)
#### [Přehled vyrovnání](../financials/cash-bank-management/settlement-overview-centralized-payments.md)
#### [Mezipodnikové účetnictví pro centralizované platby](../financials/cash-bank-management/settlement-overview-centralized-payments.md)
#### [Akreditivy](../financials/cash-bank-management/letters-of-credit.md)
#### [Kolekce akreditivu a importu](../financials/cash-bank-management/letters-of-credit-import-collections.md)
#### [Záruční listiny](../financials/cash-bank-management/letters-of-guarantee.md)
#### [Postdatované šeky](../financials/cash-bank-management/postdated-checks.md)
#### [Zálohové faktury a zálohy](../financials/accounts-payable/prepayments-invoices-vs-prepayments.md)
#### [Práce s hotovostními slevami u přeplatků](../financials/cash-bank-management/cash-discount-handling-overpayments.md)
#### [Nastavení centralizovaných plateb](../financials/cash-bank-management/set-up-centralized-payments.md)
#### [Automatické vyrovnání a stanovení priorit](../financials/accounts-receivable/automatic-settlement-prioritization.md)

#### Úkoly modulu Pokladna a banka
##### [Vytvoření smlouvy bankovního zařízení pro akreditiv](../financials/cash-bank-management/tasks/create-bank-facility-agreement-letter-credit.md)
##### [Vytvoření smlouvy bankovního zařízení pro záruční listinu](../financials/cash-bank-management/tasks/create-bank-facility-agreement-letter-guarantee.md)
##### [Přehled plateb odběratele](../financials/cash-bank-management/tasks/customer-payment-overview.md)
##### [Export akreditivu](../financials/cash-bank-management/tasks/export-letter-credit.md)
##### [Import akreditivu](../financials/cash-bank-management/tasks/import-letter-credit.md)
##### [Transakce záruční listiny](../financials/cash-bank-management/tasks/letter-guarantee-transaction.md)
##### [Registrace a zaúčtování postdatovaného šeku pro odběratele](../financials/cash-bank-management/tasks/register-post-postdated-check-customer.md)
##### [Registrace a zaúčtování postdatovaného šeku pro dodavatele](../financials/cash-bank-management/tasks/register-post-postdated-check-vendor.md)
##### [Vyrovnání postdatovaného šeku od odběratele](../financials/cash-bank-management/tasks/settle-postdated-check-customer.md)
##### [Vyrovnání postdatovaného šeku pro dodavatele](../financials/cash-bank-management/tasks/settle-postdated-check-vendor.md)
##### [Nastavení zařízení banky a zaúčtování profilů pro záruční listiny](../financials/cash-bank-management/tasks/set-up-bank-facilities-posting-profiles.md)
##### [Nastavení zařízení banky a zaúčtování profilů pro akreditiv](../financials/cash-bank-management/tasks/set-up-bank-facilities-posting-profiles-letter-credit.md)
##### [Nastavení postdatovaných šeků](../financials/cash-bank-management/tasks/set-up-postdated-checks.md)
##### [Přehled plateb dodavatele](../financials/cash-bank-management/tasks/vendor-payment-overview.md)

#### Rozšířené odsouhlasení banky
##### [Úlohy nastavení rozšířeného odsouhlasení banky](../financials/cash-bank-management/configure-advanced-bank-reconciliation.md)
##### [Nastavení procesu importu rozšířené bankovního odsouhlasení](../financials/cash-bank-management/set-up-advanced-bank-reconciliation-import-process.md)
##### [Nastavení pravidel párování pro odsouhlasení banky](../financials/cash-bank-management/set-up-bank-reconciliation-matching-rules.md)
##### [Odsouhlasení bankovního výpisu pomocí rozšířeného odsouhlasení banky](../financials/cash-bank-management/reconcile-bank-statements-advanced-bank-reconciliation.md)
##### [Rozšířené odsouhlasení bankovního výpisu MT940 Import – kroky aktualizace entity složených dat](../financials/cash-bank-management/advanced-bank-reconciliation-mt940-data-entity-upgrade-steps.md)
##### [Přehled rozšířeného odsouhlasení banky](../financials/cash-bank-management/advanced-bank-reconciliation-overview.md)
##### [Odstraňování problémů s importem souboru bankovních výpisů](../financials/cash-bank-management/import-bank-statement-file-failed-incorrect-results.md)
##### [Aktualizace složité entity bankovního deníku](../financials/cash-bank-management/upgrade-bank-journal-composite-entity.md)

### [Dlouhodobý majetek](../financials/fixed-assets/fixed-assets.md)
#### [Pracovní prostor dlouhodobého majetku](../financials/fixed-assets/fixed-asset-management-workspace.md)
#### [Přehled upgradu knihy odpisů](../financials/fixed-assets/depreciation-book-upgrade-considerations.md)
#### [Odvozené knihy](../financials/fixed-assets/derived-books.md)
#### [Oceňovací model dlouhodobého majetku a slučování knih odpisů](../financials/fixed-assets/fixed-asset-value-model-depreciation-book-merge.md)
#### [Účtování s odvozenými knihami](../financials/fixed-assets/post-derived-value-models.md)
#### [Zaúčtování transakcí dlouhodobého majetku do účtovacích vrstev](../financials/fixed-assets/post-fixed-asset-transactions-posting-layers.md)
#### [Možnosti transakce dlouhodobého majetku](../financials/fixed-assets/enter-fixed-asset-transactions.md)
#### [Přepočet reprodukčních nákladů a pojistných částek pro skupiny dlouhodobého majetku](../financials/fixed-assets/recalculate-replacement-costs-insured-values-fixed-asset-groups.md)
#### [Účty zaúčtování dlouhodobého majetku](../financials/fixed-assets/fixed-asset-disposal-posting-accounts.md)
#### [Hromadná aktualizace dlouhodobého majetku](../financials/fixed-assets/fixed-asset-mass-update.md)
#### [Pořízení majetku pomocí zásobování](../financials/fixed-assets/acquire-assets-procurement.md)
#### Nastavení a integrace
##### [Nastavení dlouhodobého majetku](../financials/fixed-assets/set-up-fixed-assets.md)
##### [Integrace dlouhodobého majetku](../financials/fixed-assets/fixed-asset-integration.md)
#### Metody odpisování
##### [Degresivní odpis 125%](../financials/fixed-assets/125-percent-reducing-balance-depreciation.md)
##### [Degresivní odpis 150%](../financials/fixed-assets/150-percent-reducing-balance-depreciation.md)
##### [Degresivní odpis 175%](../financials/fixed-assets/175-percent-reducing-balance-depreciation.md)
##### [Degresivní odpis 200%](../financials/fixed-assets/200-percent-reducing-balance-depreciation.md)
##### [Počáteční mimořádný odpis](../financials/fixed-assets/bonus-depreciation.md)
##### [Odpis spotřeby](../financials/fixed-assets/consumption-depreciation.md)
##### [Efekty odpisů se storny](../financials/fixed-assets/depreciation-effects-reversals.md)
##### [Odpisové metody a způsoby odpisu](../financials/fixed-assets/depreciation-methods-conventions.md)
##### [Odpis koeficientem](../financials/fixed-assets/factor-depreciation.md)
##### [Účty zaúčtování pořízení dlouhodobého majetku](../financials/fixed-assets/fixed-asset-acquisition-posting-accounts.md)
##### [Odpisy dlouhodobého majetku](../financials/fixed-assets/fixed-asset-depreciation.md)
##### [Ruční odpis](../financials/fixed-assets/manual-depreciation.md)
##### [Degresivní odpis](../financials/fixed-assets/reduce-balance-depreciation.md)
##### [Částka pro zaokrouhlení výpočtů odpisů](../financials/fixed-assets/round-off-amount-depreciation-calculations.md)
##### [Lineární odpis s vyrovnáním na konci životnosti](../financials/fixed-assets/straight-line-life-remaining-depreciation.md)
##### [Lineární odpisování](../financials/fixed-assets/straight-line-service-life-depreciation.md)

#### Úkoly dlouhodobého majetku
##### [Výpočet odpisů dlouhodobého majetku mezi právnickými osobami](../financials/fixed-assets/tasks/calculate-fixed-asset-depreciation-legal-entities.md)
##### [Změna způsobu odpisování pro více položek dlouhodobého majetku](../financials/fixed-assets/tasks/change-depreciation-conventions-multiple-fixed-assets.md)
##### [Změna skupiny dlouhodobého majetku](../financials/fixed-assets/tasks/change-fixed-asset-group.md)
##### [Vytvoření a získání majetku z modulu Závazky](../financials/fixed-assets/tasks/create-acquire-assets-accounts-payable.md)
##### [Vytvoření návrhu odpisování](../financials/fixed-assets/tasks/create-depreciation-proposal.md)
##### [Vytvoření dlouhodobého majetku](../financials/fixed-assets/tasks/create-fixed-asset.md)
##### [Vyřazení dlouhodobého majetku s použitím volné faktury](../financials/fixed-assets/tasks/dispose-fixed-asset-free-text-invoice.md)
##### [Zadání příslušenství dlouhodobého majetku](../financials/fixed-assets/tasks/enter-addition-fixed-asset.md)
##### [Nabídka pořízení dlouhodobého majetku](../financials/fixed-assets/tasks/propose-fixed-asset-acquisitions.md)
##### [Přeřazení dlouhodobého majetku](../financials/fixed-assets/tasks/reclassify-fixed-assets.md)
##### [Nastavení počátečních mimořádných odpisů](../financials/fixed-assets/tasks/set-up-bonus-depreciation.md)
##### [Nastavení knih odpisů ](../financials/fixed-assets/tasks/set-up-depreciation-books-2016-05.md)
##### [Nastavení a vytvoření odpisových plánů](../financials/fixed-assets/tasks/set-up-depreciation-profiles.md)
##### [Nastavení skupin dlouhodobého majetku](../financials/fixed-assets/tasks/set-up-fixed-asset-groups.md)
##### [Nastavení účetních profilů dlouhodobého majetku](../financials/fixed-assets/tasks/set-up-fixed-asset-posting-profiles.md)
##### [Nastavení oceňovacích modelů](../financials/fixed-assets/tasks/set-up-value-models.md)
##### [Rozdělení dlouhodobého majetku](../financials/fixed-assets/tasks/split-fixed-asset.md)
##### [Převedení dlouhodobého majetku](../financials/fixed-assets/tasks/transfer-fixed-asset.md)

### [Správa výdajů](../financials/expense-management/expense-management.md)
#### [Plánování správy výdajů](../financials/expense-management/plan-expense-management.md)
#### [Parametry správy výdajů](../financials/expense-management/Expense-Management-Parameters.md)
#### [Pracovnímu prostor správy výdajů](../financials/expense-management/expense-management-mobile-workspace.md)
#### [Nastavit workflowy pro správu výdajů](../financials/expense-management/expense-workflows.md)
#### [Workflow správy výdajů](../financials/expense-management/expense-workflow.md)
#### [Import a udržování transakce platební karty](../financials/expense-management/import-credit-card.md)
#### [Distribuce na sestavě výdajů](../financials/expense-management/expense-report-distributions.md)
#### [Mezipodnikové výdaje](../financials/expense-management/intercompany-expense.md)
#### [Zásady výdajů](../financials/expense-management/travel-expense-policies.md)


### [Rozpočtování](../financials/budgeting/budgeting-overview.md)
#### [Přehled plánování rozpočtu ](../financials/budgeting/budget-planning-overview-configuration.md)
#### [Přehled řízení ](../financials/budgeting/budget-control-overview-configuration.md)
#### [Přehled rozpočtování ](../financials/budgeting/basic-budgeting-overview-configuration.md)
#### [Prognóza pozice](../financials/budgeting/position-forecasting.md)
#### [Dokumenty odůvodnění plánování rozpočtu](../financials/budgeting/budget-planning-justification-docs.md)
#### [Vytváření rozpočtů z účtů transakcí a souhrnných účtů](../financials/budgeting/create-budget-transaction-accounts-total-accounts.md)
#### [Šablony plánování rozpočtu pro Excel](../financials/budgeting/budget-planning-excel-templates.md)
#### [Odstraňování problémů s rozpočtováním pozice](../financials/budgeting/position-budgeting-set-up-issues.md)
#### [Plánování rozpočtu](../financials/budgeting/budget-plan.md)
#### [Přidělení dat pro plánování rozpočtu](../financials/budgeting/budget-planning-data-allocation.md)
#### [Integrace plánování rozpočtu s jinými moduly](../financials/budgeting/budget-planning-integration-other-modules.md)
### [Vykazování](../financials/general-ledger/financial-reporting-getting-started.md)
#### [Finanční sestavy rozvah](../financials/general-ledger/balance-sheet-financial-reports.md)
#### [Generování finanční sestavy](../dev-itpro/analytics/generate-financial-report.md)
#### [Finanční sestava výpisu příjmů](../financials/general-ledger/income-statement-financial-report.md)
#### [Finanční sestavy rozvahy](../financials/general-ledger/trial-balance-financial-reports.md)
#### [Zobrazit finanční sestavy](../financials/general-ledger/view-financial-reports.md)
#### [Návrhář sestav](../dev-itpro/analytics/report-designer-interface.md)

### [Nákladové účetnictví](../financials/cost-accounting/cost-accounting-home-page.md)
#### [Začínáme](../financials/cost-accounting/get-started.md)
##### [Terminologie týkající se účtování nákladů](../financials/cost-accounting/terms-cost-accounting.md)
#### [Konfigurovat základních entit](../financials/cost-accounting/configure-base-entities.md)
##### [Měna (kurz elektronického vzdělávání)](https://mbspartner.microsoft.com/AX/CourseModules/1215.md) 
##### [Směnné kurzy (kurz elektronického vzdělávání)](https://mbspartner.microsoft.com/AX/CourseModules/1215.md) 
##### [Fiskální kalendáře, fiskální roky a období](../financials/budgeting/fiscal-calendars-fiscal-years-periods.md)
##### [Finanční dimenze](../financials/general-ledger/financial-dimensions.md)
##### [Účetní struktury](../financials/general-ledger/Default-dimensions.md)
##### [Vytvoření účetních struktur (průvodce záznamem úloh)](../financials/general-ledger/tasks/create-account-structures.md)
##### [Přehled rozpočtování](../financials/budgeting/basic-budgeting-overview-configuration.md)
#### [Vytvoření dimenzí a import členů dimenze](../financials/cost-accounting/cost-dimension.md) 
##### [Dimenze prvku nákladů](../financials/cost-accounting/cost-elements.md)
##### [Vytváření prvků nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-cost-elements.md)
##### [Dimenze objektu nákladů](../financials/cost-accounting/cost-objects.md)
##### [Vytváření objektů nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-cost-objects.md)
##### [Mapování členů dimenze prvků nákladů na společnou sadu členů dimenze](../financials/cost-accounting/map-cost-elements-dimension-members.md)
##### [Namapování dimenze prvku nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/map-cost-element-dimension.md)
##### [Členy statistické dimenze a šablony poskytovatelů statistických měření](../financials/cost-accounting/statistical-measure-provider-template.md)
#### Vytvoření hierarchie dimenzí
##### [Hierarchie dimenzí](../financials/cost-accounting/dimension-hierarchy.md)
##### [Vytvoření hierarchie sestav organizace (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-an-organization-report-hierarchy.md) 
##### [Přístupová práva kontrolora objektu nákladů](../financials/cost-accounting/access-rights-cost-object-controller.md)
##### [Konfigurace přístupových práv pro kontrolora objektu nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/configure-access-rights-cost-object-controller.md) 
##### [Mobilní pracovní prostor kontroly nákladů (video)](https://youtu.be/imsuTg8rUVk.md)
#### Vytvoření základního nastavení
##### [Definování jednotek řízení nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/define-cost-control-units.md)
##### [Vytvoření hlavní knihy nákladového účetnictví (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-cost-accounting-ledger.md)
##### [Členy statistické dimenze a šablony poskytovatelů statistických měření](../financials/cost-accounting/statistical-measure-provider-template.md)
#### [Definování základu přidělení](../financials/cost-accounting/allocation-bases.md)
#### Definování zásad
##### [Vytvoření zásad přidělování nákladů a jejich přiřazení jednotce řízení nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-assign-cost-allocation-policy-cost-control-unit.md)
##### [Vytvoření a přiřazení zásad chování nákladů k jednotce řízení nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-assign-cost-behavior-policy-cost-control-unit.md)
##### [Vytvoření a přiřazení zásad distribuce nákladů k jednotce řízení nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-assign-cost-distribution-policy-cost-control-unit.md)
##### [Vytvoření zásad shrnutí nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/create-cost-rollup-policy.md)
#### [Správa datového zdroje pro hlavní knihu nákladového účetnictví (průvodce záznamem úloh)](../financials/cost-accounting/tasks/manage-data-source-cost-accounting-ledger.md)
#### [Zpracování a sledování zdrojových dat (průvodce záznamem úloh)](../financials/cost-accounting/tasks/process-trace-source-data.md)
#### Výpočet režijních nákladů
##### [Výpočet režijních nákladů](../financials/cost-accounting/overhead-calculation.md)
##### [Zásady shrnutí nákladů a výpočet režijních nákladů](../financials/cost-accounting/cost-rollup.md)
#### Analýzy a výkaznictví
##### [Konfigurace pracovního prostoru řízení nákladů](../financials/cost-accounting/cost-control-workspace.md)
##### [Konfigurace parametrů pracovního prostoru řízení nákladů (průvodce záznamem úloh)](../financials/cost-accounting/tasks/configure-cost-control-workspace-parameters.md)
##### [Použití pracovního prostoru řízení nákladů pro výkaznictví (video)](https://youtu.be/imsuTg8rUVk.md)
##### [Export položek do souboru Excel (video)](https://youtu.be/-HKHYdClvx8.md)
##### [Použití balíčku obsahu pro výkaznictví v Microsoft Power BI](../dev-itpro/analytics/cost-accounting-analysis-content-pack.md)
##### [Pochopení zabezpečení přístupového seznamu a RLS v Microsoft Power BI](../dev-itpro/analytics/setup-security-cost-accounting-content-pack.md)
##### [Použití pracovního prostoru řízení nákladů pro mobilní aplikaci Finance and Operations](../financials/cost-accounting/cost-controlling-mobile-workspace.md)

### [Veřejný sektor](../financials/public-sector/public-sector-functionality.md)
#### [Plán pro funkci veřejného sektoru](../financials/public-sector/plan-public-sector-functionality.md)
#### [Závazky ve veřejném sektoru](../financials/public-sector/accounts-payable-public-sector.md)
##### [Kódy nákupní objednávky ve veřejném sektoru](../financials/public-sector/purchase-order-codes-public-sector.md)
##### [Definice účtování ve veřejném sektoru](../financials/public-sector/posting-definitions-public-sector.md)
##### [Plán pro jednorázové dodavatele ve veřejném sektoru](../financials/public-sector/plan-one-time-vendors-public-sector.md)
##### [Jednorázoví dodavatelé ve veřejném sektoru](../financials/public-sector/one-time-vendors-public-sector.md)
#### [Pohledávky ve veřejném sektoru](../financials/public-sector/accounts-receivable-public-sector.md)
##### [Účtovací klasifikace a účtovací kódy ve veřejném sektoru](../financials/public-sector/billing-classifications-billing-codes-public-sector.md)
##### [Priorita vyrovnání ve veřejném sektoru](../financials/public-sector/settlement-priority-public-sector.md)
##### [Volné faktury ve veřejném sektoru](../financials/public-sector/free-text-invoices-public-sector.md)
##### [Náhrady ve veřejném sektoru](../financials/public-sector/reimbursements-public-sector.md)
#### [Rozpočtování ve veřejném sektoru](../financials/public-sector/budgeting-public-sector.md)
##### [Analýza rozpočtování ve veřejném sektoru](../financials/public-sector/budget-analysis-public-sector.md)
##### [Předběžné rozpočty a rozdělení ve veřejném sektoru](../financials/public-sector/preliminary-budgets-apportionments-public-sector.md)
#### [Dlouhodobý majetek ve veřejném sektoru](../financials/public-sector/fixed-asset-public-sector.md)
#### [Hlavní kniha ve veřejném sektoru](../financials/public-sector/general-ledger-public-sector.md)
##### [Pokročilé záznamy hlavní knihy ve veřejném sektoru](../financials/public-sector/advanced-ledger-entries-public-sector.md)
##### [Zpracování na konci roku ve veřejném sektoru](../financials/public-sector/year-end-processing-public-sector.md)
##### [Odvozené finanční hierarchie ve veřejném sektoru](../financials/public-sector/derived-financial-hierarchies-public-sector.md)
#### [Zásobování a zdroje ve veřejném sektoru](../financials/public-sector/procurement-sourcing-public-sector.md)
#### [Role zabezpečení ve veřejném sektoru](../financials/public-sector/security-roles-public-sector.md)
#### [Fondy ve veřejném sektoru](../financials/public-sector/funds-public-sector.md)

#### Úkoly veřejného sektoru
##### [Přidání typu certifikátu k dodavateli ve veřejném sektoru](../financials/public-sector/tasks/add-certification-type-vendor-public-sector.md)
##### [Řízení přístupu k nákupním smlouvám ve veřejném sektoru](../financials/public-sector/tasks/control-access-purchase-agreements-public-sector.md)
##### [Vytvoření položky rozšířeného účetnictví ve veřejném sektoru](../financials/public-sector/tasks/create-advanced-ledger-entry-public-sector.md)
##### [Vytvoření a přiřazení kódu obchodního partnera ve veřejném sektoru](../financials/public-sector/tasks/create-assign-trading-partner-code-public-sector.md)
##### [Vytvoření účtovací klasifikace ve veřejném sektoru](../financials/public-sector/tasks/create-billing-classification-public-sector.md)
##### [Vytvoření účtovacího kódu ve veřejném sektoru](../financials/public-sector/tasks/create-billing-code-public-sector.md)
##### [Vytvoření závazku pro rezervaci rozpočtových prostředků pro veřejný sektor](../financials/public-sector/tasks/create-commitment-reserve-budget-funds-public-sector.md)
##### [Vytvoření titulní stránky a podpisové stránky pro sestavu plateb pro veřejný sektor](../financials/public-sector/tasks/create-cover-signature-pages-payments-report-public-sector.md)
##### [Vytvoření typu fondu ve veřejném sektoru](../financials/public-sector/tasks/create-fund-type-public-sector.md)
##### [Vytvoření jednorázového dodavatele a faktury ve veřejném sektoru](../financials/public-sector/tasks/create-one-time-vendor-invoice-public-sector.md)
##### [Vytvoření původního rozpočtu a stornování předběžných položek rozpočtu ve veřejném sektoru](../financials/public-sector/tasks/create-original-budget.md)
##### [Vytvoření předběžného rozpočtu pro veřejný sektor](../financials/public-sector/tasks/create-preliminary-budget-public-sector.md)
##### [Vytvoření typu certifikátu dodavatele ve veřejném sektoru](../financials/public-sector/tasks/create-vendor-certification-type-public-sector.md)
##### [Import a vytvoření více jednorázových dodavatelů a faktur ve veřejném sektoru](../financials/public-sector/tasks/import-multiple-one-time-vendors.md)
##### [Nastavení odvozené finanční hierarchie ve veřejném sektoru](../financials/public-sector/tasks/set-up-derived-financial-hierarchy-public-sector.md)
##### [Nastavení fondu ve veřejném sektoru](../financials/public-sector/tasks/set-up-fund-public-sector.md)
##### [Nastavení klasifikace nákupních smluv ve veřejném sektoru](../financials/public-sector/tasks/set-up-purchase-agreement-classifications-public-sector.md)

## [Finanční výkaznictví](../dev-itpro/analytics/financial-reporting-intro.md)
### [Generování finanční sestavy](../dev-itpro/analytics/generate-financial-report.md)
### [Součásti finančních sestav](../dev-itpro/analytics/financial-report-components.md)
### [Návrhář sestav](../dev-itpro/analytics/report-designer-interface.md)
### [Uživatelské předvolby](../dev-itpro/analytics/user-preferences-financial-report-designer.md)
### [Rozšířené možnosti formátování](../dev-itpro/analytics/advanced-formatting-options-financial-reporting.md)
### [Organizování součástí sestav](../dev-itpro/analytics/organize-components-report-designer.md)
### [Definice sestavy](../dev-itpro/analytics/design-financial-report-definitions.md)
### [Definice řádku](../dev-itpro/analytics/row-definitions-financial-reporting.md)
### [Definice sloupce](../dev-itpro/analytics/column-definitions-financial-reports.md)
### [Úprava buněk definice řádku](../dev-itpro/analytics/modify-row-definition-cells-financial-reporting.md)
### [Obnovení datového tržiště finančního vykazování po obnovení databáze](../dev-itpro/analytics/reset-financial-reporting-datamart-after-restore.md)

## [Lokalizace a funkce týkající se předpisů](../dev-itpro/lcs-solutions/country-region.md)
### Asie a Tichomoří
#### Austrálie
##### [Přidání polí sestavy a vygenerování australského výkazu podnikatelské činnosti (BAS)](../financials/localizations/tasks/add-bas-report-boxes.md)
##### [Formáty bankovních souborů](../financials/localizations/apac-aus-method-of-payment-pay-vendors-banks.md)
##### [Výkaz podnikatelské činnosti ](../financials/localizations/apac-aus-business-activity-statement.md)
##### [Odpisy skupiny majetku s nízkou hodnotou](../financials/localizations/apac-aus-low-value-pool-depreciation.md)
##### [Nastavení kódů důvodů výkazu BAS pro Austrálii](../financials/localizations/tasks/set-up-reason-codes-australia-bas.md)

#### Čína
##### [Porovnání kusovníků](../financials/localizations/apac-chn-bom-comparison.md)
##### [Čínská hierarchie účtové osnovy](../financials/localizations/tasks/china-hierarchy-chart-accounts.md)
##### [Kontrola kontinuity čínského dokladu](../financials/localizations/tasks/chinese-voucher-continuity-check.md)
##### [Čínské doklady](../financials/localizations/apac-chn-vouchers.md)
##### [Konfigurace integrace daní](../financials/localizations/apac-chn-tax-integration.md)
##### [Metody odpisu dlouhodobého majetku ](../financials/localizations/apac-chn-depreciation-methods-fixed-assets.md)
##### [Nastavení exportu integrace s čínským systémem Golden Tax](../financials/localizations/tasks/golden-tax-integration-export-setup.md)
##### [Import čínské daňové entity Chinese Golden](../financials/localizations/apac-chn-import-golden-tax-data-entity.md)
##### [Model měsíčních průměrných nákladů](../financials/localizations/apac-chn-monthly-average-cost-model.md)
##### [Zaúčtování hlavních účtů zásob podle pracoviště](../financials/localizations/apac-chn-post-inventory-main-accounts-by-site.md)
##### [Zaúčtování dokladů z jiných modulů](../financials/localizations/tasks/post-vouchers-other-modules-like-sales-invoices.md)
##### [Zaúčtování dokladů z hlavního deníku](../financials/localizations/tasks/post-vouchers-general-journal.md)
##### [Nastavení profilu základní daňové integrace pro Čínu](../financials/localizations/tasks/set-up-basic-tax-integration-profile-china.md)
##### [Nastavení čínských typů dokladů](../financials/localizations/tasks/set-up-chinese-vouchers.md)
##### [Nastavení přidělení odpisů dlouhodobého majetku](../financials/localizations/tasks/fixed-asset-depreciation-allocation.md)
##### [Nastavení kreditů záporných zůstatků ](../financials/localizations/apac-chn-negative-debits-credits.md)
##### [Protokol uživatelských operací podle čínského pracovního pravidla](../financials/localizations/tasks/user-operation-log-china-working-rule.md)
##### [Nejčastější dotazy týkající se zákaznických faktur s DPH](../financials/localizations/apac-chn-tax-integration-vat-customer-invoices.md)

#### Japonsko
##### [Pořízení dlouhodobého majetku a nárokování subvencí státního grantu](../financials/localizations/tasks/acquire-fixed-asset-claim-government-grant-subsidy.md)
##### [Pořízení dlouhodobého majetku s povinností vyřazení majetku](../financials/localizations/tasks/acquire-fixed-asset-asset-retirement-obligations.md)
##### [Úprava odhadu povinnosti vyřazení majetku](../financials/localizations/tasks/adjustment-asset-retirement-obligation-estimate.md)
##### [Přidělení zůstatkové hodnoty sdíleného majetku a nehmotného jmění k penězotvorným jednotkám](../financials/localizations/tasks/allocate-carrying-amount.md)
##### [Pravidla přidělení](../financials/localizations/apac-jpn-allocation-rules-fixed-assets.md)
##### [Bankovní platby pokryté dodavateli](../financials/localizations/apac-jpn-bank-payment-fees-vendors.md)
##### [Změna metody odpisování pro knihu v průběhu životnosti majetku](../financials/localizations/tasks/change-depreciation-method-during-asset-life-book.md)
##### [Změna metody odpisování pro jeden majetek v průběhu životnosti majetku](../financials/localizations/tasks/change-depreciation-method-during-asset-life-one-asset.md)
##### [Konfigurace parametrů zrychleného odpisování a účetních profilů](../financials/localizations/tasks/accelerated-depreciation-posting-profiles.md)
##### [Konfigurace parametrů a nastavení souhrnné faktury pro závazky](../financials/localizations/tasks/consolidated-invoice-parameters-setup-accounts-payable.md)
##### [Konfigurace odpisového plánu a účetního profilu pro další odpisy](../financials/localizations/tasks/configure-depreciation-profile-posting.md)
##### [Konfigurace hlavních účtů pro zaúčtování povinnosti vyřazení majetku a sazeb slev na trhu](../financials/localizations/tasks/configure-main-accounts-asset-retirement.md)
##### [Souhrnné faktury](../financials/localizations/apac-jpn-consolidate-invoices.md)
##### [Vytvoření dlouhodobého majetku s dodatečnými odpisy](../financials/localizations/tasks/create-fixed-asset-additional-depreciation.md)
##### [Vytvoření dlouhodobého majetku se zvláštním odpisovým plánem](../financials/localizations/tasks/create-fixed-asset-special-depreciation-profile.md)
##### [Vytvoření dokumentu zrychleného odpisování a zadání dat o využití](../financials/localizations/tasks/create-accelerated-depreciation-document-enter-usage-data.md)
##### [Vytvoření plánu zrychleného odpisování a jeho přiřazení knize](../financials/localizations/tasks/create-accelerated-depreciation-profile-assign-it-book.md)
##### [Vytvoření a přiřazení dokumentu položky snížení hodnoty pro subvence státního grantu](../financials/localizations/tasks/create-assign-reduction-document.md)
##### [Vytvoření a přiřazení skupiny vybavení](../financials/localizations/tasks/create-assign-equipment-group.md)
##### [Vytvoření a potvrzení souhrnné faktury odběratele](../financials/localizations/tasks/create-confirm-customer-consolidated-invoice.md)
##### [Vytvoření a potvrzení testu uznání](../financials/localizations/tasks/create-confirm-recognition-test.md)
##### [Vytvoření a potvrzení souhrnné faktury dodavatele](../financials/localizations/tasks/create-confirm-vendor-consolidated-invoice.md)
##### [Vytvoření skupiny jednotky CGU a penězotvorných jednotek](../financials/localizations/tasks/create-cgu-group-cash-generating-units.md)
##### [Vytvoření majetku s paušálními odpisy s použitím metody rovnoměrného dělení](../financials/localizations/tasks/create-lump-sum-depreciation-assets-equally-divided-method.md)
##### [Definování období nečinnosti majetku a ověření procesu odpisování](../financials/localizations/tasks/define-asset-idle-period-validate-depreciation-process.md)
##### [Odepsání a časové rozlišení úrokových výdajů pro povinnosti vyřazení majetku](../financials/localizations/tasks/depreciate-accrue-interest-expense.md)
##### [Odepsání dlouhodobého majetku se zaúčtovanou položkou snížení hodnoty](../financials/localizations/tasks/depreciation-fixed-assets-reduction-entry-posted.md)
##### [Nejčastější dotazy k odpisům](../financials/localizations/apac-jpn-fixed-asset-depreciation.md)
##### [Vyřazení dlouhodobého majetku s položkou snížení hodnoty](../financials/localizations/tasks/dispose-fixed-asset-reduction-entry.md)
##### [Povolení formátu souboru plateb JBA](../financials/localizations/tasks/jba-payment-file-format.md)
##### [Zadání plánu odpisové sazby a přiřazení k odpisovému plánu](../financials/localizations/tasks/enter-depreciation-rate-schedule.md)
##### [Rovnoměrně rozdělená metoda odpisů](../financials/localizations/apac-jpn-equally-divided-depreciation-method.md)
##### [Formulář 26 pro přiznání odepisovatelné daně](../financials/localizations/tasks/jp-00027-form-26-depreciable-tax-declaration.md)
##### [Vygenerování souboru plateb EFT ve formátu JBA](../financials/localizations/tasks/eft-payment-file-jba-format.md)
##### [Vygenerování a zaúčtování platebního poplatku](../financials/localizations/tasks/post-payment-fee.md)
##### [Vygenerování sestavy spotřební daně](../financials/localizations/tasks/japan-consumption-tax-report.md)
##### [Účtování se snížením hodnoty v jednotce CGU](../financials/localizations/apac-jpn-impairment-accounting-cash-generating-unit.md)
##### [Účtování se snížením hodnoty](../financials/localizations/apac-jpn-impairment-accounting-fixed-assets.md)
##### [Import platby odběratele ve formátu souboru JBA](../financials/localizations/tasks/import-customer-payment-jba-file-format.md)
##### [Import PSČ](../financials/localizations/apac-jpn-import-postal-codes.md) 
##### [Zadávání deníku ve formátu T účtu](../financials/localizations/tasks/jp-00005-input-journal-t-account-form-japan.md)
##### [Dotazy na transakce související s povinností vyřazení majetku](../financials/localizations/tasks/inquiries-asset-retirement-obligation.md)
##### [Dotazy na odečitatelné výdaje](../financials/localizations/tasks/inquiry-deductible-expenses.md)
##### [Dotazy na platební poplatky v deníku](../financials/localizations/tasks/inquiries-payment-fee-journal.md)
##### [Udržování souborů hlavních dat dlouhodobého majetku pro odečitatelné výdaje](../financials/localizations/tasks/maintain-fixed-asset-master-data-files-deductible-expenses.md)
##### [Udržování ukazatelů snížení hodnoty u jednotlivých položek majetku](../financials/localizations/tasks/maintain-impairment-indicators-individual-assets.md)
##### [Zaplacení dodavateli formou cizí směnky](../financials/localizations/apac-jpn-endorse-bill-of-exchange.md)
##### [Uhrazení dodavatelské transakce formou zákazníkovy směnky](../financials/localizations/tasks/pay-vendor-transaction.md)
##### [Pravidelné vyrovnání příliš velkých nebo příliš malých odpisů](../financials/localizations/tasks/periodic-settlement-over-under-depreciation.md)
##### [Výpočet progresivní srážkové daně](../financials/localizations/apac-jpn-progressive-withholding-tax-calculation.md)
##### [Návrh dodatečného odpisování](../financials/localizations/tasks/propose-additional-depreciation.md)
##### [Návrh a zaúčtování zrychleného odpisování](../financials/localizations/tasks/propose-post-accelerated-depreciation.md)
##### [Návrh a zaúčtování částky snížení hodnoty v dávce](../financials/localizations/tasks/propose-post-impairment-amount-batch.md)
##### [Návrh a zaúčtování částky snížení hodnoty s použitím deníku dlouhodobého majetku](../financials/localizations/tasks/propose-post-impairment-amount-fixed-asset-journal.md)
##### [Návrh a zaúčtování částky snížení hodnoty pro penězotvornou jednotku](../financials/localizations/tasks/propose-post-impairment-amount-cash-generating-unit.md)
##### [Návrh zvláštního odpisování](../financials/localizations/tasks/propose-special-depreciation.md)
##### [Opětovné otevření a úprava souhrnné faktury odběratele](../financials/localizations/tasks/reopen-edit-customer-consolidated-invoice.md)
##### [Opětovné otevření a úprava souhrnné faktury dodavatele](../financials/localizations/tasks/reopen-edit-vendor-consolidated-invoice.md)
##### [Stornování indosované cizí směnky](../financials/localizations/tasks/reverse-endorsed-bill-exchange.md)
##### [Spuštění testu uznání a výpočet částky snížení hodnoty pro jednotlivé položky majetku](../financials/localizations/tasks/run-recognition-test-calculate.md)
##### [Nastavení odběratele a prodejní objednávky jako cíle souhrnné faktury](../financials/localizations/tasks/set-up-customer-sales-order-target-consolidated-invoice.md)
##### [Nastavení ARO](../financials/localizations/apac-jpn-asset-retirement-obligation-fixed-assets.md)
##### [Nastavení dokumentů povinností vyřazení majetku a zadání částky povinnosti vyřazení majetku u dlouhodobého majetku](../financials/localizations/tasks/set-up-asset-retirement-obligation.md)
##### [Nastavení souhrnných faktur](../financials/localizations/tasks/set-up-consolidated-invoices.md)
##### [Nastavení běžných parametrů a účetního profilu pro účtování se snížením hodnoty](../financials/localizations/tasks/impairment-accounting.md)
##### [Nastavení sestavy japonské spotřební daně](../financials/localizations/tasks/setup-japan-consumption-tax-report.md)
##### [Nastavení platby pro Japonsko formou cizí směnky odběratele](../financials/localizations/tasks/setup-japan-payment-endorsing-customer-bill-exchange.md)
##### [Nastavení hlavních dat pro zahrnutí odečitatelných výdajů pro více účtovacích vrstev](../financials/localizations/tasks/set-up-master-data-inclusion.md)
##### [Nastavení platebního poplatku pro Japonsko](../financials/localizations/tasks/setup-payment-fee-japan.md)
##### [Nastavení záznamů redukce](../financials/localizations/apac-jpn-reduction-entry-fixed-assets.md)
##### [Nastavení hlavních dat dodavatele a nákupní objednávky jako cíle pro souhrnnou fakturu](../financials/localizations/tasks/vendor-master-po.md)
##### [Vyrovnání indosované cizí směnky](../financials/localizations/tasks/settle-endorsed-bill-exchange.md)
##### [Vyrovnání souhrnných faktur odběratelů s použitím deníku plateb](../financials/localizations/tasks/settle-customer-consolidated-invoices-payment-journal.md)
##### [Vyrovnání souhrnných faktur odběratelů s použitím vyrovnávacích transakcí](../financials/localizations/tasks/settle-customer-consolidated-invoices-settle-transactions.md)
##### [Vyrovnání souhrnných faktur dodavatelů s použitím deníku plateb](../financials/localizations/tasks/settle-vendor-consolidated-invoices-payment-journal.md)
##### [Vyrovnání souhrnných faktur dodavatelů s použitím vyrovnávacích transakcí](../financials/localizations/tasks/settle-vendor-consolidated-invoices-settle-transactions.md)
##### [Použití seznamu složení pro dlouhodobý majetek](../financials/localizations/tasks/use-assemble-list-fixed-asset.md)
##### [Použití seznamu rozložení pro dlouhodobý majetek](../financials/localizations/tasks/use-disassemble-list-fixed-assets.md)

#### Malajsie
##### [Vygenerování dobropisu odběratele pro prodeje projektu (Malajsie)](../financials/localizations/tasks/my-00011-03-customer-credit-note-project-sales.md)
##### [Vygenerování dluhopisu odběratele pro prodeje projektu (Malajsie)](../financials/localizations/tasks/my-00011-02-customer-debit-note-project-sales-2016-11.md)
##### [Daň ze zboží a služeb](../financials/localizations/apac-mys-gst.md)
##### [Daň GST – vygenerování souborů GAF v požadovaném formátu (Malajsie)](../financials/localizations/tasks/my-00010-gst-gaf-files-required-format.md)
##### [Správa debetu a kreditu odběratele pro daň GST (Malajsie)](../financials/localizations/tasks/my-00003-manage-customer-debit-note-credit-note-gst.md)
##### [Správa debetu a kreditu dodavatele pro daň GST (Malajsie)](../financials/localizations/tasks/my-00004-manage-vendor-debit-note-credit-note-gst.md)
##### [Tisk faktur odběratele s daní GST a klauzulí úlevy (Malajsie)](../financials/localizations/tasks/my-00006-02-print-gst-customer-invoices-relief-clause.md)
##### [Tisk faktur s daní GST (Malajsie)](../financials/localizations/tasks/my-00005-print-gst-tax-invoices.md)
##### [Faktury vystavené odběratelem (Malajsie)](../financials/localizations/tasks/my-00007-self-billed-invoices.md)
##### [Nastavení klauzulí úlevy daně GST (Malajsie)](../financials/localizations/tasks/my-00006-01-gst-relief-clauses.md)
#### Singapur
##### [Generování sestavy daně GST-05](../financials/localizations/apac-sgp-generate-gst-05-report.md)
#### Thajsko
##### [Sestavy karet zásob](../financials/localizations/apac-tha-stock-card-reports.md)
##### [Nahrazení nebo úprava daňové faktury](../financials/localizations/apac-tha-substitution-and-adjustment-invoices.md)
##### [Nerealizovaná DPH](../financials/localizations/apac-tha-unrealized-vat.md)
##### [Srážková daň](../financials/localizations/apac-tha-withholding-tax.md)

### Evropa
#### [Držitelé zálohy](../financials/localizations/emea-advance-holders.md)
#### [Záloha pro zaměstnance (východní Evropa)](../financials/localizations/tasks/advance-payment-employee.md)
#### [Rozšířená fakturace](../financials/localizations/emea-advance-invoice.md)
#### [Odsouhlasení banky](../financials/localizations/emea-bank-reconciliation.md)
#### [Opravná fakturace](../financials/localizations/emea-corrective-invoice.md)
#### [Vytváření a export plateb dodavatelů s použitím formátu platby ISO20022](../financials/localizations/tasks/create-export-vendor-payments-iso20022-payment-format.md)
#### [Tvorba a účtování periodických deníků](../financials/localizations/emea-create-post-periodic-journals.md)
#### [Vytvoření a odeslání sestavy DPH](../financials/localizations/tasks/create-submit-vat-report.md)
#### [Vytváření plateb pro odběratele, který má zmocnění k přímému debetu](../financials/localizations/tasks/create-payments-customers-who-have-direct-debit-mandates.md)
#### [Úprava ID faktur na prodejních objednávkách](../financials/localizations/emea-edit-invoice-id-sales-orders.md)
#### [Vstupní certifikáty EU](../financials/localizations/emea-entry-certificates.md)
#### [Souhrnné hlášení EU](../financials/localizations/emea-eu-sales-list.md)
#### [Úpravy kurzů](../financials/localizations/emea-exchange-rate-adjustments.md)
#### [Formáty souboru pro metody platby](../financials/localizations/emea-select-file-formats-for-the-method-of-payments.md)
#### [Vyřazení dlouhodobého majetku pro Estonsko a Litvu](../financials/localizations/emea-credit-note-reverse-fixed-asset-sale.md)
#### [Vygenerování dokladu převodu pro interní převod zásob](../financials/localizations/tasks/transfer-document-internal-inventory-transfer.md)
#### [Vygenerování prohlášení Intrastat EU](../financials/localizations/tasks/eur-00002-eu-intrastat-declaration.md)
#### [Vygenerování sestavy souhrnného hlášení (EU)](../financials/localizations/tasks/eur-00011-eu-sales-list-report.md)
#### [Import konfigurace převodu kreditu ve formátu ISO20022](../financials/localizations/tasks/import-iso20022-credit-transfer-configuration.md)
#### [Import konfigurace přímého debetu ve formátu ISO20022](../financials/localizations/tasks/import-iso20022-direct-debit-configuration.md)
#### [Import souborů ISO20022](../financials/localizations/emea-ISO20022-file-formats.md)
#### [Intrastat](../financials/localizations/emea-intrastat.md)
#### [Sestavy deníku zásob](../financials/localizations/emea-set-up-report-inventory-journal-names.md)
#### [Číslování faktur a dodacích listů pro Lotyšsko a Litvu](../financials/localizations/emea-invoices-packing-slips-numbering.md)
#### [Koncová data vystavení faktury](../financials/localizations/emea-invoice-issue-deadline.md)
#### [Vystavení vstupního certifikátu EU](../financials/localizations/tasks/eur-00012-issue-eu-entry-certificate.md)
#### [Normy OIOUBL](../financials/localizations/emea-oioubl-standards-electronic-invoicing.md)
#### [Odkaz na původní fakturu (východní Evropa)](../financials/localizations/tasks/ee-00004-original-invoice-reference.md)
#### [Vyhledání strany s použitím DIČ](../financials/localizations/tasks/eur-00015-party-search-vat-id.md)
#### [Nástroj pro generování platebních referencí (východní Evropa)](../financials/localizations/tasks/ee-00015-payment-reference-generation-tool.md)
#### [Sestava platebních dokladů](../financials/localizations/emea-eur-payment-slip-report-giro.md)
#### [Pokladní hotovost](../financials/localizations/emea-petty-cash.md)
#### [Zaúčtování předpořízení dlouhodobého majetku](../financials/localizations/emea-pre-acquisition-acquisition-fixed-asset.md)
#### [PSČ](../financials/localizations/emea-import-create-postal-codes-manually.md)
#### [Přijetí vstupního certifikátu EU](../financials/localizations/tasks/eur-00012-receive-eu-entry-certificate.md)
#### [Zaregistrování DIČ dodavatele](../financials/localizations/tasks/eur-00015-registration-vendor-vat-id.md)
#### [ID registrace](../financials/localizations/emea-registration-ids.md)
#### [Stornovací poplatky](../financials/localizations/emea-reverse-charge.md)
#### [Přehled převodu kreditu SEPA](../financials/accounts-payable/sepa-credit-transfer.md)
#### [Přehled přímého inkasa SEPA](../financials/accounts-receivable/sepa-direct-debit-overview.md)
#### [Nastavení bankovních účtů společnosti pro převody kreditu ve formátu ISO20022](../financials/localizations/tasks/set-up-company-bank-accounts-iso20022-credit-transfers.md)
#### [Nastavení bankovních účtů společnosti pro přímé debety ve formátu ISO20022](../financials/localizations/tasks/set-up-company-bank-accounts-iso20022-direct-debits.md)
#### [Nastavení kompenzace odběratelů a dodavatelů](../financials/localizations/emea-compensation-customer-vendor-transactions.md)
#### [Nastavení bankovních účtů odběratelů a zákazníků pro přímé debety ve formátu ISO20022](../financials/localizations/tasks/set-up-bank-accounts-iso20022-direct-debits.md)
#### [Nastavení souhrnného hlášení EU](../financials/localizations/tasks/eur-00011-eu-sales-list-reporting.md)
#### [Nastavení způsobu platby pro převody kreditu ve formátu ISO20022](../financials/localizations/tasks/set-up-method-payment-iso20022-credit-transfer.md)
#### [Nastavení způsobu platby pro přímý debet ve formátu ISO20022](../financials/localizations/tasks/setup-method-payment-iso20022-direct-debit.md)
#### [Nastavení formátu platebního dokladu pro faktury projektů](../financials/localizations/tasks/set-up-payment-slip-format-project-invoices.md)
#### [Nastavení podepisujících uživatelů tištěných formulářů](../financials/localizations/emea-set-up-signers-for-printing-forms.md)
#### [Nastavení dokladů převodu pro pohyb zboží v rámci společnosti](../financials/localizations/tasks/set-up-transfer-documents-goods-movement-inside-company.md)
#### [Nastavení DIČ](../financials/localizations/tasks/eur-00015-vat-id.md)
#### [Nastavení dodavatelů a bankovních účtů dodavatelů pro převody kreditu ve formátu ISO20022](../financials/localizations/tasks/set-up-vendor-iso20022-credit-transfers.md)
#### [Zadání adresy nakládky pro intrakomunitární transakci](../financials/localizations/tasks/eur-00002-specify-lading-address-intra-community.md)
#### [Záporné storno](../financials/localizations/emea-storno.md)
#### [Daňové předpisy](../financials/localizations/emea-tax-directives.md)
#### [Převod transakcí do systému Intrastat](../financials/localizations/tasks/transfer-transactions-intrastat.md)
#### [Aktualizace zobrazení částek v sestavách a dokumentech](../financials/localizations/emea-amount-printing-forms.md) 
#### [Směnný kurz DPH](../financials/localizations/emea-vat-exchange-rate.md)
#### [Sestava odsouhlasení DPH](../financials/localizations/tasks/eur-00018-vat-reconciliation-report.md)
#### [Transakce v rejstříku DPH pro východní Evropu](../financials/localizations/emea-vat-register-transactions.md)
#### [Vykazování DPH](../financials/localizations/emea-vat-reporting.md)
#### [Zobrazení transakcí u vyrovnání](../financials/localizations/emea-transactions-settlement-form.md)

#### Rakousko
##### [Půlroční plán odpisů](../financials/localizations/emea-aut-half-year-depreciation.md)
##### [Poplatky za obalový materiál](../financials/localizations/emea-aut-packing-material-fee-calculation.md)
##### [Nákupní cla](../financials/localizations/emea-aut-purchase-duties.md)
##### [Výpis DPH](../financials/localizations/emea-aut-vat-statement-details.md)
#### Belgie
##### [Bankovní výpis CODA](../financials/localizations/emea-bel-coda-bank-statement-import.md)
##### [Vytvoření a přenos transakcí do systému BLWI](../financials/localizations/tasks/be-00011-create-transfer-blwi.md)
##### [Daňové přiznání Intervat](../financials/localizations/emea-bel-intervat-tax-declaration.md)
##### [Nastavení sestavy PRODCOM](../financials/localizations/emea-bel-prodcom-report.md)
##### [Sestavy odsouhlasení](../financials/localizations/emea-bel-reconciliation-reports.md)
##### [Nastavení vykazování zůstatku platby](../financials/localizations/tasks/be-00011-set-up-payment-balance-reporting.md)

#### Česká republika
##### [Dobropis na platební slevu](../financials/localizations/emea-cze-credit-note-cash-discount.md)
##### [Zaokrouhlení odpisování](../financials/localizations/emea-cze-depreciation-rounding.md)
##### [Pozastavení odpisování](../financials/localizations/emea-cze-depreciation-suspension-holidays.md)
##### [Odpisy dlouhodobého majetku](../financials/localizations/emea-cze-fixed-assets-depreciation.md)
##### [Půlroční plán odpisů](../financials/localizations/emea-cze-half-depreciation-fixed-asset-disposal.md)
##### [Výkaznictví DPH ](../financials/localizations/emea-cze-intra-community-vat-transactions.md)
##### [Výpis DPH](../financials/localizations/emea-cze-vat-statement-details.md)

#### Dánsko
##### [Přiřazení formátu platebního dokladu k účtu odběratele](../financials/localizations/tasks/assign-payment-slip-format-customer-account.md)
##### [Vytvoření a zaúčtování faktury odběratele pro odběratele z veřejného sektoru](../financials/localizations/tasks/create-post-customer-invoice-public-sector-customer.md)
##### [Vytvoření a zaúčtování volné faktury odběratele pro odběratele z veřejného sektoru](../financials/localizations/tasks/create-post-free-text-invoice-public-sector-customer.md)
##### [Vytvoření a zaúčtování faktury projektu pro odběratele z veřejného sektoru](../financials/localizations/tasks/create-post-project-invoice-public-sector-customer.md)
##### [Import konfigurací elektronické fakturace OIOUBL](../financials/localizations/tasks/import-oioubl-electronic-invoicing-configurations.md)
##### [Zaúčtování volné faktury s platebním dokladem](../financials/localizations/tasks/post-free-text-invoice-payment-slip.md)
##### [Zaúčtování faktury projektu s platebním dokladem](../financials/localizations/tasks/post-project-invoice-payment-slip.md)
##### [Zaúčtování prodejní faktury s platebním dokladem](../financials/localizations/tasks/post-sales-invoice-payment-slip.md)
##### [Nastavení účtů odběratelů pro elektronickou fakturaci OIOUBL](../financials/localizations/tasks/set-up-customer-accounts-oioubl-electronic-invoicing.md)
##### [Nastavení pro elektronickou fakturaci OIOUBL](../financials/localizations/tasks/set-up-oioubl-electronic-invoicing.md)
##### [Nastavení formátu platebního dokladu](../financials/localizations/tasks/set-up-payment-slip-format.md)

#### Estonsko
##### [Výpis DPH](../financials/localizations/emea-est-vat-statement-details.md)

#### Francie
##### Veřejný sektor
###### [Účetnictví ve veřejném sektoru](../financials/localizations/emea-fra-public-sector-accounting.md)
###### [Závazky](../financials/localizations/emea-fra-commitments-public-sector.md)
###### [Mandats de paiement](../financials/localizations/emea-fra-mandats-de-paiement.md)
###### [Zásobování a zdroje](../financials/localizations/emea-fra-procurement-sourcing-public-sector.md)
###### [Titres de recette](../financials/localizations/emea-fra-titres-de-recette-public-sector.md)
###### [Blokování plateb faktur dodavatele](../financials/localizations/emea-fra-vendor-invoice-payment-holds-public-sector.md)
##### [Schválení a zaúčtování vystavení cizí směnky](../financials/localizations/tasks/fr-00004-approve-post-draw-bill-exchange.md)
##### [Chronologické doklady faktur](../financials/localizations/emea-fra-chronological-invoices-vouchers.md)
##### [Výpisy z účtu odběratele a dodavatele](../financials/localizations/tasks/fr-00002-customer-vendor-account-statements.md)
##### [Denní výpočet úroku pro prodlení a paušál ve faktuře a v upomínce](../financials/localizations/tasks/fr-00018-daily-interest.md)
##### [Derogační odpisy](../financials/localizations/emea-fra-derogatory-depreciation.md)
##### [Francouzské cizí směnky a vlastní směnky](../financials/localizations/tasks/fr-00004-french-bills-exchange-promissory-notes.md)
##### [Kódy NAF a čísla SIRET](../financials/localizations/emea-fra-naf-codes-siret-numbers.md)
##### [Nastavení kódů NAF a čísel SIRET](../financials/localizations/tasks/fr-00003-naf-codes-siret-numbers.md)
##### [Pokuty za zpožděné platby zákazníka](../financials/localizations/emea-fra-apply-penalty-customer-payment-past-due.md)
##### [Zaúčtování deníku protestů cizích směnek](../financials/localizations/tasks/fr-00004-post-protest-draw-bill-exchange-journal.md)
##### [Zaúčtování deníku opětovného vystavení cizích směnek](../financials/localizations/tasks/fr-00004-post-re-draw-bill-exchange-journal.md)
##### [Tisk textu paušálního penále na faktuře odběratele](../financials/localizations/emea-fra-print-lump-sum-recovery-text.md)
##### [RB/SL odpis](../financials/localizations/emea-fra-rbsl-depreciation.md)
##### [Nastavení metody platby](../financials/localizations/tasks/fr-00004-setup-method-payment.md)
##### [Aktualizace způsobu platby pro odběratele](../financials/localizations/tasks/fr-00004-update-method-payment-customer.md)
#### Německo
##### [Odpis dodatečných pořízení](../financials/localizations/emea-deu-additional-acquisition-depreciation.md)
##### [Soubor auditu ](../financials/localizations/emea-deu-gdpdu-audit-data-export.md)
##### [Přizpůsobení konfigurace souboru auditu](../financials/localizations/tasks/customize-german-audit-file-configuration.md)
##### [Opravy odpisů u dodatečných pořízení za druhý rok](../financials/localizations/tasks/de-00002-depreciation.md)
##### [Elektronický přenos přiznání k DPH (ELSTER)](../financials/localizations/tasks/de-00003-electronic-transmission-elster.md)
##### [Vygenerování souboru auditu](../financials/localizations/tasks/german-audit-file.md)
##### [Import konfigurace souboru auditu](../financials/localizations/tasks/import-german-audit-file-configuration.md)
##### [Sestava seznamu deníku](../financials/localizations/emea-deu-journal-list-report.md)
##### [Tisk samostatného štítku pro dobropisy z prodejů](../financials/localizations/emea-deu-print-separate-label-credit-memo-originating-sales.md)
##### [Nastavení elektronického daňového přiznání](../dev-itpro/analytics/tasks/setup-electronic-tax-declaration-germany.md)

#### Maďarsko
##### [Výpočet směnného kurzu](../financials/localizations/tasks/hu-00001-exchange-rate-calculation.md)
##### [Metody odpisu dlouhodobého majetku](../financials/localizations/emea-hun-fixed-assets-depreciation-methods.md)
##### [Nízkonákladový dlouhodobý majetek](../financials/localizations/emea-hun-low-cost-fixed-assets.md)
##### [Přesun dlouhodobého majetku do zásob](../financials/localizations/emea-hun-fixed-asset-to-inventory.md)

#### Island
##### [Vytvoření účtu dodavatele a připojení kategorie prohlášení k faktuře](../financials/localizations/tasks/create-vendor-account-attach-invoice-declaration-category.md)
##### [Nastavení prohlášení k faktuře pro dodavatele](../financials/localizations/tasks/set-up-invoice-declaration-vendors.md)
##### [Nastavení povinných platebních referencí](../financials/localizations/tasks/set-up-mandatory-payment-references.md)
##### [Aktualizace prohlášení k faktuře dodavatele a vygenerování sestavy](../financials/localizations/tasks/update-vendor-invoice-declarations-report.md)

#### Itálie
##### [Podrobnosti výkazů DPH](../financials/localizations/emea-ita-vat-statements-details.md)
##### [Sestava černé listiny](../financials/localizations/emea-ita-black-list-report.md)
##### [Datum kompetence](../financials/localizations/emea-ita-competence-date.md)
##### [Odpisování dlouhodobého majetku](../financials/localizations/emea-ita-depreciation-of-fixed-assets.md)
##### [Ověření data dodacího listu na fakturách](../financials/localizations/emea-ita-packing-slip-date-verification-on-invoice.md)
##### [Knihy DPH](../financials/localizations/emea-ita-fiscal-books.md)
##### [Rozdělená platba za faktury vydané pro veřejnou správu](../financials/localizations/emea-ita-split-payment-invoices-issued-public-administration.md)
##### [Srážková daň](../financials/localizations/emea-ita-withholding-tax.md)
#### Lotyšsko
##### [Účtování dlouhodobého majetku](../financials/localizations/emea-lva-fixed-assets-accounting.md)
##### [Sledování změn umístění dlouhodobého majetku](../financials/localizations/emea-lva-fixed-assets-location-fields-change.md)
##### [Sestava daně z přírodních zdrojů](../financials/localizations/emea-lva-tax-natural-resources.md)
##### [Podrobnosti výkazu DPH](../financials/localizations/emea-lva-vat-statement-details.md)
#### Litva
##### [Výkaz opravy dlouhodobého majetku](../financials/localizations/emea-ltu-fixed-asset-repair-statement.md)
##### [Vygenerování dokumentu převodu dlouhodobého majetku mezi sklady](../financials/localizations/tasks/lt-00003-fixed-asset-transfer.md)
##### [Podrobnosti výpisu DPH](../financials/localizations/emea-ltu-vat-statement-details.md)

#### Nizozemsko
##### [Elektronické daňové přiznání (EVAT)](../financials/localizations/tasks/nl-00010-electronic-tax-declarations-evat.md)
##### [Integrace Semansys XBRL](../financials/localizations/tasks/nl-00003-semansys-xbrl-integration.md)

#### Norsko
##### [Formáty plateb odběratelů a dodavatelů](../financials/localizations/tasks/no-00003-customer-vendor-payment-formats.md)
##### [Platba odběratele na základě ID platby](../financials/localizations/tasks/no-00002-customer-payment-based-payment-id.md)
##### [Formát importu Nets](../financials/localizations/emea-nor-nets-import-format.md)
##### [Výpis DPH](../financials/localizations/emea-nor-sales-tax-payment-report.md) 
#### Polsko
##### [Výpočet slev řádku z jednotkové ceny](../financials/localizations/emea-pol-line-discount-calculation-from-unit-price.md)
##### [Poznámky k opravě](../financials/localizations/emea-pol-correction-notes.md)
##### [Duplikování číslování prodejních faktur](../financials/localizations/emea-pol-sales-invoice-duplicates-numbering.md)
##### [Odpisy dlouhodobého majetku](../financials/localizations/emea-pol-fixed-assets-depreciation.md)
##### [Rozšíření funkce disponibility investičního majetku](../financials/localizations/emea-pol-fixed-asset-disposal-functionality-extension.md)
##### [Vratný obal](../financials/localizations/emea-pol-returnable-packages.md)
##### [Sestavy DPH](../financials/localizations/emea-pol-sales-tax-reports.md)
##### [Soubor standardního auditu](../financials/localizations/emea-pol-standard-audit-file-saf.md)
##### [Výkaz obratu](../financials/localizations/emea-pol-turnover-balances-statement.md)
#### Španělsko
##### [Použití hotovostních slev na faktuře](../financials/localizations/emea-esp-cash-discount-applied-invoice.md)
##### [Možnosti cizí směnky](../financials/localizations/emea-esp-bill-of-exchange-options.md)
##### [Nerealizovaná DPH](../financials/localizations/emea-esp-conditional-sales-tax.md)
##### [Účet faktury fiskálních dat](../financials/localizations/emea-esp-fiscal-data-invoice-account.md)
##### [Přecenění skupiny dlouhodobého majetku](../financials/localizations/emea-esp-fixed-asset-group-revaluation.md)
##### [DPH intrakomunitárního plnění](../financials/localizations/emea-esp-intra-community-vat.md)
##### [Jednorázoví odběratelé](../financials/localizations/emea-esp-no-one-time-customer-for-project-contracts.md)
##### [Tisk šeků se španělským rozvržením](../financials/localizations/emea-esp-print-checks-with-spanish-layout.md)
##### [Možnosti vlastní směnky](../financials/localizations/emea-esp-promissory-note-options.md)
##### [Prodejní a nákupní cykly](../financials/localizations/emea-esp-sales-purchase-cycle.md)
##### [Speciální záznamy a počáteční stavy](../financials/localizations/emea-esp-opening-sheets-spain.md)
#### Švédsko
##### [Import formátů plateb](../financials/localizations/emea-swe-payment-formats-import.md)
#### Švýcarsko
##### [Import plateb odběratelů v ESR](../financials/localizations/emea-che-esr-customer-payments-import.md)
#### Spojené království
##### [Schéma pro obor stavebnictví ](../financials/localizations/emea-gbr-cis-construction-industry-scheme.md)
##### [Vytvoření dobropisu pro slevu při vyrovnání](../financials/localizations/tasks/gb-00009-credit-note-settlement-discount.md)
##### [Vytvoření nákupní objednávky obsahující položky, na které se vztahuje stornovací poplatek DPH](../financials/localizations/tasks/purchase-order-reverse-charge-vat.md)
##### [Vytvoření prodejní objednávky obsahující položky, na které se vztahuje stornovací poplatek DPH](../financials/localizations/tasks/gb-00002-sales-order.md)
##### [Nastavení skupin DPH položky pro stornovací poplatek DPH](../financials/localizations/tasks/gb-00002-item-sales-tax-groups-reverse-charge-vat.md)
##### [Nastavení skupin položek, pravidel a parametrů stornovacího poplatku DPH](../financials/localizations/tasks/gb-00002-reverse-charge-vat-item-groups.md)
##### [Nastavení skupin DPH pro stornovací poplatek DPH](../financials/localizations/tasks/gb-00002-sales-tax-groups-reverse-charge-vat.md)

### Latinská Amerika
#### Brazílie
##### [Fiskální knihy](../financials/localizations/latam-bra-fiscal-books.md)
###### [Fiskální dokumenty daně ICMS](../financials/localizations/latam-bra-icms-tax-fiscal-documents.md)
###### [Import fiskálních dokumentů](../financials/localizations/latam-bra-import-fiscal-documents.md)
###### [Export a import transakcí právnické osoby z konsolidace hlavní knihy](../financials/localizations/latam-bra-general-ledger-consolidation-transactions.md)
###### [Import a ověření dokumentů NF-e XML a souborů DANFE](../financials/localizations/latam-bra-import-verify-nf-e-xml-documents-danfe-emails.md)
###### [Zpracování elektronických platebních úhrad](../financials/localizations/latam-bra-process-electronic-payment-remittances.md)
###### [Fiskální dokumenty](../financials/localizations/latam-bra-fiscal-documents-fiscal-document-framework.md)
###### [Zástupný text u fiskálních dokumentů](../financials/localizations/latam-bra-fiscal-document-text-placeholders.md)
###### [Nákupní doplňkové fiskální dokumenty](../financials/localizations/latam-bra-purchase-complementary-fiscal-documents.md)
###### [Odkazované fiskální dokumenty](../financials/localizations/latam-bra-referenced-fiscal-documents.md)
###### [Prodejní doplňkové fiskální dokumenty](../financials/localizations/latam-bra-sales-complementary-fiscal-documents.md)
###### [Elektronický fiskální dokument služby](../financials/localizations/latam-bra-service-electronic-fiscal-document.md)
###### [Deníky generované během procesu vracení elektronické platby](../financials/localizations/latam-bra-examples-journals-generated-electronic-payment-return-process.md)
##### [Proces NF-e](../financials/localizations/latam-bra-nf-e-process.md)
##### [Parametry pro právnické osoby](../financials/localizations/latam-bra-legal-entity-parameters.md)
##### [Pořízení dlouhodobého majetku CIAP](../financials/localizations/latam-bra-ciap-fixed-asset.md)
##### [Vyřazení dlouhodobého majetku CIAP](../financials/localizations/latam-bra-ciap-fixed-asset.md)
##### [Daně](../financials/localizations/latam-bra-calculate-taxes.md)
###### [Daňové atributy](../financials/localizations/latam-bra-tax-attributes.md)
###### [Platby daně](../financials/localizations/latam-bra-tax-payments.md)

##### Úkoly pro Brazílii
###### [Nastavení parametrů právnické osoby](../financials/localizations/tasks/br-00001-1-set-up-brazil-legal-entity-parameters.md)
###### [Nastavení zdrojového textu fiskálních dokumentů](../financials/localizations/tasks/br-00001-2-set-up-fiscal-document-source-text.md)
###### [Daňové atributy fiskálního subjektu](../financials/localizations/tasks/br-00002-fiscal-establishment-tax-attributes.md)
###### [Srážkové daně – Brazílie ](../financials/localizations/tasks/br-00009-brazilian-withholding-taxes.md)
###### [Přijetí fiskálních dokumentů dodavatele (dlouhodobý majetek – dlouhodobě vratné daně PIS a COFINS)](../financials/localizations/tasks/br-00014-receiving-vendor-fiscal-document-fixed-assets-pis-cofins-long-term-recoverable.md)
###### [Přijetí fiskálních dokumentů dodavatele (produkty)](../financials/localizations/tasks/br-00024-receiving-vendor-fiscal-document-products.md)
###### [Vytvoření a zaúčtování nákupního doplňkového fiskálního dokumentu](../financials/localizations/tasks/br-00026-1-create-post-purchase-complementary-fiscal-document.md)
###### [Zrušení nákupního doplňkového fiskálního dokumentu](../financials/localizations/tasks/br-00026-2-cancel-purchase-complementary-fiscal-document.md)
###### [Výpočet daně v nákupních nabídkách](../financials/localizations/tasks/br-00031-tax-calculation-purchase-quotation.md)
###### [Vystavení fiskálních dokumentů pro dodavatele](../financials/localizations/tasks/br-00033-issuing-fiscal-document-non-taxable-vendors.md)
###### [Vystavení fiskálních dokumentů odběratele](../financials/localizations/tasks/br-00038-issuing-customer-fiscal-document.md)
###### [Dodací listy](../financials/localizations/tasks/br-00041-delivery-slip.md)
###### [Srážkové daně pro platby odběratelů](../financials/localizations/tasks/br-00042-withholding-taxes-customers-payments.md)
###### [Vedlejší náklady na volných fakturách](../financials/localizations/tasks/br-00050-miscellaneous-charge-free-text-invoice.md)
###### [Vytvoření a zaúčtování prodejního doplňkového fiskálního dokumentu](../financials/localizations/tasks/br-00052-1-create-post-sales-complementary-fiscal-documents.md)
###### [Zrušení prodejního doplňkového fiskálního dokumentu](../financials/localizations/tasks/br-00052-2-cancel-sales-complementary-fiscal-document.md)
###### [Nastavení federálních parametrů NF-e](../financials/localizations/tasks/br-00053-1-set-up-nf-e-federal-parameters.md)
###### [Nastavení parametrů NF-e pro fiskální subjekt](../financials/localizations/tasks/br-00053-2-set-up-nf-e-parameters-fiscal-establishment.md)
###### [Generování e-mailů pro schválený dokument NF-e a připojení souborů DANFE PDF a souborů NF-e XML k e-mailům](../financials/localizations/tasks/br-00053-3-generate-emails-approved-nf-e-attach-danfe-pdf-files-nf-e-xml-files-emails.md)
###### [NF-e 3.10](../financials/localizations/tasks/br-00053-nf-e-3-10.md)
###### [Brazilská daň v prodejní nabídce](../financials/localizations/tasks/br-00057-brazilian-taxes-sales-quotation.md)
###### [Automatické předávání fiskálních dokumentů NF-e](../financials/localizations/tasks/br-00058-automatic-transmission-nf-e-fiscal-documents.md)
###### [Elektronický fiskální dokument služby využívající textové soubory](../financials/localizations/tasks/br00059-service-electronic-fiscal-document-text-files.md)
###### [Srážkové daně pro platby dodavatelů](../financials/localizations/tasks/br-00061-withholding-taxes-vendors-payments.md)
###### [Nastavení úroků a pokut pro platby dodavatelů](../financials/localizations/tasks/br-00065-1-set-up-interest-fines-vendor-payments.md)
###### [Výpočet úroků a pokut pro platby dodavatelů](../financials/localizations/tasks/br-00065-2-calculate-interest-fines-vendor-payments.md)
###### [Nastavení úroků a pokut pro platby odběratelů](../financials/localizations/tasks/br-00066-1-set-up-interest-fines-customer-payments.md)
###### [Výpočet úroků a pokut pro platby odběratelů](../financials/localizations/tasks/br-00066-2-calculate-interest-fines-customer-payments.md)
###### [Správa elektronických plateb pro platby dodavatelů](../financials/localizations/tasks/br-00067-electronic-payment-management-vendor-payments.md)
###### [Vystavení fiskálních dokumentů převodu daňového dobropisu ICMS](../financials/localizations/tasks/br-00071-issuing-icms-tax-credit-transfer-fiscal-document.md)
###### [Vystavení fiskálních dokumentů daňového dobropisu ICMS](../financials/localizations/tasks/br-00072-issuing-icms-tax-credit-fiscal-document.md)
###### [Dluhopisy projektů](../financials/localizations/tasks/br-00076-project-debit-notes.md)
###### [Správa textů fiskálních dokumentů v dokumentech zásobování](../financials/localizations/tasks/br-00080-manage-fiscal-document-texts-procurement-documents.md)
###### [Přijetí fiskálních dokumentů dodavatele (dlouhodobý majetek – dlouhodobě vratná daň ICMS)](../financials/localizations/tasks/br-00082-receiving-vendor-fiscal-document-fixed-assets-icms-long-term-recoverable.md)
###### [Vystavení fiskálních dokumentů odběratele (služby)](../financials/localizations/tasks/br-00084-issuing-customer-fiscal-documents-services.md)
###### [Vystavení fiskálních dokumentů odběratele (dlouhodobý majetek)](../financials/localizations/tasks/br-00085-issuing-customer-fiscal-documents-fixed-assets.md)
###### [Vystavení fiskálních dokumentů odběratele (projekt)](../financials/localizations/tasks/br-00086-issuing-customer-fiscal-documents-project.md)
###### [Vystavení fiskálních dokumentů odběratele (pro koncové uživatele)](../financials/localizations/tasks/br-00087-issuing-customer-fiscal-documents-end-user.md)
###### [Vystavení fiskálních dokumentů odběratele (pro odběratele SUFRAMA)](../financials/localizations/tasks/br-00088-issuing-customer-fiscal-documents-suframa-customers.md)
###### [Zrušení fiskálního dokumentu odběratele (projekt)](../financials/localizations/tasks/br-00092-cancel-customer-fiscal-document-project.md)
###### [Nastavení kódů oprav pro daň ICMS](../financials/localizations/tasks/br-10001-1-set-up-adjustment-codes-icms-tax.md)
###### [Nastavení kódů oprav pro daně ICMS ve fiskálních dokumentech](../financials/localizations/tasks/br-10001-2-set-up-adjustment-codes-icms-taxes-fiscal-documents.md)
###### [Zadání a zaúčtování transakcí oprav daně](../financials/localizations/tasks/br-10001-3-enter-post-tax-adjustment-transactions.md)
###### [Vytvoření vyměření daně – ICMS](../financials/localizations/tasks/br-10001-4-create-tax-assessment-icms.md)
###### [Nastavení kódů oprav pro daně IPI](../financials/localizations/tasks/br-10004-1-set-up-adjustment-codes-ipi-taxes.md)
###### [Vytvoření vyměření daně – daň IPI](../financials/localizations/tasks/br-10004-2-create-tax-assessment-ipi-tax.md)
###### [Vyměření daně pro daň ISS](../financials/localizations/tasks/br-10005-tax-assessment-iss-tax.md)
###### [Nastavení tabulek PIS a COFINS](../financials/localizations/tasks/br-10006-1-set-up-pis-cofins-tables.md)
###### [Nastavení kódů oprav pro daně PIS a COFINS](../financials/localizations/tasks/br-10006-2-set-up-adjustment-codes-pis-cofins-taxes.md)
###### [Zadání transakcí oprav daní PIS a COFINS](../financials/localizations/tasks/br-10006-3-enter-pis-cofins-tax-adjustment-transactions.md)
###### [Vytvoření vyměření daně – daně PIS a COFINS](../financials/localizations/tasks/br-10006-4-create-tax-assessment-pis-cofins-tax.md)
###### [Nefiskální operace](../financials/localizations/tasks/br-10016-non-fiscal-operations.md)
###### [Zadání transakce CIAP](../financials/localizations/tasks/br-10020-1-enter-ciap-transaction.md)
###### [Výpočet částky dobropisu CIAP](../financials/localizations/tasks/br-10020-2-calculate-ciap-credit-amount.md)
###### [Zahrnutí dobropisu CIAP z předchozího období](../financials/localizations/tasks/br-10020-3-include-ciap-credit-from-previous-period.md)
###### [Daně PIS a COFINS z dlouhodobého majetku](../financials/localizations/tasks/br-10020-4-pis-cofins-fixed-assets.md)

#### Mexiko
##### [Sestava prohlášení o úpravě inflace](../financials/localizations/latam-mex-adjustment-inflation-declaration-report.md)
##### [Zrušení elektronické faktury](../financials/localizations/tasks/mx-00010-cancel-electronic-invoice.md)
##### [ID registrace odběratele a další informace](../financials/localizations/tasks/mx-00007-customer-registration-ids-other-information.md)
##### [Sestava prohlášení DIOT](../financials/localizations/latam-mex-diot-declaration-statement.md)
##### [Elektronické faktury CFDI](../financials/localizations/tasks/mx-00010-e-invoicing-cfdi.md)
##### [Elektronické faktury ](../financials/localizations/latam-mex-CFDI-electronic-invoices.md)
##### [Výpisy účtování elektronické knihy](../financials/localizations/latam-mex-electronic-ledger-accounting-statements.md)
##### [Zadání ID registrace daně pro mexické právnické osoby](../financials/localizations/tasks/mx-00010-enter-tax-registration-ids-mexican-legal-entities.md)
##### [Dotazy na elektronické faktury a jejich tisk](../financials/localizations/tasks/mx-00010-inquire-print-electronic-invoice.md)
##### [Sestava prohlášení ISR](../financials/localizations/latam-mex-isr-declaration-report-supporting-processes.md)
##### [Zaúčtování volné faktury](../financials/localizations/tasks/mx-00010-post-free-text-invoice.md)
##### [Vytvoření mexického účetního výkazu elektronické hlavní knihy verze 1.1](../financials/localizations/tasks/mx-00020-electronic-ledger.md)
##### [Nastavení parametrů pro elektronickou fakturu](../financials/localizations/tasks/mx-00010-set-parameters-electronic-invoice.md)
##### [Nastavení kódů DPH pro Mexiko](../financials/localizations/tasks/mx-00006-sales-tax-code.md)
##### [Nastavení účtové osnovy pro právnickou osobu v Mexiku](../financials/localizations/tasks/mx-00020-chart-accounts-legal-entity-mexico.md)
##### [Zadání registračního ID RFC pro mexické bankovní účty](../financials/localizations/tasks/mx-00020-specify-rfc-registration-id-mexican-bank-accounts.md)
##### [ID registrace dodavatele](../financials/localizations/tasks/mx-00008-vendor-registration-ids.md)

### Střední Východ
#### [Vykazování zakátu pro Saúdskou Arábii](../financials/localizations/emea-sau-zakat-reporting.md)

### Severní Amerika
#### [Vykazování 1099 pro USA ke konci roku](../financials/localizations/noam-usa-year-end-1099-reporting.md)
#### [Formulář 1099 pro veřejný sektor](../financials/localizations/noam-usa-form-1099-public-sector.md)

## [Lidské zdroje](hr/hr-landing-page.md)
### Vytvoření strategie lidských zdrojů
#### [Uspořádání zaměstnanců podle oddělení, prací a pozic](../talent/departments-jobs-positions.md)
#### [Nastavení komponent práce](../talent/create-job.md)
#### [Vytvoření oddělení a jeho přidružení k hierarchii oddělení](../talent/create-department-add-department-hierarchy.md)
#### [Nastavení parametrů lidských zdrojů mezi právnickými osobami](../talent/set-up-hr-parameters-across-legal-entities.md)
#### [Nastavení parametrů lidských zdrojů pro konkrétní společnost](../talent/set-up-company-specific-hr-parameters.md)
#### [Často kladené dotazy k akcím personálu](../talent/personnel-actions-faq.md)
#### [Správa informací pomocí workflowu](../talent/workflow-manage-employee-information.md)

### Nábor
#### [Řízení náborového procesu](hr/manage-recruiting-process.md)
#### [Projekty hromadného zařazení](hr/mass-hire-projects.md)

### Motivace zaměstnanců
#### [Plány kompenzace](../talent/compensation-plans.md)
#### [Vytvoření plánů fixní kompenzace](../talent/create-fixed-compensation-plans.md)
#### [Vytvoření plánů variabilní kompenzace](../talent/create-variable-compensation-plans.md)
#### [Proces kompenzace](../talent/process-compensation.md)

### Rozvoj a zaškolení zaměstnanců
#### [Přehled řízení výkonnosti](../talent/performance-management-overview.md)
#### [Vyrovnání dovedností zaměstnanců s obchodními potřebami](../talent/skills.md)
#### [Nastavení školicích kurzů](../talent/courses.md)

### Vytvoření a správa výhod
#### [Správa programu výhod](../talent/manage-benefit-program.md)
#### [Zásady způsobilosti k zaměstnaneckým výhodám](../talent/benefit-eligibility-policies.md)
#### [Spravovat položky půjčené zaměstnancům](../talent/loan-items.md)

### Shromáždění informací pomocí dotazníku
#### [Dotazníky](../talent/questionnaires.md)
#### [Návrh dotazníku](../talent/design-questionnaires.md)
#### [Distribuce a vyplnění dotazníku](../talent/distribute-questionnaires.md)
#### [Hodnocení výsledků dotazníku](../talent/evaluate-questionnaire-results.md)

### Bezpečnost na pracovišti a dodržování předpisů (USA)
#### [Ověřování zaměstnanců pomocí formuláře I-9](hr/localizations/noam-usa-form-i-9-verification.md)
#### [Ověřování zaměstnanců pomocí formuláře I-9](hr/localizations/tasks/employment-verification-i9-verification.md)
#### [Nastavení typů dokumentů I-9](hr/localizations/tasks/set-up-i9-document-types.md)
#### [Vyhovění normě ADA](hr/localizations/noam-usa-comply-ada.md)
#### [Správa přizpůsobení poskytovaných pracovníkům](hr/localizations/tasks/manage-worker-accommodations.md)

### [Mzdy v USA](hr/localizations/noam-usa-payroll.md)
#### Nastavení mezd
##### [Nastavení mezd pro pracovníky](hr/localizations/noam-usa-worker-position-payroll-tasks.md)
##### [Nastavení pracovních cyklů a pracovních období](hr/localizations/noam-usa-work-cycle-work-period-tasks.md)
##### [Nastavení plánů s časovým rozlišením ](hr/localizations/noam-usa-benefit-accrual-plan-tasks.md)
##### [Nastavení výhod](hr/localizations/noam-usa-benefit-set-up-tasks.md)

#### Intervaly a cykly
#### [Úkoly platebního cyklu a platebního období](hr/localizations/noam-usa-pay-cycle-pay-period-tasks-sample.md)
#### [Úkoly výpočtu mezd a frekvence](hr/localizations/noam-usa-payroll-calculation-frequencies-tasks.md)
#### [Úkoly pracovního rozvrhu a dovolené](hr/localizations/noam-usa-work-schedule-leave-tasks.md)
#### Kódy a daně
##### [Kód příjmu a úkoly skupiny kódu příjmu](hr/localizations/noam-usa-earning-code-group-tasks.md)
##### [Úkoly daňových informací](hr/localizations/noam-usa-tax-information-tasks.md)
#### Obstavení, pružné daně a výplata prémií
##### [Nastavení obstavení a pružných daní](hr/localizations/noam-usa-garnishment-tax-levy-set-up-tasks.md)
##### [Přihlášení pracovníků k obstavení a pružným daním](hr/localizations/noam-usa-garnishment-tax-levy-enrollment-tasks.md)
##### [Nastavení prémiových příjmů ](hr/localizations/noam-usa-premium-earning-setup-tasks.md)
##### [Nejčastější dotazy týkající se obstavení, pružných daní a administrativních poplatků](hr/localizations/noam-usa-garnishment-tax-levy-administrative-fees.md)
#### Úkoly zpracování mezd
##### [Generování příjmů](hr/localizations/noam-usa-earnings-generation-process.md)
##### [Vystavení plateb pracovníkům](hr/localizations/noam-usa-issue-worker-payments.md)
##### [Zaúčtování platů a generování faktur pro dodavatele](hr/localizations/noam-usa-post-payroll-generate-vendor-invoices.md)
##### [Nejčastější dotazy k výkazům mezd a generování plateb](hr/localizations/noam-usa-pay-statements-payment-generation-process.md)
#### Dokončení procesu platby
##### [Zpracování stávajících plateb mezd](hr/localizations/noam-usa-existing-payroll-payments.md)
##### [Zpracování stávajících příjmů](hr/localizations/noam-usa-existing-earnings.md)
##### [Zpracování výkazů mezd](hr/localizations/noam-usa-pay-statements.md)
#### Údržba a vykazování
##### [Nejčastější dotazy týkající se údajů o mzdách](hr/localizations/noam-usa-payroll-data-updates.md)
##### [Generování mzdových sestav](hr/localizations/noam-usa-generate-payroll-reports.md)

### Úkoly lidských zdrojů
#### [Přidávání informací do deníku výkonnosti a udělení pochvaly některému z uživatelů](hr/tasks/add-your-performance-journal-send-praise-someone.md)
#### [Analýza výsledků dotazníku](hr/tasks/analyze-questionnaire-results.md)
#### [Schválení záznamů doručených přihlášek](hr/tasks/approve-application-inbox-records.md)
#### [Proces získání nároku na zaměstnaneckou výhodu](hr/tasks/benefit-eligibility-process.md)
#### [Vytvoření kalendáře a vygenerování pracovní doby](hr/tasks/create-calendar-working-times.md)
#### [Vytvoření dotazu s uzavřeným koncem](hr/tasks/create-closed-ended-question.md)
#### [Vytvoření cíle](hr/tasks/create-goal.md)
#### [Vytváření položek výpůjček](hr/tasks/create-loan-items.md)
#### [Vytvoření projektu hromadného zařazení](hr/tasks/create-mass-hire-project.md)
#### [Vytvoření nové zaměstnanecké výhody](hr/tasks/create-new-benefit.md)
#### [Vytvoření dotazu s otevřeným koncem](hr/tasks/create-open-ended-question.md)
#### [Vytvoření kontroly výkonnosti](hr/tasks/create-performance-review.md)
#### [Definování pravidel a zásad nároků na zaměstnanecké výhody](hr/tasks/define-benefit-eligibility-rules-policies.md)
#### [Definování procesu kompenzací a výpočet výsledků](hr/tasks/define-compensation-process-calculate-results.md)
#### [Definování nových oddělení](hr/tasks/define-new-departments.md)
#### [Definování nových pracovních míst](hr/tasks/define-new-jobs.md)
#### [Definování programu zaměstnaneckých výhod](hr/tasks/deliver-employee-benefits-program.md)
#### [Vytvoření a otevření požadavku na pozici](hr/tasks/develop-open-job-requisition.md)
#### [Vývoj struktury platu/kompenzace a plánů](hr/tasks/develop-salary-compensation-structure-plan.md)
#### [Vypracování plánu kariérního postupu](hr/tasks/develop-succession-plan.md)
#### [Distribuce dotazníků s použitím plánování](hr/tasks/distribute-questionnaires-scheduling.md)
#### [Přihlášení zaměstnance k plánu fixní kompenzace](hr/tasks/enroll-employee-fixed-compensation-plan.md)
#### [Přihlášení zaměstnance k plánu variabilní kompenzace](hr/tasks/enroll-employee-variable-compensation-plan.md)
#### [Registrace a odebrání zaměstnaneckých výhod pracovníků](hr/tasks/enroll-remove-benefits-workers.md)
#### [Ruční zadávání dat o přihlášce a uchazeči](hr/tasks/enter-applicant-application-data-manually.md)
#### [Zadávání časových rozvrhů projektů](hr/tasks/enter-project-timesheets.md)
#### [Zadávání informací o pracovnících](hr/tasks/enter-worker-information.md)
#### [Přijetí kandidáta prostřednictvím náboru](hr/tasks/hire-candidate-through-recruiting.md)
#### [Najmutí existujícího zaměstnance prostřednictvím náboru](hr/tasks/hire-existing-employee-through-recruiting.md)
#### [Identifikování a nasazení nástrojů pro výběr kandidáta](hr/tasks/identify-deploy-candidate-selection-tools.md)
#### [Zapůjčení položek pracovníkovi](hr/tasks/loan-item-worker.md)
#### [Udržování informací o zraněních a onemocněních zaměstnanců](hr/tasks/maintain-employee-injury-illness-information.md)
#### [Nastavení dotazu jako závislého na odpovědi na předcházející otázku](hr/tasks/make-question-dependent-answer-previous-question.md)
#### [Správa dat vypršení platnosti zaměstnaneckých výhod](hr/tasks/manage-benefit-expiration-dates.md)
#### [Správa šablon e-mailů](hr/tasks/manage-email-templates.md)
#### [Správa dovolené](hr/tasks/manage-leave-absence.md)
#### [Úprava vztahů podřízenosti pro určitou pozici](hr/tasks/modify-reporting-relationships-position.md)
#### [Nastavení kompenzačních mřížek](hr/tasks/set-up-compensation-grids.md)
#### [Nastavit informace o zraněních a onemocněních](hr/tasks/set-up-injury-illness-information.md)
#### [Nastavení pozic](hr/tasks/set-up-positions.md)

## [Intelligence](../dev-itpro/analytics/bi-reporting-home-page.md)
### [Přístup k informacím a vykazování](../dev-itpro/analytics/information-access-reporting.md)
### [Obsah Power BI](../dev-itpro/analytics/power-bi-home-page.md)
#### [Skutečnost a rozpočet](../dev-itpro/analytics/ledger-budgets-power-bi.md)
#### [Výhody](../dev-itpro/analytics/benefits-power-bi.md)
#### [Přehled hotovosti](../financials/cash-bank-management/Cash-Overview-Power-BI-content.md)
#### [Přehled CFO](../dev-itpro/analytics/CFO-power-bi.md)
#### [Kompenzace](../dev-itpro/analytics/compensation-power-bi.md)
#### [Kompenzace a zaměstnanecké výhody](../dev-itpro/analytics/compensation-and-benefits-analysis-power-bi-content-pack.md)
#### [Analýza nákladového účetnictví](../dev-itpro/analytics/cost-accounting-analysis-content-pack.md)
##### [Zabezpečení pro analýzy nákladového účetnictví](../dev-itpro/analytics/setup-security-cost-accounting-content-pack.md)
#### [Správa nákladů](../dev-itpro/analytics/cost-management-content-pack.md)
#### [Správa kreditu a inkasa](../financials/accounts-receivable/credit-collections-power-bi.md)
#### [Kompetence a rozvoj zaměstnance](../dev-itpro/analytics/employee-competencies-and-development-analysis-power-bi-content-pack.md)
#### [Rozvoj zaměstnance](../dev-itpro/analytics/employee-development-PBI.md) 
#### [Finanční výkonnost](../dev-itpro/analytics/financial-performance-power-bi-content-pack.md)
#### [Správa dlouhodobého majetku](../financials/fixed-assets/Fixed-asset-management-workspace.md)
#### [Výuka](../dev-itpro/analytics/learning-power-bi.md)
#### [Školení organizace](../dev-itpro/analytics/organizational-training-analysis-power-bi-content-pack.md)
#### [Manažer školení](../dev-itpro/analytics/practice-manager-power-bi.md)
#### [Výkonnost výroby](../dev-itpro/analytics/production-performance-power-bi.md)
#### [Analýza nákupu a výdajů](../dev-itpro/analytics/purchase-content-pack-for-power-bi.md)
#### [Nábor](../dev-itpro/analytics/recruiting-analysis-power-bi-content-pack.md)
#### [Výkonnost maloobchodního kanálu ](../dev-itpro/analytics/retail-channel-performance-dashboard-power-bi-data.md)
#### [Výkon prodeje a ziskovosti](../dev-itpro/analytics/sales-profitability-performance-content-pack.md)
#### [Platby dodavatelů](../financials/accounts-payable/Vendor-payments-workspace.md)
#### [Výkonnost skladu](../dev-itpro/analytics/warehouse-power-bi-content.md)
#### [Metriky zaměstnanců](../dev-itpro/analytics/workforce-analysis-power-bi-content-pack.md)

### Vykazování a tisk dokumentů
#### [Přehled služby vykazování dokumentů](../dev-itpro/analytics/document-reporting-services.md)
#### [Konfigurace služby SSRS pro nasazení on-premises](../dev-itpro/analytics/configure-ssrs-on-premises.md)
#### [Tisk](../dev-itpro/analytics/print-documents.md)
##### [Instalace agenta směrování dokumentu pro aktivaci zařízení síťové tiskárny](../dev-itpro/analytics/install-document-routing-agent.md)
##### [Generování dokumentů, publikování a možnosti tisku v nasazeních on-premises](../dev-itpro/analytics/printing-capabilities-on-premises.md)
##### [Spuštění agenta směrování dokumentu jako služby Windows](../dev-itpro/analytics/run-document-routing-agent-as-windows-service.md)
#### [Nápověda k zabránění vypršení časového limitu u dlouhodobých sestav](../dev-itpro/analytics/prevent-long-running-reports-timing-out.md)
#### [Seznam sestav služby SQL Server Reporting Services](../dev-itpro/analytics/SSRS-report.md)
#### [Vytvoření řešení výkaznictví](../dev-itpro/analytics/create-nextgen-reporting-solutions.md)
#### [Instalace šablon návrhu sestavy](../dev-itpro/analytics/install-modern-report-design-templates.md)
#### [Rozšíření nabídky položek sestavy ](../dev-itpro/analytics/extend-report-menu-items.md)
#### [Obchodní dokumenty s vlastním designem ](../dev-itpro/analytics/custom-designs-business-docs.md)


### [Elektronické sestavy](../dev-itpro/analytics/general-electronic-reporting.md)
#### [Správa životního cyklu konfigurace](../dev-itpro/analytics/general-electronic-reporting-manage-configuration-lifecycle.md)
#### [Vytvoření konfigurace](../dev-itpro/analytics/electronic-reporting-configuration.md)
#### [Konfigurace architektury elektronického výkaznictví](../dev-itpro/analytics/electronic-reporting-er-configure-parameters.md)
#### [Stažení konfigurací z Lifecycle Services](../dev-itpro/analytics/download-electronic-reporting-configuration-lcs.md)
#### [Import konfigurací elektronického výkaznictví](../dev-itpro/analytics/electronic-reporting-import-ger-configurations.md)
#### [Rozšíření funkcí elektronického výkaznictví](../dev-itpro/analytics/general-electronic-reporting-formulas-list-extension.md)
#### [Návrhář vzorců v elektronickém výkaznictví](../dev-itpro/analytics/general-electronic-reporting-formula-designer.md)
#### [Vložení obrázků a tvarů do obchodních dokumentů generovaných pomocí nástroje elektronického výkaznictví](../dev-itpro/analytics/electronic-reporting-embed-images-shapes.md)
#### [Úprava formátu elektronického výkaznictví opětovným použitím šablony aplikace Microsoft Excel](../dev-itpro/analytics/modify-electronic-reporting-format-reapply-excel-template.md)
#### [Místa určení](../dev-itpro/analytics/electronic-reporting-destinations.md)
#### [Nastavení elektronického výkaznictví v Power BI](../dev-itpro/analytics/general-electronic-reporting-report-configuration-get-data-powerbi.md)
#### [Generování elektronických dokumentů a aktualizace dat aplikace pomocí nástroje elektronického výkaznictví](../dev-itpro/analytics/generate-electronic-documents-update-application-data.md)
#### Úlohy elektronického výkaznictví
##### [Vytvoření poskytovatele konfigurace a jeho označení jako aktivního pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-configuration-provider-mark-it-active-2016-11.md)
##### [Definování závislosti konfigurací z jiných komponent pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-define-dependency-er-configurations-from-other-components-july-2017.md)
##### [Definování mapování modelů a výběr zdrojů dat pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-define-model-mapping-select-data-sources-2016-11.md)
##### [Návrh konfigurace pro generování sestav ve formátu Microsoft Word pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-design-configuration-word-2016-11.md)
##### [Návrh datového modelu pro určitou doménu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-design-domain-specific-data-model-2016-11.md)
##### [Návrh konfigurace pro generování sestav ve formátu OpenXML pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-design-reports-openxml-2016-11.md)
##### [Konfigurace místa určení pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-destinations-2016-11.md)
##### [Příprava datového modelu k použití souborů pro správu dokumentů ve formátech výstupu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-document-management-files-1.md)
##### [Rozšíření datového modelu k použití souborů pro správu dokumentů ve formátech výstupu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-document-management-files-2.md)
##### [Vytvoření formátu k použití souborů pro správu dokumentů ve formátech výstupu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-document-management-files-3.md)
##### [Spuštění formátu k použití souborů pro správu dokumentů ve formátech výstupu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-document-management-files-4.md)
##### [Úprava a spuštění formátu k použití souborů pro správu dokumentů ve formátech výstupu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-document-management-files-5.md)
##### [Generování elektronických dokumentů pro platby za použití konfigurace formátu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-electronic-payments.md)
##### [Návrh datového model k používání finančních dimenzí jako zdroje dat pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-financial-dimensions-data-source-1.md)
##### [Mapování modelů k používání finančních dimenzí jako zdroje dat pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-financial-dimensions-data-source-2.md)
##### [Návrh sestavy k používání finančních dimenzí jako zdroje dat pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-financial-dimensions-data-source-3.md)
##### [Spuštění sestavy používající finanční dimenze jako zdroj dat pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-financial-dimensions-data-source-4.md)
##### [Vytvoření konfigurace formátu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-format-configuration-2016-11.md)
##### [Vytvoření formátu pro počítání a sčítání pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-format-counting-summing-1.md)
##### [Konfigurace výpočtů pro provádění počítání a sčítání pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-format-counting-summing-2.md)
##### [Použití výpočtů pro provádění výstupu počítání a sčítání pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-format-counting-summing-3.md)
##### [Spuštění formátu pro provádění počítání a sčítání pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-format-counting-summing-4.md)
##### [Import konfigurací pro generování dokumentů s aktualizací dat aplikace pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-generate-documents-application-data-update-part-1-import-configurations.md)
##### [Generování dokumentů s aktualizací dat aplikace pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-generate-documents-application-data-update-part-2-generate-documents.md)
##### [Úprava modelu a mapování pro generování dokumentů s aktualizací dat aplikace pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-generate-documents-application-data-update-part-3-modify-model-mapping.md)
##### [Úprava formátu pro generování dokumentů s aktualizací dat aplikace pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-generate-documents-application-data-update-part-4-modify-format.md)
##### [Generování dokumentů s aktualizací dat aplikace pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-generate-documents-application-data-update-part-5-generate-documents-application-data-update.md)
##### [Návrh formátu pro použití vodorovně rozbalovacích oblastí k dynamickému přidání sloupců v sestavách aplikace Excel pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-horizontal-1.md)
##### [Spuštění formátu používajícího vodorovně rozbalovací oblasti k dynamickému přidání sloupců v sestavách aplikace Excel pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-horizontal-2.md)
##### [Import konfigurace ze služby Lifecycle Services pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-import-configuration-lifecycle-services.md)
##### [Provedení sestav ve formátu Microsoft Office s integrovanými obrázky pro elektronické výkaznictví (Part 1.md) ](../dev-itpro/analytics/tasks/er-make-reports-ms-office-formats-embedded-images-july-2017.md)
##### [Kontrola konfigurací pro provedení sestav ve formátu Microsoft Office s integrovanými obrázky pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-make-reports-ms-office-formats-embedded-images-part-2-review-configurations.md)
##### [Generování sestav ve formátu Microsoft Office s integrovanými obrázky pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-make-reports-ms-office-formats-embedded-images-part-3-generate-reports.md)
##### [Správa konfigurací mapování modelu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-manage-model-mapping-configurations-july-2017.md)
##### [Mapování komponent vytvořeného formátu na prvky datového modelu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-map-components-data-model-elements--2016-11.md)
##### [Mapování datového modelu na zvolené zdroje dat pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-map-data-model-selected-data-sources-2016-11.md)
##### [Úprava formátu opětovným použitím šablony aplikace Microsoft Excel pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-modify-format-by-reapplying-microsoft-excel-template-july-2017.md)
##### [Vytvoření požadovaných konfigurací pro import dat z externího souboru pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-required-configurations-import-data.md)
##### [Zvolení definice datového modelu při vytváření formátu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-select-data-model-definition-creating-er-format-july-2017.md)
##### [Upgrade formátu přijetím nové základní verze tohoto formátu pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-upgrade-format.md)
##### [Odeslání konfigurace do služby Lifecycle Services pro elektronické výkaznictví](../dev-itpro/analytics/tasks/er-upload-configuration-into-lifecycle-services.md)

### Analytické nástroje a vizualizace
#### [Analýzy, měrné systémy agregace a modelování klíčového indikátoru výkonnosti](../dev-itpro/analytics/analytics.md)
#### [Modelování a agregovaná data](../dev-itpro/analytics/model-aggregate-data.md)
#### [Přidání finančních dimenzí k měrným systémům agregace](../dev-itpro/analytics/add-financial-dimensions-aggregate-measurements.md)
#### [Přidání kontextové BI do formulářů](../dev-itpro/analytics/add-contextual-bi-forms.md)
#### [Sestava měrných systémů agregace](../dev-itpro/analytics/aggregate-measurements-report.md)
#### [Sestava entit agregovaných dat](../dev-itpro/analytics/aggregate-data-entities-report.md)
#### [Sestava klíčového indikátoru výkonnosti (KPI.md)](../dev-itpro/analytics/key-performance-indicators-report.md)
#### [Integrace služby Power BI](../dev-itpro/analytics/power-bi-integration.md)
#### [Přístup k obsahu v Power BI](../dev-itpro/analytics/power-bi-content-microsoft-partners.md)
#### [Vytvoření sestavy Power BI a řídicího panelu](../dev-itpro/analytics/create-powerbi-report-dashboard.md)
#### [Vytvoření sestavy Power BI pomocí dat aplikace Finance and Operations](../dev-itpro/analytics/create-powerbi-report-data.md)
#### [Konfigurace integrace služby Power BI pro pracovní prostory](../dev-itpro/analytics/configure-power-bi-integration.md)
#### [Integrace služby Power BI Embedded](../dev-itpro/analytics/power-bi-embedded-integration.md)
#### [Embedded Power BI v pracovních prostorech](../dev-itpro/analytics/embed-power-bi-workspaces.md)
#### [Přidání analýz do pracovního prostoru pomocí Power BI Embedded](../dev-itpro/analytics/add-analytics-tab-workspaces.md)
#### [Pomoc se zabezpečením analytických pracovních prostorů a sestav pomocí Power BI Embedded](../dev-itpro/analytics/secure-analytical-workspaces.md)
#### [Přidání Power BI do pracovních prostor](../dev-itpro/analytics/add-bi-workspaces.md)
#### [Připnutí sestav Power BI do pracovní prostor](../dev-itpro/analytics/pin-power-bi-reports.md)
#### [Integrace Power BI s úložištěm entit](../dev-itpro/analytics/power-bi-integration-entity-store.md)
#### [Export entit do vaší vlastní databáze Azure SQL](../dev-itpro/analytics/export-entities-to-your-own-database.md)
#### [Vytváření analytických sestav pomocí Power BI Desktop](../dev-itpro/analytics/author-distribute-power-bi-reports.md)
#### [Publikování analytických aplikací v Power BI](../dev-itpro/analytics/publish-apps-powerbi.md)


## Řízení zásob
### [Příchozí proces](../supply-chain/inventory/arrival-overview.md)
#### [Přehled doručení](../supply-chain/inventory/arrival-overview.md)
#### [Nastavení profilu přehledu doručení zboží (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/set-up-item-arrival-overview-profile.md)
### [Odchozí proces](../supply-chain/inventory/outbound-process.md)
#### [Rezervace skladového množství](../supply-chain/inventory/reserve-inventory-quantities.md)
#### [Rezervace stejné dávky pro prodejní objednávku](../supply-chain/sales-marketing/reserve-same-batch-sales-order.md)
### Nastavení sledování zásob
#### [Skladové deníky](../supply-chain/inventory/inventory-journals.md)
#### [Skladová místa](../supply-chain/inventory/inventory-locations.md)
#### [Stavy zásob](../supply-chain/inventory/inventory-statuses.md)
#### [Vytvoření nového rozvržení skladu (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/create-new-warehouse-layout.md)
#### [Udržování typů čárových kódů (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/maintain-barcode-types.md)
### Skladové operace
#### [Kontrola dostupnosti zásob (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/check-availability-stock.md)
#### [Inicializace úrovní zásob ve skladu (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/initialize-stock-levels-warehouse.md)
#### [Převedení fyzických zásob ve skladu (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/transfer-physical-inventory-within-warehouse.md)
#### [Úprava úrovně zásob ve skladu (basic warehousing.md) (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/adjust-stock-levels-warehouse-basic-warehousing.md)
#### [Vytvoření a správa blokování zásob (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/create-maintain-inventory-blocking.md)
### Inventura skladu
#### [Inventura zásob ve skladu (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/count-inventory-warehouse.md)
#### [Definování procesů inventur zásob (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/define-inventory-counting-processes.md)
### Zásoby dodávky
#### [Zásilka](../supply-chain/inventory/consignment.md)
#### [Nastavení zásilky](../supply-chain/inventory/set-up-consignment.md)
#### [Změna vlastnictví zásob dodávky na základě výrobní poptávky (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/change-ownership-consignment.md)
#### [Vytvoření nové zakázky na doplnění stavu zásob dodávky (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/create-consignment-replenishment-order.md)
#### [Sledování zásob dodávky s použitím dodavatelské spolupráce (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/monitor-consignment-inventory-vendor-collaboration.md)
### [Správa kvality](../supply-chain/inventory/quality-management-processes.md)
#### [Proces správy kvality](../supply-chain/inventory/quality-management-processes.md)
#### [Povolení správy kvality](../supply-chain/inventory/enable-quality-management.md)
#### [Řízení neshody](../supply-chain/inventory/enable-nonconformance-management.md)
#### [Nastavení předpokladů pro správu (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/set-up-prerequisites-nonconformance-management.md)
#### [Vytvoření a zpracování neshody (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/create-process-non-conformance.md)
#### [Karanténní příkaz](../supply-chain/inventory/quarantine-orders.md)
#### [Nastavení objednávky kvality (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/set-up-quality-orders.md)
#### [Kontrola kvality zboží (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/inspect-quality-goods.md)
#### [Blokování zásob](../supply-chain/inventory/inventory-blocking.md)
### Sledovací dimenze položky
#### [Oprava informací o sledování zásob (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/correct-inventory-tracking-information.md)
#### [Sloučení dávky skladových zásob](../supply-chain/inventory/merge-inventory-batches.md)
#### [Sledování zboží a surovin](../supply-chain/inventory/trace-items-raw-materials-inventory-production-sales.md)
#### [Registrace sériového čísla v prodejním procesu](../supply-chain/sales-marketing/register-serial-numbers-sales-process.md)
#### [Sledování položky nebo suroviny (Průvodce záznamem úloh)](../supply-chain/inventory/tasks/trace-item-or-raw-material.md)
#### [Mobilní pracovní prostor zásob na skladě](../supply-chain/inventory/inventory-on-hand-mobile-workspace.md)


## Hlavní plánování
### Vytvoření prognózy poptávky
#### [Přehled prognózy poptávky](../supply-chain/master-planning/introduction-demand-forecasting.md)
#### [Nastavení prognózy poptávky](../supply-chain/master-planning/demand-forecasting-setup.md)
#### [Import historických dat pro prognózy poptávky](../supply-chain/master-planning/import-historical-data.md)
#### [Generování statistické základní prognózy](../supply-chain/master-planning/generate-statistical-baseline-forecast.md)
#### [Ruční úpravy základní prognózy](../supply-chain/master-planning/manual-adjustments-baseline-forecast.md)
#### [Autorizace upravené prognózy](../supply-chain/master-planning/authorize-adjusted-forecast.md)
#### [Monitorování přesnosti prognózy](../supply-chain/master-planning/monitor-forecast-accuracy.md)
#### [Odebrání odlehlých hodnot z dat historických transakcí](../supply-chain/master-planning/remove-historical-outliers-calculating-demand-forecast.md)
#### [Redukční klíče](../supply-chain/master-planning/reduction-keys.md)
#### [Vytvoření základní prognózy (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/create-baseline-forecast.md)
#### [Ruční úprava prognózy poptávky (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/modify-demand-forecast-manually.md)

### Hlavní plán
#### [Zprávy akce](../supply-chain/master-planning/action-messages.md)
#### [Nastavení distponibility](../supply-chain/master-planning/coverage-settings.md)
#### [Zpoždění](../supply-chain/master-planning/delays.md)
#### [Hlavní plány](../supply-chain/master-planning/master-plans.md)
#### [Sledování běhu hlavního plánování (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/monitor-master-planning-run.md)
#### [Hlavní plány a funkce více pracovišť](../supply-chain/master-planning/master-plan-multisite-functionality.md)
##### [Plán pro disponibilitu pracoviště, sklad je povinný](../supply-chain/master-planning/master-plan-site-coverage-warehouse-mandatory.md)
##### [Plán pro disponibilitu pracoviště, sklad je není povinný](../supply-chain/master-planning/master-plan-site-coverage-warehouse-not-mandatory.md)
##### [Plán pro disponibilitu pracoviště a skladu, sklad je povinný](../supply-chain/master-planning/master-plan-site-warehouse-coverage-warehouse-mandatory.md)
##### [Plán pro disponibilitu pracoviště a skladu, sklad není povinný](../supply-chain/master-planning/master-plan-site-warehouse-coverage-warehouse-not-mandatory.md)
##### [Určení verze kusovníku](../supply-chain/master-planning/master-plan-bom-version-determined.md)
##### [Rozpad verze kusovníku](../supply-chain/master-planning/master-plan-explosion-bom-version.md)
##### [Správa plánovaných objednávek](../supply-chain/master-planning/maintain-planned-orders.md)
##### [Sledování a rozpad](../supply-chain/master-planning/trace-explosion.md)
##### [Definování pravidel disponibility pro položky (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/define-coverage-rules-items.md)
##### [Vytvoření plánu pro pracoviště (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/create-plan-site.md)
##### [Vytvoření plánu pro pracoviště (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/create-schedule-site.md)
##### [Vygenerování plánu s omezeními (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/constrained-plan.md)
##### [Vytvoření mezipodnikového plánu (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/create-intercompany-plan.md)
##### [Vytvoření materiálového plánu pro vedlejší produkty (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/create-material-plan-co-products.md)
##### [Zobrazení výstupní plánované mezipodnikové poptávky (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/view-outbound-planned-intercompany-demand.md)
##### [Spolupráce s odběrateli interního dodavatelského řetězce (Průvodce záznamem úloh)](../supply-chain/master-planning/tasks/collaborate-internal-supply-chain-customers.md)


## [Mobilní aplikace](../dev-itpro/mobile-apps/Mobile-app-home-page.md)
### [Mobilní pracovní prostory](../dev-itpro/mobile-apps/mobile-workspaces-released.md)
#### [Adresář společnosti](../dev-itpro/mobile-apps/company-directory-mobile-workspace.md)
#### [Řízení nákladů](../financials/cost-accounting/cost-controlling-mobile-workspace.md)
#### [Správa výdajů](../financials/expense-management/expense-management-mobile-workspace.md)
#### [Množství na skladě](../supply-chain/inventory/inventory-on-hand-mobile-workspace.md)
#### [Schválení faktur](../dev-itpro/mobile-apps/invoice-approval-mobile-workspace.md)
#### [Můj tým](../dev-itpro/mobile-apps/manager-self-service-mobile-workspace.md)
#### [Časový záznam projektu](../financials/project-management/project-time-entry-mobile-workspace.md)
#### [Schválení nákupní objednávky](../supply-chain/procurement/purchase-order-mobile-workspace.md)
#### [Prodejní objednávky](../supply-chain/sales-marketing/sales-orders-mobile-workspace.md)
#### [Dodavatelská spolupráce](../supply-chain/procurement/vendor-collaboration-mobile-workspace.md)
### [Publikování mobilního pracovního prostoru](../dev-itpro/mobile-apps/publish-mobile-workspace.md)


## [Integrace s Office](../dev-itpro/office-integration/office-integration.md)
### [Kurz integrace s Office](../dev-itpro/office-integration/office-integration-tutorial.md)
### [Použití doplňku Excel](../dev-itpro/office-integration/use-excel-add-in.md)
### [Vytvoření zkušeností s otevíráním v Excelu](../dev-itpro/office-integration/office-integration-edit-excel.md)
### [Přidání šablon do otevřených řádků v nabídce Excel](../dev-itpro/user-interface/add-templates-open-lines-excel-menu.md)
### [Přizpůsobení nabídky Otevřít v Microsoft Office](../dev-itpro/office-integration/customize-open-office-menu.md)
### [Konfigurace a odeslání e-mailu](organization-administration/configure-email.md)
### [Řešení problému s integrací s Office](../dev-itpro/office-integration/office-integration-troubleshooting.md)

## [Zásobování a zdroje](../supply-chain/procurement/procurement-sourcing-overview.md)
### [Workflow zásobování a zdrojů](../supply-chain/procurement/procurement-sourcing-workflows.md)
### [Zásady nákupu](../supply-chain/procurement/purchase-policies.md)
### [Zásobovací katalogy](../supply-chain/procurement/procurement-catalogs.md)
### [Nákupní požadavky](../supply-chain/procurement/purchase-requisitions-overview.md)
#### [Workflow nákupního požadavku](../supply-chain/procurement/purchase-requisitions-workflow.md) 
### [Požadavek na nabídky (RFQ)](../supply-chain/procurement/request-quotations.md)
### Nákupní objednávky
#### [Přehled nákupních objednávek](../supply-chain/procurement/purchase-order-overview.md)
#### [Vytváření nákupních objednávek](../supply-chain/procurement/purchase-order-creation.md)
#### [Schválení a potvrzení nákupních objednávek](../supply-chain/procurement/purchase-order-approval-confirmation.md)
#### [Mobilní pracovní prostor schvalování nákupních objednávek](../supply-chain/procurement/purchase-order-mobile-workspace.md)
### Ceny, slevy a rabaty
#### [Nákupní smlouvy](../supply-chain/procurement/purchase-agreements.md)
#### [Rabaty dodavatele](../supply-chain/procurement/vendor-rebates.md)
### Příjemky a fakturace produktů
#### [Příjemka produktu proti nákupním objednávkám](../supply-chain/procurement/product-receipt-against-purchase-orders.md)
#### [Přehled faktur dodavatele](../financials/accounts-payable/vendor-invoices-overview.md)
#### [Účetní profily dodavatele](../financials/accounts-payable/vendor-posting-profiles.md)
### Správa a spolupráce dodavatelů
#### [Nastavení a správa dodavatelské spolupráce](../supply-chain/procurement/set-up-maintain-vendor-collaboration.md)
#### [Správa uživatelů dodavatelské spolupráce](../supply-chain/procurement/manage-vendor-collaboration-users.md)
#### [Nastavení účtů dodavatele](../supply-chain/procurement/set-up-vendor-accounts.md)
#### [Dodavatelská spolupráce s externími dodavateli](../supply-chain/procurement/vendor-collaboration-work-external-vendors.md)
#### [Dodavatelská spolupráce se zákazníky](../supply-chain/procurement/vendor-collaboration-work-customers-dynamics-365-operations.md)
#### [Nastavení externího katalogu pro funkci PunchOut eProcurement](../supply-chain/procurement/set-up-external-catalog-for-punchout.md)
#### [Použití externích katalogů pro funkci PunchOut eProcurement](../supply-chain/procurement/use-external-catalogs-for-punchout.md)
#### [Spolupráce s dodavateli pomocí portálu pro dodavatele](../supply-chain/procurement/collaborate-vendors-vendor-portal.md)
#### [Mobilní pracovní prostor dodavatelské spolupráce](../supply-chain/procurement/vendor-collaboration-mobile-workspace.md)
#### [Zabezpečení uživatele dodavatelského portálu](../supply-chain/procurement/configure-security-vendor-portal-users.md)

## [Správa organizace](organization-administration/organization-administration-home-page.md)
### [Číselné řady](organization-administration/number-sequence-overview.md)
#### [Nastavení číselných řad na jednotlivém základě](organization-administration/tasks/set-up-number-sequences-individual-basis.md)
#### [Nastavení číselných řad pomocí průvodce](organization-administration/tasks/set-up-number-sequences-wizard.md)
### [Organizace a organizační hierarchie](organization-administration/organizations-organizational-hierarchies.md)
#### [Plánování organizační hierarchie](organization-administration/plan-organizational-hierarchy.md)
#### [Vytvoření organizační hierarchie](organization-administration/tasks/create-organization-hierarchy.md)
#### [Vytvoření právnické osoby](organization-administration/tasks/create-legal-entity.md)
#### [Vytvoření provozní jednotky](organization-administration/tasks/create-operating-unit.md)
### [Globální adresář](organization-administration/overview-global-address-book.md)
#### [Plánování konfigurace globálního adresáře](organization-administration/plan-configuration-global-address-book-additional-address-books.md)
#### [Konfigurace globálního adresáře](organization-administration/tasks/configure-global-address-book.md)
#### [Často kladené dotazy týkající se adresářů](organization-administration/qa-address-books.md)


### [Workflow](organization-administration/overview-workflow-system.md)
#### [Architektura systému workflowu](organization-administration/workflow-system-architecture.md)
#### [Prvky workflowu](organization-administration/workflow-elements.md)
#### [Akce workflowu](organization-administration/workflow-actions.md)
#### [Vytvoření workflow](organization-administration/create-workflow.md)
##### [Konfigurace vlastností workflow](organization-administration/configure-workflow-properties.md)
##### [Konfigurace ručního úkolu](organization-administration/configure-manual-task-workflow.md)
##### [Konfigurace automatizovaného úkolu](organization-administration/configure-automated-task-workflow.md)
##### [Konfigurace procesu schválení](organization-administration/configure-approval-process-workflow.md)
##### [Konfigurace kroku schválení](organization-administration/configure-approval-step-workflow.md)
##### [Konfigurace ručního rozhodnutí](organization-administration/configure-manual-decision-workflow.md)
##### [Konfigurace podmíněného rozhodnutí](organization-administration/configure-conditional-decision-workflow.md)
##### [Konfigurace paralelní aktivity](organization-administration/configure-parallel-activity-workflow.md)
##### [Konfigurace paralelní větve](organization-administration/configure-parallel-branch-workflow.md)
##### [Konfigurace workflow položky řádku](organization-administration/configure-line-item-workflow.md)
#### [Konfigurace dávkové úlohy zpracování zprávy](organization-administration/workflow-batch-job-critical.md)
#### [Delegování položek práce ve workflow](organization-administration/tasks/delegate-work-items-workflow.md)
#### [Zobrazit historii workflowu](organization-administration/tasks/view-workflow-history.md)
#### [Povolení pro uživatele přijímat e-mailové zprávy související s workflow](organization-administration/tasks/users-receive-workflow-related-email-messages.md)
#### [Sestava typů workflow](organization-administration/workflow-types-report.md)

### [Elektronické podpisy](organization-administration/electronic-signature-overview.md)
#### [Nastavení elektronických podpisů](organization-administration/tasks/set-up-electronic-signatures.md)
### [Správa případů](organization-administration/cases.md)
#### [Konfigurace bezpečnosti případu, procesů a kategorií](organization-administration/plan-case-management.md)
### [Šablony záznamu](organization-administration/record-templates.md)
#### [Vytvoření šablony záznamů](../dev-itpro/data-entities/tasks/create-record-template-facilitate-data-entry.md)
#### [Vytvoření nového záznamu s použitím šablony záznamu](../dev-itpro/data-entities/tasks/use-record-template-new-record.md)
### [Konfigurace správy dokumentů](organization-administration/configure-document-management.md)
### [Konfigurace a odeslání e-mailu](organization-administration/configure-email.md)
### [Údaje data a času a časové pásmo](organization-administration/date-time-zones.md)

### Úkoly zásobování a zdrojů
#### [Schválení dodavatelů pro konkrétní kategorie zásobování](../supply-chain/procurement/tasks/approve-vendors-specific-procurement-categories.md)
#### [Schválení dodavatelů pro konkrétní produkty](../supply-chain/procurement/tasks/approve-vendors-specific-products.md)
#### [Vytvoření zásobovacího katalogu](../supply-chain/procurement/tasks/create-procurement-catalog.md)
#### [Vytvoření nákupní objednávky projektu](../supply-chain/procurement/tasks/create-project-purchase-order.md)
#### [Vytvoření nákupní smlouvy](../supply-chain/procurement/tasks/create-purchase-agreement.md)
#### [Vytvoření nákupní objednávky](../supply-chain/procurement/tasks/create-purchase-order.md)
#### [Vytvoření nákupní objednávky s plánem dodávek](../supply-chain/procurement/tasks/create-purchase-order-delivery-schedule.md)
#### [Vytvoření nákupní objednávky pro jednorázového dodavatele](../supply-chain/procurement/tasks/create-purchase-order-one-time-supplier.md)
#### [Vytvoření dílčí nákupní objednávky při vytváření nákupní objednávky](../supply-chain/procurement/tasks/create-purchase-release-order-creating-purchase-order.md)
#### [Vytvoření dílčí nákupní objednávky z nákupní smlouvy](../supply-chain/procurement/tasks/create-purchase-release-order-purchase-agreement.md)
#### [Vytvoření nákupní vratky](../supply-chain/procurement/tasks/create-purchase-return-order.md)
#### [Vytvoření zásad nákupu](../supply-chain/procurement/tasks/create-purchasing-policies.md)
#### [Vytvoření opakované nákupní objednávky](../supply-chain/procurement/tasks/create-repeat-purchase-order.md)
#### [Vytvoření požadavku na nabídku](../supply-chain/procurement/tasks/create-request-quotation.md)
#### [Vytvoření žádanky pro spotřebu](../supply-chain/procurement/tasks/create-requisition-consumption.md)
#### [Vytvoření žádanky používající požadavek na nabídku](../supply-chain/procurement/tasks/create-requisition-uses-rfq.md)
#### [Vytvoření metody hodnocení pro požadavky na nabídku](../supply-chain/procurement/tasks/create-scoring-method-rfqs.md)
#### [Vytvoření typů oslovení a kritérií hodnocení pro požadavky na nabídku](../supply-chain/procurement/tasks/create-solicitation-types-scoring-criteria-rfqs.md)
#### [Vytvoření účtu dodavatele](../supply-chain/procurement/tasks/create-vendor-account.md)
#### [Vytvoření bankovního účtu dodavatele](../supply-chain/procurement/tasks/create-vendor-bank-account.md)
#### [Zadání a porovnání nabídek pro požadavek na nabídku a přidělení smlouvy](../supply-chain/procurement/tasks/enter-compare-rfq-bids-award-contracts.md)
#### [Zaznamenání příjmu zboží na nákupní objednávce](../supply-chain/procurement/tasks/record-receipt-goods-purchase-order.md)
#### [Přiřazení a přepsání DPH](../supply-chain/procurement/tasks/sales-tax-assignment-overrides.md)
#### [Vyhledání dodavatelů](../supply-chain/procurement/tasks/search-vendors.md)
#### [Nastavení oprávnění pro objednávání produktů jménem jiného uživatele](../supply-chain/procurement/tasks/set-up-permissions-ordering-products.md)
#### [Nastavení zásad pro hierarchie kategorií zásobování](../supply-chain/procurement/tasks/set-up-policies-procurement-category-hierarchies.md)
#### [Nastavení hierarchie kategorií zásobování](../supply-chain/procurement/tasks/set-up-procurement-category-hierarchy.md)
#### [Nastavení informací o prostředku projektu](../supply-chain/procurement/tasks/set-up-project-resource-information.md)
#### [Odesílání a schvalování rozpočtů projektů](../supply-chain/procurement/tasks/submit-approve-project-budget.md)
#### [Odesílání a schvalování revizí projektových rozpočtů](../supply-chain/procurement/tasks/submit-approve-project-budget-revision.md)

## Výroba
### [Přehled výrobního procesu](../supply-chain/production-control/production-process-overview.md)
### [Plánování kombinovaného režimu: kombinování diskrétního, procesního a štíhlého zajištění zdrojů](../supply-chain/production-control/mixed-mode-plan.md)
### [Správa subdodavatelské práce ve výrobě](../supply-chain/production-control/manage-subcontract-work-production.md)
### Diskrétní výroba
#### Správa hlavních dat výroby
##### [Požadavky na nastavení výroby](../supply-chain/production-control/production-set-up-requirements.md)
##### [Kusovníky a receptury](../supply-chain/production-control/bill-of-material-bom.md)
##### [Funkce návrháře kusovníku](../supply-chain/production-control/bom-designer-functionality.md)
##### [Výpočty kusovníku](../supply-chain/cost-management/bom-calculations.md)
##### [Parametry výroby v modulu Provádění výroby](../supply-chain/production-control/production-parameters-manufacturing-execution.md)
##### [Provozní prostředky](../supply-chain/production-control/operations-resources.md)
##### [Schopnosti zdroje](../supply-chain/production-control/resource-capabilities.md)
##### [Postupy a operace](../supply-chain/production-control/routes-operations.md)
##### [Zásady práce ve skladu](../supply-chain/warehousing/warehouse-work-policies.md)
#### Plánování výrobních operací
##### [Plánování operací](../supply-chain/production-control/operations-scheduling.md)
##### [Plánování úlohy](../supply-chain/production-control/job-scheduling.md)
##### [Ganttův diagram pro plánování úloh](../supply-chain/production-control/visual-scheduling-production.md)
##### [Možnosti plánování v aplikaci Operations](../supply-chain/production-control/operation-scheduling-options.md)
#### Provozování výrobních úloh
##### [Vytvoření výrobních zakázek](../supply-chain/production-control/create-production-orders.md)
##### [Uvolnění výrobních zakázek](../supply-chain/production-control/release-production-orders.md)
##### [Vrácení stavu výrobní zakázky](../supply-chain/production-control/reverse-production-order-status.md)
##### [Vykázání výrobních zakázek jako dokončených](../supply-chain/production-control/report-production-orders-as-finished.md)
##### [Vykázání kusovníku jako dokončeného](../supply-chain/production-control/report-boms-as-finished.md)
##### [Nahrazení materiálů ve výrobě](../supply-chain/production-control/substitute-items-bom-lines.md)
##### [Registrace pro realizaci výroby](../supply-chain/production-control/registration-manufacturing-execution.md)
##### [Zpětná vazba z výroby](../supply-chain/production-control/production-feedback.md)
##### [Registrace času a docházky](../supply-chain/production-control/time-attendance-registrations.md)
#### Správa financí ve výrobě
##### [Přidělení času úlohám v sadě úloh](../supply-chain/production-control/allocate-time-jobs-job-bundle.md)
##### [Výpočet spotřeby materiálu](../supply-chain/production-control/consumption.md)
##### [Zaúčtování výroby](../supply-chain/cost-management/production-posting.md)
#### Správa surovin
##### [Cross docking z výrobních příkazů na výstupní překladiště](../supply-chain/production-control/cross-docking-opportunities.md)
##### [Registrace spotřeby materiálu pomocí mobilního zařízení](../supply-chain/production-control/register-material-consumption-hand-held-device.md)
##### [Kontrola spotřeby surovin pomocí principů vyprazdňování](../supply-chain/production-control/flushing-principles.md) 
#### Správa dokončeného zboží
##### [Nastavení výstupního místa výroby pro dokončené zboží](../supply-chain/production-control/production-output-location.md)
### Lean manufacturing 
#### [Lean manufacturing – přehled](../supply-chain/production-control/lean-manufacturing-overview.md)
#### [Subdodávky založené na aktivitě](../supply-chain/production-control/activity-based-subcontracting.md)
#### Správa hlavních dat výroby
##### [Modelování organizace úsporné výroby](../supply-chain/production-control/lean-manufacturing-modeling-lean-organization.md)
##### [Provozní prostředky](../supply-chain/production-control/operations-resources.md)
#### Plánování kanbanových úloh
##### [Vizuální plánování pro lean manufacturing](../supply-chain/production-control/visual-scheduling-lean-manufacturing.md)
##### [Plánování kanbanové úlohy pro lean manufacturing](../supply-chain/production-control/lean-manufacturing-kanban-job-scheduling.md)
##### [Plánování úlohy](../supply-chain/production-control/job-scheduling.md)
#### Provoz kanbanových úloh
##### [Podpora desky převodů kanbanu pro čtečky čárového kódu](../supply-chain/production-control/kanban-transfer-board-support-barcode-scanner.md)
##### [Zpětná vazba z výroby](../supply-chain/production-control/production-feedback.md)
### Zpracování výroby
#### Správa hlavních dat výroby
##### [Kusovníky a receptury](../supply-chain/production-control/bill-of-material-bom.md)
##### [Receptury a verze receptury](../supply-chain/production-control/formulas-versions.md)
##### [Provozní prostředky](../supply-chain/production-control/operations-resources.md)
##### [Schopnosti zdroje](../supply-chain/production-control/resource-capabilities.md)
##### [Atributy dávky](../supply-chain/production-control/batch-attributes.md)
##### [Parametry výroby v modulu Provádění výroby](../supply-chain/production-control/production-parameters-manufacturing-execution.md)
##### [Požadavky na nastavení výroby](../supply-chain/production-control/production-set-up-requirements.md)
#### Plánování výrobních operací
##### [Plánování operací](../supply-chain/production-control/operations-scheduling.md)
##### [Možnosti plánování v aplikaci Operations](../supply-chain/production-control/operation-scheduling-options.md)
##### [Plánování úlohy](../supply-chain/production-control/job-scheduling.md)
#### Provozování výrobních úloh
##### [Konsolidované dávkové objednávky](../supply-chain/production-control/consolidated-batch-orders.md)
##### [Nahrazení materiálů ve výrobě](../supply-chain/production-control/substitute-items-bom-lines.md)
##### [Registrace pro realizaci výroby](../supply-chain/production-control/registration-manufacturing-execution.md)
##### [Zpětná vazba z výroby](../supply-chain/production-control/production-feedback.md)
##### [Registrace času a docházky](../supply-chain/production-control/time-attendance-registrations.md) 
#### Správa financí ve výrobě
##### [Přidělení času úlohám v sadě úloh](../supply-chain/production-control/allocate-time-jobs-job-bundle.md)
##### [Výpočet spotřeby materiálu](../supply-chain/production-control/consumption.md)
##### [Zaúčtování výroby](../supply-chain/cost-management/production-posting.md)

#### Úkoly výroby
##### [Přidání zásad výpočtu kanbanového množství ke kanbanovému pravidlu](../supply-chain/production-control/tasks/add-kanban-quantity-calculation-policy-kanban-rule.md)
##### [Výpočet návrhů kanbanového množství](../supply-chain/production-control/tasks/calculate-kanban-quantity-suggestions.md)
##### [Změna kanbanových pravidel pro úlohu procesu](../supply-chain/production-control/tasks/change-kanban-rules-process-job.md)
##### [Vytvoření kanbanového pravidla události řádku kusovníku](../supply-chain/production-control/tasks/create-bom-line-event-kanban-rule.md)
##### [Vytvoření kanbanového pravidla pro pevné množství pro výrobu](../supply-chain/production-control/tasks/create-fixed-quantity-kanban-rule-manufacturing.md)
##### [Vytvoření kanbanového pravidla s použitím události řádku kanbanu](../supply-chain/production-control/tasks/create-kanban-rule-kanban-line-event.md)
##### [Vytvoření kanbanového pravidla s použitím události pro minimální úroveň zásob](../supply-chain/production-control/tasks/create-kanban-rule-minimum-stock-event.md)
##### [Vytvoření kanbanového pravidla pro více aktivit](../supply-chain/production-control/tasks/create-kanban-rule-multiple-activities.md)
##### [Vytvoření nového kanbanového pravidla duplikováním existujícího kanbanového pravidla](../supply-chain/production-control/tasks/create-new-kanban-rule-duplicating-existing-kanban-rule.md)
##### [Vytvoření nahrazujícího kanbanového pravidla](../supply-chain/production-control/tasks/create-replacement-kanban-rule.md)
##### [Vytvoření kanbanového pravidla události prodeje](../supply-chain/production-control/tasks/create-sales-event-kanban-rule.md)
##### [Vytvoření nového kanbanového pravidla odběru](../supply-chain/production-control/tasks/create-withdrawal-kanban-rule.md)
##### [Provádění úloh kanbanových procesů](../supply-chain/production-control/tasks/execute-kanban-process-jobs.md)
##### [Přesunutí naplánovaných kanbanových úloh](../supply-chain/production-control/tasks/move-scheduled-kanban-jobs.md)
##### [Příprava kanbanové úlohy procesu, když pro pracovní buňku jsou k dispozici materiály](../supply-chain/production-control/tasks/prepare-kanban-job-materials.md)
##### [Příprava kanbanové úlohy procesu, když pro pracovní buňku nejsou k dispozici materiály](../supply-chain/production-control/tasks/prepare-process-kanban-job-materials-not-available.md)
##### [Odebrání kanbanové úlohy z plánu](../supply-chain/production-control/tasks/remove-kanban-job-schedule.md)
##### [Vrácení stavu kanbanové úlohy](../supply-chain/production-control/tasks/revert-kanban-job-status.md)
##### [Plánování kanbanových úloh](../supply-chain/production-control/tasks/schedule-kanban-jobs.md)
##### [Převádění materiálů s kanbanovými úlohami (jen ve verzi z února 2016)](../supply-chain/production-control/tasks/transfer-materials-kanban-jobs.md)
##### [Aktualizace stavu kanbanu](../supply-chain/production-control/tasks/update-kanban-status.md)

##### [Vytváření aktivit procesu pro lean manufacturing](../supply-chain/production-control/tasks/create-process-activities-lean-manufacturing.md)
##### [Vytváření aktivit převodu pro lean manufacturing](../supply-chain/production-control/tasks/create-transfer-activities-lean-manufacturing.md)
##### [Definování pracovních buněk pro lean manufacturing](../supply-chain/production-control/tasks/define-lean-manufacturing-work-cells.md)
##### [Definování skupin štíhlého plánu](../supply-chain/production-control/tasks/define-lean-schedule-groups.md)
##### [Doložení z prodejních objednávek štíhlé výroby](../supply-chain/production-control/tasks/lean-pegging-sales-orders.md)
##### [Vytvoření subdodavatelské pracovní buňky pro lean manufacturing](../supply-chain/production-control/tasks/create-subcontracted-work-cell-lean-manufacturing.md)


##### [Definování skupin prostředků diskrétní výroby](../supply-chain/production-control/tasks/define-discrete-manufacturing-resource-group.md)

##### [Aktivace verze výrobního toku](../supply-chain/production-control/tasks/activate-production-flow-version.md)
##### [Přidání existující aktivity k verzi výrobního toku](../supply-chain/production-control/tasks/add-existing-activity-production-flow-version.md)
##### [Přidání předchůdce k aktivitě výrobního toku](../supply-chain/production-control/tasks/add-predecessor-production-flow-activity.md)
##### [Vytvoření verze výrobního toku](../supply-chain/production-control/tasks/create-production-flow-version.md)
##### [Deaktivace verze výrobního toku](../supply-chain/production-control/tasks/deactivate-production-flow-version.md)
##### [Definování data vypršení platnosti verze výrobního toku](../supply-chain/production-control/tasks/define-expiry-date-production-flow-version.md)
##### [Definování modelů výrobních toků](../supply-chain/production-control/tasks/define-production-flow-models.md)
##### [Ověření výrobního toku a verze](../supply-chain/production-control/tasks/validate-production-flow-version.md)

##### [Vytváření vlastností plánování pracovní doby](../supply-chain/production-control/tasks/create-working-time-scheduling-properties.md)
##### [Vytváření šablon pracovní doby](../supply-chain/production-control/tasks/create-working-time-templates.md)

##### [Vytvoření výrobní zakázky](../supply-chain/production-control/tasks/create-production-order.md)
##### [Ukončení výrobní zakázky](../supply-chain/production-control/tasks/end-production-order.md)
##### [Odhad výrobní zakázky](../supply-chain/production-control/tasks/estimate-production-order.md)
##### [Uvolnění výrobní zakázky](../supply-chain/production-control/tasks/release-production-order.md)
##### [Ohlášení výrobní zakázky jako dokončené](../supply-chain/production-control/tasks/report-production-order-finished.md)
##### [Naplánování výrobní zakázky](../supply-chain/production-control/tasks/schedule-production-order.md)
##### [Naplánování výrobní zakázky s naplánováním operací a úloh](../supply-chain/production-control/tasks/schedule-production-order-operations-job-scheduling.md)
##### [Spuštění výrobní zakázky](../supply-chain/production-control/tasks/start-production-order.md)

##### [Zkopírování vedlejších produktů z existující verze receptury](../supply-chain/production-control/tasks/copy-co-products-existing-formula-version.md)
##### [Zkopírování receptury](../supply-chain/production-control/tasks/copy-formula.md)
##### [Vytvoření receptury kopírováním z existující receptury](../supply-chain/production-control/tasks/create-formula-copying-existing-formula.md)

##### [Životní cyklus dávkové objednávky od vytvoření po spuštění](../supply-chain/production-control/tasks/batch-order-lifecycle-start.md)
##### [Klasifikace produktu s použitím hierarchií kategorií](../supply-chain/pim/tasks/classify-product-category-hierarchies.md)
##### [Vytvoření relace aktivity: následník](../supply-chain/production-control/tasks/create-activity-relation-successor.md)
##### [Vytvoření receptury kopírováním z existující receptury](../supply-chain/production-control/tasks/create-formula-copying-existing-formula.md)
##### [Vytvoření provozního prostředku](../supply-chain/production-control/tasks/create-operations-resource.md)
##### [Vytvoření hodnotového proudu](../supply-chain/production-control/tasks/create-value-stream.md)
##### [Definování schopností prostředku](../supply-chain/production-control/tasks/define-resource-capabilities.md)
##### [Povolení procesu mezd pro čas a docházku](../supply-chain/production-control/tasks/payroll-process-time-attendance.md)
##### [Ohlášení jako dokončené pro skladové místo bez řízení na základě registračních značek ](../supply-chain/production-control/tasks/report-finished-non-license-plate-controlled-location-2016-11.md)
##### [Hlášení průběhu na mobilním pracovním zařízení](../supply-chain/production-control/tasks/report-progress-mobile-job-device.md)
##### [Seřazení výrobních úloh pro procesní výrobu](../supply-chain/production-control/tasks/sequence-production-jobs-process-manufacturing.md)
##### [Použití deníku pojistných zásob pro aktualizaci minimální disponibility](../supply-chain/production-control/tasks/use-safety-stock-journal-update-minimum-coverage.md)
##### [Konfigurace pracovníka s použitím mobilního pracovního zařízení](../supply-chain/production-control/tasks/worker-mobile-job-device.md)
##### [Správa měrných jednotek (Průvodce záznamem úloh)](../supply-chain/pim/tasks/manage-unit-measure.md)


## [Řízení informací o produktech](../supply-chain/pim/product-information.md)
### Informace o produktu
#### Kategorie produktu
##### [Vytvoření hierarchie klasifikace produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-hierarchy-product-classification.md)
##### [Klasifikace produktu s použitím hierarchií kategorií (Průvodce záznamem úloh)](../supply-chain/pim/tasks/classify-product-category-hierarchies.md)
#### Definice produktu
##### [Názvosloví čísla produktu](../supply-chain/pim/product-variant-identification-nomenclature.md)
##### [Vytvoření názvosloví čísel produktů pro předdefinované varianty produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-product-number-nomenclature-predefined-variants-2016-11.md)
##### [Vytvoření názvosloví čísel produktů pro konfigurované varianty produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-product-number-nomenclature-product-variants_2016_11.md)
##### [Vytvoření nového produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-new-product.md)
##### [Vytvoření základního produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-product-master.md)
##### [Vytváření předdefinovaných variant produktů (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-predefined-product-variants.md)
##### [Vytvoření uvolněného produktu pro jednu společnost (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-released-product-single-company.md)
##### [Vytvoření čárového kódu pro produkt (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-bar-code-product.md)
##### [Zadání kódu GTIN pro produkt (Průvodce záznamem úloh)](../supply-chain/pim/tasks/enter-gtin-code-product.md)
##### [Vytvoření atributů dávky pro produkt (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-batch-attributes-product.md)
##### [Přidání obrázku k produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/add-image-product.md)
##### [Nejčastější otázky o překladech produktových informací](../supply-chain/pim/translations-product-related-information.md)
#### Vyhledávání produktu 
##### [Vyhledávání produktu během zadání objednávky](../supply-chain/pim/search-products-product-variants.md)
#### Příprava produktu pro plánované objednávky 
##### [Výchozí nastavení objednávky pro dimenze a varianty produktu](../supply-chain/production-control/default-order-settings.md)
#### Příprava produktu pro výrobu
##### [Kusovníky a receptury](../supply-chain/production-control/bill-of-material-bom.md)
##### [Použití návrháře kusovníků](../supply-chain/production-control/bom-designer-functionality.md)
##### [Použití návrháře receptur](../supply-chain/production-control/formula-designer.md)
##### [Přidávání variant produktů do nákupních objednávek s použitím různých hmotností (Průvodce záznamem úloh)](../supply-chain/pim/tasks/add-variant-products-purchase-order-variant-weights.md)
##### [Vytvoření prodejní objednávky pro konfigurovatelný produkt (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-sales-order-configurable-product.md)
##### [Vytvoření kritérií pro výběr prodejní ceny (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-sales-price-selection-criteria.md)
##### [Nastavení ocenění podle atributů pro konfigurovatelné produkty (Průvodce záznamem úloh)](../supply-chain/pim/tasks/set-up-attribute-based-pricing-configurable-products.md)
#### Odhad nákladů pro produkt
##### [Výpočty kusovníku](../supply-chain/cost-management/bom-calculations.md)
### Modely konfigurace produktu
#### [Vytvoření modelu konfigurace produktu](../supply-chain/pim/build-product-configuration-model.md)
#### [Vytváření konfigurace založené na dimenzích](../supply-chain/pim/dimension-based-product-configuration.md) 
##### [Dimenze produktu](../supply-chain/pim/product-dimensions.md)
##### [Konfigurace založená na dimenzích](../supply-chain/pim/dimension-based-product-configuration.md)
##### [Vytvoření základního produktu založeného na dimenzích (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-dimension-based-product-master.md)
##### [Uvolnění základního produktu založeného na dimenzích (Průvodce záznamem úloh)](../supply-chain/pim/tasks/release-dimension-based-product-master.md)
##### [Základní nastavení uvolněného základního produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/complete-basic-setup-released-product-master.md)
##### [Definování konfiguračních skupin (Průvodce záznamem úloh)](../supply-chain/pim/tasks/define-configuration-groups.md)
##### [Vytvoření kusovníku pro základní produkt založený na dimenzích (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-bill-materials-dimension-based-product-master.md)
##### [Definování konfiguračních cest (Průvodce záznamem úloh)](../supply-chain/pim/tasks/define-configuration-route.md)
##### [Vytvoření konfiguračních pravidel (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-configuration-rules.md)
##### [Vytváření konfigurací založených na dimenzích (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-dimension-based-configurations.md)
#### Vytvoření konfigurace založené na omezeních
##### [Nejčastější dotazy týkající se výpočtu pro model konfigurace produktu](../supply-chain/pim/calculate-product-configuration-models.md)
##### [Konfigurační pravidlo](../supply-chain/pim/configuration-rules.md)
##### [Omezení výrazu a omezení tabulky](../supply-chain/pim/expression-constraints-table-constraints-product-configuration-models.md)
##### [Opětovné použití konfigurace produktu](../supply-chain/pim/reuse-product-configurations.md)
##### [Nastavení modelu konfigurace](../supply-chain/pim/set-up-maintain-product-configuration-model.md)
##### [Omezení systémem definované a uživatelem definované tabulky](../supply-chain/pim/system-defined-user-defined-table-constraints.md)
##### [Vytváření komponent modelu produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-product-model-components.md)
##### [Vytvořit nový model konfigurace produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/create-product-configuration-model.md)
##### [Přidání omezení výrazu do modelu konfigurace produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/add-expression-constraint-product-configuration-model.md)
##### [Přidání výpočtu do modelu konfigurace produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/add-calculation-product-configuration-model.md)
##### [Udržování kusovníku pro model konfigurace produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/maintain-bom-product-configuration-model.md)
##### [Udržování cesty pro model produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/maintain-route-product-model.md)
##### [Schválení modelu konfigurace produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/approve-product-configuration-model.md)
##### [Převedení dat pro modely produktu (Průvodce záznamem úloh)](../supply-chain/pim/tasks/transfer-data-product-models.md)


## [Řízení projektů a účetnictví](../financials/project-management/overview-project-management-accounting.md)
### [Mezipodnikové fakturování](../financials/project-management/intercompany-invoicing.md)
### [Konfigurace fakturování mezipodnikového projektu](../financials/project-management/tasks/configure-intercompany-project-invoicing.md)
### [Konfigurace standardních nákladů pro práci a výdaje](../financials/project-management/tasks/configure-standard-costs-labor-expenses.md)
### [Příjemky časově rozlišeného zásobování](../financials/accounts-payable/project-cost-accrual-purchase-receipts.md)
### [Fakturace projektu](../financials/accounts-payable/project-invoicing.md)
### [Projektové smlouvy](../financials/project-management/project-contracts.md)
### [Prognózy projektu a rozpočty](../financials/project-management/project-forecasts-budgets.md)
### [Nákupní objednávky pro projekt](../financials/project-management/project-purchase-orders.md)
### [Nabídky projektu](../financials/project-management/project-quotations.md)
### [Prostředky projektu](../financials/project-management/project-resourcing.md)
### [Přijetí položek na nákupní objednávce z požadavku na položku](../financials/project-management/tasks/receive-items-purchase-order-item-requirement.md)
### [Strukturované rozpisy prací](../financials/project-management/work-breakdown-structures.md)
### [Mobilní pracovní prostor zadání času projektu](../financials/project-management/project-time-entry-mobile-workspace.md)

## [Prodej a marketing](../supply-chain/sales-marketing/overview-sales-marketing.md)
### Prodejní objednávky
#### [Příslib objednávky](../supply-chain/sales-marketing/delivery-dates-available-promise-calculations.md)
#### [Alternativní dodání](../supply-chain/sales-marketing/delivery-alternatives.md)
#### [Konečné termíny zadání objednávek](../supply-chain/sales-marketing/order-entry-deadlines.md)
#### [Přímé dodávky](../supply-chain/sales-marketing/direct-deliveries.md)
#### [Plány dodávek](../supply-chain/sales-marketing/delivery-schedules.md)
#### [Sériová čísla v prodejním procesu](../supply-chain/sales-marketing/register-serial-numbers-sales-process.md)
#### [Rezervace ze stejné dávky](../supply-chain/sales-marketing/reserve-same-batch-sales-order.md)
#### [Mobilní pracovní prostor prodejních objednávek](../supply-chain/sales-marketing/sales-orders-mobile-workspace.md)
### [Prodejní smlouvy](../supply-chain/sales-marketing/sales-agreements.md)
### Prodejní nabídky
#### [Simulace ceny](../supply-chain/sales-marketing/price-simulation.md)
### Vratky
#### [Prodejní vratky](../supply-chain/warehousing/sales-returns.md)

### Úkoly prodeje a marketingu
#### [Potvrzení prodejních objednávek](../supply-chain/sales-marketing/tasks/confirm-sales-orders.md)
#### [Vytvoření plánu dodávek](../supply-chain/sales-marketing/tasks/create-delivery-schedule.md)
#### [Vytváření a úpravy prodejních nabídek](../supply-chain/sales-marketing/tasks/create-edit-sales-quotations.md)
#### [Vytvoření nové obchodní smlouvy](../supply-chain/sales-marketing/tasks/create-new-trade-agreement.md)
#### [Vytvoření nákupní objednávky z prodejní objednávky](../supply-chain/sales-marketing/tasks/create-purchase-order-sales-order.md)
#### [Vytváření prodejních objednávek](../supply-chain/sales-marketing/tasks/create-sales-orders.md)
#### [Zadávání prodejních smluv](../supply-chain/sales-marketing/tasks/enter-sales-agreements.md)
#### [Plnění prodejních smluv](../supply-chain/sales-marketing/tasks/fulfill-sales-agreements.md)
#### [Vyhledání platných cen a slev](../supply-chain/sales-marketing/tasks/look-up-applicable-prices-discounts.md)
#### [Správa blokování objednávek](../supply-chain/sales-marketing/tasks/manage-order-holds.md)
#### [Hromadné vytváření prodejních nabídek](../supply-chain/sales-marketing/tasks/mass-sales-quotations.md)
#### [Generování a zpracování rabatů odběratelů](../supply-chain/sales-marketing/tasks/process-customer-rebates.md)
#### [Zpracování rabatu pro platbu](../supply-chain/sales-marketing/tasks/process-rebates-payment.md)
#### [Registrace prodejních provizí](../supply-chain/sales-marketing/tasks/register-sales-commissions.md)
#### [Nastavení pravidel prodejních provizí](../supply-chain/sales-marketing/tasks/set-up-sales-commission-rules.md)
#### [Expedování objednávek jako přímých dodávek](../supply-chain/sales-marketing/tasks/ship-orders-direct-deliveries.md)
#### [Expedování prodejních objednávek bez skladování](../supply-chain/sales-marketing/tasks/ship-sales-orders-without-warehousing.md)

## [Správa přepravy](../supply-chain/transportation/transportation-management-overview.md)
### [Moduly správy přepravy](../supply-chain/transportation/transportation-management-engines.md)
### [Vytvoření nového modulu správy přepravy](../supply-chain/transportation/create-new-transportation-management-engine.md)
### [Přidružení indexu paliva k dopravci jako nákladů na dodatečné služby](../supply-chain/transportation/tasks/associate-fuel-index-carrier-accessorial-charge.md)
### [Vytvoření přepravního dokladu](../supply-chain/transportation/create-bill-of-lading.md)
### [Plánování tras přepravy s dopravným](../supply-chain/transportation/plan-freight-transportation-routes-multiple-stops.md)
### [Plánování nákladů pomocí konsolidace centra](../supply-chain/transportation/plan-loads-hub-consolidation.md)
### [Zahrnutí hmotnosti kontejneru a objemu při naložení](../supply-chain/transportation/include-container-weight-volume-on-load.md) 
### [Ruční odsouhlasení dopravného](../supply-chain/transportation/tasks/reconcile-freight-manually.md)
### [Odsouhlasení dopravného](../supply-chain/transportation/reconcile-freight-transportation-management.md)
### [Registrace přihlášení a odhlášení řidiče k události](../supply-chain/transportation/tasks/register-driver-check-check-out-appointment.md)
### [Nastavení indexu paliva dopravce](../supply-chain/transportation/tasks/set-up-carrier-fuel-index.md)
### [Nastavení úhrady přepravy](../supply-chain/transportation/tasks/set-up-transportation-tender.md)
### [Nastavení přiřazení dodatečných služeb](../supply-chain/transportation/tasks/set-up-accessorial-assignments.md)
### [Naplánování události pro vytížení](../supply-chain/transportation/tasks/set-up-appointment-load.md)
### [Nastavení automatického odsouhlasení dopravného](../supply-chain/transportation/tasks/set-up-automatic-freight-reconciliation.md)
### [Nastavení nákladů na dodatečnou službu centra a hlavních dodatečných služeb](../supply-chain/transportation/tasks/set-up-hub-accessorial-charges-accessorial-masters.md)
### [Nastavení hlavních sazeb](../supply-chain/transportation/tasks/set-up-rate-masters.md)
### [Nastavení dopravců](../supply-chain/transportation/tasks/set-up-shipping-carriers.md)
### [Nastavení omezení přepravy pro položku](../supply-chain/transportation/tasks/set-up-transportation-constraints-item.md)

## [Řízení skladu](../supply-chain/warehousing/warehouse-management-overview.md)
### [Konfigurace skladu](../supply-chain/warehousing/warehouse-configuration.md) 
#### [Řízení práce pomocí šablon práce a směrnice pro místa](../supply-chain/warehousing/control-warehouse-location-directives.md)
#### [Měrná jednotka a zásady uskladnění](../supply-chain/warehousing/unit-measure-stocking-policies.md)
#### [Obsah skladu v Power BI](../dev-itpro/analytics/warehouse-power-bi-content.md)
#### [Systémové seskupení na otevřeném seznamu úkolů](../supply-chain/warehousing/system-group-on-open-work-list.md)
#### [Mobilní podpora čárového kódu](../supply-chain/warehousing/mobile-barcode-support.md)
### [Zásady práce ve skladu](../supply-chain/warehousing/warehouse-work-policies.md)
#### [Pohyb zásob s přidruženou prací v řízení skladu](../supply-chain/warehousing/move-inventory-associated-work.md)
#### [Řízení pracovníků skladu](../supply-chain/warehousing/manage-warehouse-workers.md)
### Výdej a balení
#### [Potvrzení dávky a poznávací značky](../supply-chain/warehousing/batch-and-license-plate-confirmation.md)
#### [Vytvoření položky nabídky na mobilním zařízení pro konsolidaci registračních značek](../supply-chain/warehousing/tasks/create-mobile-device-license-plate-consolidation.md)
#### [Přijetí smíšené registrační značky](../supply-chain/warehousing/mixed-license-plate-receiving.md)
#### [Konfigurace možnosti Zobrazení starších dávek v rámci skladu na mobilním zařízení](../supply-chain/warehousing/display-older-batches-within-warehouses.md)
#### [Vyskladnění nejstarší dávky na mobilním zařízení](../supply-chain/warehousing/pick-oldest-batch.md)
#### [Potvrzení výdeje kusů](../supply-chain/warehousing/piece-picking-confirmation.md)
#### [Potvrzení produktu pro výdej v seskupení](../supply-chain/warehousing/cluster-picking-item-confirmation.md)
#### [Vydání dávky částečně rezervovaných převodních příkazů](../supply-chain/warehousing/batch-release-of-partially-reserved-transfer-orders.md)
#### [Obalové materiály a poplatky](../supply-chain/warehousing/pack-materials-packing-material-fees.md)
### Inventura skladu
#### [Blokování zásob](../supply-chain/inventory/inventory-blocking.md)
#### [Cyklická inventura](../supply-chain/warehousing/cycle-counting.md)
#### [Definování cyklické inventury ](../supply-chain/warehousing/tasks/define-cycle-counting-microsoft-dynamics-365-finance-operations-enterprise-edition-july-2017.md)
#### [Částečná cyklická inventura místa](../supply-chain/warehousing/partial-location-cycle-counting.md)
#### [Definování procesu částečné cyklické inventury skladového místa ](../supply-chain/warehousing/tasks/define-partial-location-cycle-counting-process-microsoft-dynamics-365-finance-operations-enterprise-edition-july-2017-update.md)
#### [Deník inventáře](../supply-chain/inventory/inventory-journals.md)
#### [Skladové místo](../supply-chain/inventory/inventory-locations.md)
#### [Stav skladu](../supply-chain/inventory/inventory-statuses.md)
#### [Počet označení zásob](../supply-chain/inventory/inventory-tag-counting.md)
### [Doplnění](../supply-chain/warehousing/replenishment.md)
#### [Doplnění s kanbany odběru](../supply-chain/warehousing/material-replenishment-withdrawal-kanban.md)
### [Instalace a konfigurace skladové aplikace ](../supply-chain/warehousing/install-configure-warehousing-app.md)
#### [Konfigurace názvů polí ve skladové aplikaci](../supply-chain/warehousing/configure-app-field-names-priorities-warehouse.md)
#### [Nastavení mobilních zařízení pro práci ve skladu](../supply-chain/warehousing/configure-mobile-devices-warehouse.md)
#### [Nastavení položky nabídky na mobilním zařízení pro dokončení práce v nákupní objednávce](../supply-chain/warehousing/tasks/set-up-mobile-device-menu.md)
#### [Nastavení položky nabídky na mobilním zařízení pro registraci přijatých položek](../supply-chain/warehousing/tasks/set-up-mobile-device-menu-item-register-received-items.md)
#### [Portál skladu pro mobilní zařízení (WMDP)](../supply-chain/warehousing/warehouse-mobile-devices-portal.md)
#### [Nastavení zobrazení skladových mobilních zařízení](../supply-chain/warehousing/change-warehouse-mobile-device-displays.md)
### Upgrade a migrace
#### [Upgrade procesů řízení skladu z aplikace AX 2012 na aplikaci Finance and Operations](../supply-chain/warehousing/upgrade-migration-warehouse-management-processes.md)

### Úkoly řízení skladu
#### [Konfigurace umístění ve skladu s povolenými procesy řízení skladu](../supply-chain/warehousing/tasks/configure-locations-wms-enabled-warehouse.md)
#### [Konfigurace zpracování vlny](../supply-chain/warehousing/tasks/configure-wave-processing.md)
#### [Vytvoření profilu skladového místa](../supply-chain/warehousing/tasks/create-location-profile.md)
#### [Vytvoření pracovní třídy](../supply-chain/warehousing/tasks/create-work-class.md)
#### [Používání cross dockingu pro distribuci produktů z přijímacího skladu do obchodů](../supply-chain/warehousing/tasks/cross-dock-products-receiving-warehouse-stores.md)
#### [Povolení tisku popisku registrační značky](../supply-chain/warehousing/tasks/license-plate-label-printing.md)
#### [Registrace položek pro položky umožňující pokročilé uskladnění s použitím deníku doručení zboží](../supply-chain/warehousing/tasks/register-items-advanced-warehousing.md)
#### [Registrace položek pro položky umožňující základní uskladnění s použitím deníku doručení zboží](../supply-chain/warehousing/tasks/register-items-basic-warehousing.md)
### [Nastavení směrnice skladového místa pro vyskladnění v rámci nákupní objednávky](../supply-chain/transportation/tasks/set-up-location-directive-purchase-order-put-away.md)
#### [Vytvoření procesu minimálního nebo maximálního doplňování](../supply-chain/warehousing/tasks/set-up-min-max-replenishment-process.md)
#### [Nastavení vytváření kontejnerů](../supply-chain/warehousing/tasks/set-up-containerization.md)
#### [Nastavení dispozičních kódů](../supply-chain/warehousing/tasks/set-up-dispositions-codes.md)
#### [Nastavení ručního balení (jen ve verzi z února 2016 a května 2016)](../supply-chain/warehousing/tasks/set-up-manual-packing-2016-02-05.md)
#### [Nastavení opakovaného přidělení zboží při krátkodobém vyskladnění](../supply-chain/warehousing/tasks/set-up-short-picking-item-reallocation.md)
#### [Nastavení zásad práce ve skladu ](../supply-chain/warehousing/tasks/set-up-warehouse-work-policies-2016-05.md)
#### [Nastavení šablony práce pro nákupní objednávky](../supply-chain/warehousing/tasks/set-up-work-template-purchase-orders.md)
#### [Plánování vytížení a dodávek s použitím pracovní plochy plánování vytížení](../supply-chain/warehousing/tasks/use-load-planning-workbench-plan-loads-shipments.md)

# [Vývoj a správa](../dev-itpro/toc.md)

# [Dynamics 365 for Retail](../retail/toc.md)

# [Dynamics 365 for Talent](../talent/toc.md)
