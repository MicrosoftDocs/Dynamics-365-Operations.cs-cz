...
Chcete-li použít správu obchodních dokumentů pro úpravy šablon ve formátech Excel nebo Word pomocí desktopových aplikací Microsoft Office, je nutné mít nainstalováno Microsoft Office 2010 nebo novější verzi. To je podporováno v cloudu a místním nasazení aplikace Finance and Operations.
...
# <a name="example-enable-configure-and-use-business-document-management"></a>Příklad: Povolení, konfigurace a použití správy obchodních dokumentů
...
Vzhledem k tomu, že správa obchodních dokumentů je na vrcholu architektury elektronického výkaznictví, je nutné parametry elektronického výkaznictví nakonfigurovat tak, aby začaly spolupracovat se správou obchodních dokumentů. Nejprve je třeba nastavit parametry elektronického výkaznictví, jak je popsáno v tématu [Konfigurace architektury elektronického výkaznictví](electronic-reporting-er-configure-parameters.md). Je také nutné přidat nového poskytovatele konfigurace, jak je popsáno v tématu [Vytvoření poskytovatelů konfigurace a jejich označení jako aktivních](tasks/er-configuration-provider-mark-it-active-2016-11.md).
...
Je nutné importovat konfigurace elektronického výkaznictví, které obsahují šablony obchodních dokumentů, do aktuální instance aplikace Finance and Operations. Chcete-li provést tento postup, stáhněte a místně uložte následující soubory.
...
Další informace o aktivaci nových funkcí naleznete v tématu [Přehled správy funkcí](../../fin-and-ops/get-started/feature-management/feature-management-overview.md).
...
Informace o nastavení požadovaných parametrů správy dokumentů a typů dokumentů naleznete v tématu [Konfigurace správy dokumentů](../../fin-and-ops/organization-administration/configure-document-management.md).
...
1. Přihlaste se k aplikaci Finance and Operations jako uživatel s přístupem na stránku **Parametry obchodního dokumentu**.
...
1. Přihlaste se k aplikaci Finance and Operations jako uživatel s přístupem na stránku **Konfigurátor přístupových oprávnění**.
...
Při tomto nastavení může každý uživatel s libovolnou rolí zabezpečení s oprávněním pro **správu šablon obchodních dokumentů** (název AOT **ERBDManageTemplates**) otevřít pracovní prostor správy obchodních dokumentů a upravit libovolnou šablonu, která je k dispozici v aplikaci Finance and Operations.
...
> [!NOTE]
> Pravidla **Přístupová oprávnění dle konfigurací** jsou uložena za pomoci jedinečného ID konfigurace formátu elektronického výkaznictví. To znamená, že tato pravidla nebudou odstraněna, pokud konfigurace elektronického výkaznictví, která na ně odkazuje, bude odstraněna. Při importu odstraněných konfigurací zpět do této instance aplikace Finance and Operations budou tato pravidla na ně odkazovat znovu. Po opětovném importu odstraněných konfigurací není nutné pravidla nastavovat znovu.
...
1. Přihlaste se k aplikaci Finance and Operations jako uživatel s přístupem na stránku pracovního prostoru správy dokumentů.
...
Otevře **Editor šablon BDM** aplikace Finance and Operations. Vybraná šablona bude k dispozici pro online úpravy pomocí Office 365.
...
1. V aplikaci Finance and Operations změňte na společnost **USMF**.
...
2. Vyberete-li možnost **OK** pro potvrzení publikování, draft odvozeného formátu elektronického výkaznictví **Kopie sestavy FTI odběratele (GER)**, který obsahuje upravenou šablonu, bude označen jako dokončený. Upravená šablona bude k dispozici pro ostatní uživatele aplikace Finance and Operations. Dokončené verze tohoto formátu elektronického výkaznictví zachovají pouze poslední aktivní revizi vaší šablony. Ostatní revize budou odstraněny. Chcete-li prozkoumat ostatní možnosti, vyberte možnost **Storno**.

## <a name="frequently-asked-questions"></a>Časté dotazy

#### <a name="i-selected-new-document-but-instead-of-opening-the-bdm-template-editor-page-in-finance-and-operations-i-have-been-sent-to-the-office-365-web-page"></a>Vybral jsem možnost **Nový dokument**, ale namísto otevření stránky **Editor šablon BDM** v aplikaci Finance and Operations jsem byl přesměrován na webovou stránku Office 365.
Jedná se o známý problém s přesměrováním Office 365 zpět do aplikace Finance and Operations. To se stává při prvním přihlášení k Office 365. Chcete-li tento problém vyřešit, v prohlížeči vyberte tlačítko **Zpět**, čímž přejdete zpět do aplikace Finance and Operations.
...
Pravděpodobně jste se přihlásili k aktuální instanci aplikace Finance and Operations v doméně Azure AD, která se liší od domény Azure AD, která byla použita pro nasazení této instance aplikace Finance and Operations. Vzhledem k tomu, že služba SharePoint, která slouží k ukládání šablon pro jejich zpřístupnění pro úpravy pomocí desktopových aplikací Office, patří do stejné domény jako aplikace Finance and Operations, nemáme žádná oprávnění pro přístup ke službě SharePoint. Chcete-li tento problém vyřešit, přihlaste se k aktuální instanci aplikace Finance and Operations s použitím přihlašovacích údajů uživatele se správnou doménou Azure AD.
