...
> [!NOTE]
> Pokud je hardwarová stanice "vyhrazena", je vztah 1:1 mezi klientem POS a hardwarovou stanicí.
...
3. Pak vytvořte hardwarovou stanici typu **Vyhrazený** pro obchod, kde bude pokladna POS používána. Popis je volitelný.
...
- Je třeba povolit a používat pouze bezpečnostní vrstvu Transport Layer Security (TLS) verze 1.2 (nebo stávající nejnovější verzi).
    ...
    > [!NOTE]
    > Ve výchozím nastavení, SSL a všechny verze TLS, s výjimkou TLS 1.2, jsou zakázány.

    Chcete-li upravit nebo povolit tyto hodnoty, postupujte takto:
...
- Počítač, na kterém běží moderní POS, důvěřuje certifikátu používanému v počítači, který spouští hardwarovou stanici.
    ...
    - Chcete-li ověřit tohoto nastavení ve webovém prohlížeči, přejděte na následující adresu URL: `https://<Computer Name>:<Port Number>/HardwareStation/ping`.
