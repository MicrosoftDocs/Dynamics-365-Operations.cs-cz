...
Funkce `VALUEINLARGE` určuje, zda uvedený vstup typu *Int64* nebo *Integer* odpovídá jakékoli hodnotě zadané položky v zadaném seznamu. Tato funkce vrátí *logickou hodnotu* **TRUE**, pokud zadaný vstup odpovídá výsledku spuštění zadaného výrazu pro alespoň jeden záznam zadaného seznamu. V opačném případě výraz vrátí *logickou hodnotu* **FALSE**. Abyste pochopili rozdíl oproti funkci `VALUEIN`, viz sekce [Poznámka k použití](#usage_note) dále v tomto tématu.
