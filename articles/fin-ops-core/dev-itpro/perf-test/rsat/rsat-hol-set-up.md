...
### <a name="user-eligibility"></a><span data-ttu-id="6349d-124">Způsobilost uživatele</span><span class="sxs-lookup"><span data-stu-id="6349d-124">User eligibility</span></span>
...
<span data-ttu-id="6349d-173">Pro svůj projekt LCS můžete použít existující zákaznickou implementaci nebo projekt partnera.</span><span class="sxs-lookup"><span data-stu-id="6349d-173">For your LCS project, you can use an existing customer implementation or partner project.</span></span>
...
3. <span data-ttu-id="6349d-187">V levém podokně vyberte **Visual Studio Team Services** a pak vyberte možnost **Nastavit Visual Studio Team Services**.</span><span class="sxs-lookup"><span data-stu-id="6349d-187">In the left pane, select **Visual Studio Team Services**, and then select **Setup Visual Studio Team Services**.</span></span>
...
8. <span data-ttu-id="6349d-270">Chcete-li záznam úkolu uložit místně a poté jej odeslat do seznamu BPM pomocí LCS, postupujte následujícím způsobem:</span><span class="sxs-lookup"><span data-stu-id="6349d-270">If you want to save the task recording locally and then upload it to BPM through LCS, follow these steps:</span></span>
    ...
    3. <span data-ttu-id="6349d-275">Přejděte do knihovny **BPM** pro RSAT a vyberte obchodní proces, do kterého chcete uložit záznam úloh.</span><span class="sxs-lookup"><span data-stu-id="6349d-275">Go to the **RSAT** BPM library, and select the business process to save the task recording against.</span></span>
...
#### <a name="tier-2-environment"></a><span data-ttu-id="6349d-335">Prostředí vrstvy 2+</span><span class="sxs-lookup"><span data-stu-id="6349d-335">Tier 2+ environment</span></span>
...
2. <span data-ttu-id="6349d-479">Vyberte testovací případ a poté výběrem možnosti **Upravit** otevřete soubor parametrů aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-479">Select the test case, and then select **Edit** to open the Excel parameter file.</span></span>
    ...
    <span data-ttu-id="6349d-480">Na listu **EcoResProductCreate** si povšimněte, že hodnota pole **Číslo produktu** je pevně kódována.</span><span class="sxs-lookup"><span data-stu-id="6349d-480">On the **EcoResProductCreate** sheet, notice that the value of the **Product number** field is hard-coded.</span></span> <span data-ttu-id="6349d-481">Chcete-li znovu spustit testovací případ, je nutné toto pole aktualizovat na nové číslo produktu.</span><span class="sxs-lookup"><span data-stu-id="6349d-481">You must update this field to a new product number before you run the test case again.</span></span>

3. <span data-ttu-id="6349d-482">Chcete-li pro každý běh generovat jedinečné číslo produktu bez nutnosti znovu otevřít soubor parametrů aplikace Excel a ručně aktualizovat číslo produktu, použijte následující vzorec aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-482">To generate a unique product number for each run without having to reopen the Excel parameter file and manually update the product number every time, use the following Excel formula.</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="6349d-483">Kromě karty **obecné** obsahuje soubor parametrů aplikace Excel kartu data pro každou stránku formuláře pro návštěvy testovacího případu.</span><span class="sxs-lookup"><span data-stu-id="6349d-483">In addition to the **General** tab, the Excel parameter file contains a data tab for every form page the test case visits.</span></span>

    ![Pole Číslo produktu](./media/setup_rsa_tool_81.png)

4. <span data-ttu-id="6349d-485">Zvolte **Uložit** a pak zavřete sešit aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-485">Select **Save**, and then close the Excel workbook.</span></span>
5. <span data-ttu-id="6349d-486">Výběrem možnosti **Nahrát** uložte soubor parametrů aplikace Excel do Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="6349d-486">Select **Upload** to save the Excel parameter file to Azure DevOps.</span></span>

    ![Zpráva o úspěšném odeslání](./media/setup_rsa_tool_82.png)

    > [!NOTE]
    > <span data-ttu-id="6349d-488">Pokud chcete spustit testovací případy v určitém kontextu uživatele, zadejte do pole **Zkušební uživatel** na kartě **Obecné** souboru parametrů Excel ID e-mailu uživatele.</span><span class="sxs-lookup"><span data-stu-id="6349d-488">To run test cases in a specific user context, enter the user's email ID in the **Test User** field on the **General** tab of the Excel parameter file.</span></span> <span data-ttu-id="6349d-489">V nejnovější verzi aplikace RSAT bylo aktualizováno rozložení polí v souboru parametrů aplikace Excel, ale koncept zůstává stejný.</span><span class="sxs-lookup"><span data-stu-id="6349d-489">In the latest version of RSAT, the layout of the fields in the Excel parameter file has been updated, but the concept remains the same.</span></span>
    >
    > ![Pole Testovací uživatel](./media/setup_rsa_tool_83.png)

### <a name="validate-the-results"></a><span data-ttu-id="6349d-491">Ověření výsledků</span><span class="sxs-lookup"><span data-stu-id="6349d-491">Validate the results</span></span>

- <span data-ttu-id="6349d-492">Výběrem **Spustit** znovu spusťte testovací případ a ověřte, zda byl testovací případ úspěšný.</span><span class="sxs-lookup"><span data-stu-id="6349d-492">Select **Run** to rerun the test case, and verify that the test case has passed.</span></span> <span data-ttu-id="6349d-493">Výsledky testů můžete zobrazit způsobem popsaným v části [Zobrazení výsledků testu](#view-the-test-results).</span><span class="sxs-lookup"><span data-stu-id="6349d-493">You can view the test results as described in the [View the test results](#view-the-test-results) section.</span></span>

    ![Pole výsledku je nastaveno na Úspěch](./media/setup_rsa_tool_84.png)

### <a name="chaining-of-test-cases"></a><span data-ttu-id="6349d-495">Zřetězení testovacích případů</span><span class="sxs-lookup"><span data-stu-id="6349d-495">Chaining of test cases</span></span>

<span data-ttu-id="6349d-496">Jedním klíčovým prvkem nástroje RSAT je zřetězení testovacích případů (tj. schopnost testu předat hodnoty jiným testům).</span><span class="sxs-lookup"><span data-stu-id="6349d-496">One key feature of RSAT is the chaining of test cases (that is, the capability of a test to pass values to other tests).</span></span> <span data-ttu-id="6349d-497">Testovací případy se spouštějí podle jejich definovaného pořadí v testovacím plánu Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="6349d-497">Test cases are run according to their defined order in the Azure DevOps test plan.</span></span> <span data-ttu-id="6349d-498">(Tuto objednávku lze také aktualizovat samotným testovacím nástrojem.) Chcete-li tedy předávat proměnné z jednoho testovacího případu do jiného, je velmi důležité, aby testy byly ve správném pořadí.</span><span class="sxs-lookup"><span data-stu-id="6349d-498">(This order can also be updated in the test tool itself.) Therefore, if you want to pass variables from one test case to another, it's very important that the tests be in the correct order.</span></span>

<span data-ttu-id="6349d-499">V tomto oddílu vytvoříte uloženou proměnnou v prvním testovacím případu, vytvoříte druhý testovací případ a předáte uloženou proměnnou z prvního testovacího případu do druhého testovacího případu.</span><span class="sxs-lookup"><span data-stu-id="6349d-499">In this section, you will create a saved variable in the first test case, create a second test case, and pass the saved variable from the first test case to the second test case.</span></span> <span data-ttu-id="6349d-500">Testovací případy pak spustíte jako zřetězený testovací případ na kartě RSAT.</span><span class="sxs-lookup"><span data-stu-id="6349d-500">You will then run the test cases as a chained test case in RSAT.</span></span>

#### <a name="modify-an-existing-task-recording-to-create-a-saved-variable"></a><span data-ttu-id="6349d-501">Úprava existujícího záznamu úlohy za účelem vytvoření uložené proměnné</span><span class="sxs-lookup"><span data-stu-id="6349d-501">Modify an existing task recording to create a saved variable</span></span>

1. <span data-ttu-id="6349d-502">Spusťte klienta.</span><span class="sxs-lookup"><span data-stu-id="6349d-502">Open the client.</span></span>
2. <span data-ttu-id="6349d-503">Vyberte tlačítko **Nastavení** (symbol ozubeného kola) a poté vyberte položku **Záznamník úloh**.</span><span class="sxs-lookup"><span data-stu-id="6349d-503">Select the **Settings** button (the gear symbol), and then select **Task recorder**.</span></span>
3. <span data-ttu-id="6349d-504">Vyberte **Upravti záznam**.</span><span class="sxs-lookup"><span data-stu-id="6349d-504">Select **Edit Recording**.</span></span>

    ![Tlačítko Upravit záznam](./media/setup_rsa_tool_85.png)

4. <span data-ttu-id="6349d-506">Vyberte **Otevřít ze služeb Lifecycle Services**</span><span class="sxs-lookup"><span data-stu-id="6349d-506">Select **Open from Lifecycle Services**.</span></span>

    ![Tlačítko Otevřít ze služeb Lifecycle Services](./media/setup_rsa_tool_86.png)

5. <span data-ttu-id="6349d-508">Vyberte **Vybrat knihovnu služeb Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="6349d-508">Select **Select the Lifecycle Services library**.</span></span>

    ![Tlačítko Vybrat knihovnu služeb Lifecycle Services](./media/setup_rsa_tool_87.png)

    <span data-ttu-id="6349d-510">Knihovny BPM jsou načítány z LCS.</span><span class="sxs-lookup"><span data-stu-id="6349d-510">BPM libraries are loaded from LCS.</span></span>

    ![Načítání knihoven BPM](./media/setup_rsa_tool_88.png)

6. <span data-ttu-id="6349d-512">Po načtení knihoven BPM z LCS vyberte knihovnu BPM **RSAT** a obchodní proces **Vytvořit nový produkt**, ke kterému byl záznam úkolu přidružen.</span><span class="sxs-lookup"><span data-stu-id="6349d-512">After the BPM libraries are loaded from LCS, select the **RSAT** BPM library and the **Create a new product** business process that the task recording was associated with.</span></span> <span data-ttu-id="6349d-513">Pak vyberte **OK**.</span><span class="sxs-lookup"><span data-stu-id="6349d-513">Then select **OK**.</span></span>

    ![Výběr knihovny BPM a obchodního procesu](./media/setup_rsa_tool_89.png)

7. <span data-ttu-id="6349d-515">Název příslušného záznamu úkolu je zadán do pole název **název záznamu**.</span><span class="sxs-lookup"><span data-stu-id="6349d-515">The name of the appropriate task recording is entered in the **Recording name** field.</span></span> <span data-ttu-id="6349d-516">Vyberte **Spustit**.</span><span class="sxs-lookup"><span data-stu-id="6349d-516">Select **Start**.</span></span>

    ![Název záznamu úlohy v poli Název úlohy](./media/setup_rsa_tool_90.png)

8. <span data-ttu-id="6349d-518">Přejděte na **Správa informací o produktu \> Produkty** a vyberte **Nový** k otevření stránky, kde byla pořízena původní nahrávka úlohy **Vytvořit produkt**.</span><span class="sxs-lookup"><span data-stu-id="6349d-518">Go to **Product information management \> Products**, and select **New** to open the page where the original task recording, **Create a product**, was recorded.</span></span>
9. <span data-ttu-id="6349d-519">Vyberte **Vložit krok**.</span><span class="sxs-lookup"><span data-stu-id="6349d-519">Select **Insert step**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="6349d-520">Nový krok je vložen **za** krok, který jste vybrali v podokně.</span><span class="sxs-lookup"><span data-stu-id="6349d-520">The new step is inserted **after** the step that you selected in the pane.</span></span>

    ![Tlačítko Vložit krok](./media/setup_rsa_tool_91.png)

10. <span data-ttu-id="6349d-522">Klepněte pravým tlačítkem myši na pole **Číslo produktu** a vyberte **Záznamník úloh \> Kopírovat**.</span><span class="sxs-lookup"><span data-stu-id="6349d-522">Right-click the **Product number** field, and then select **Task recorder \> Copy**.</span></span>

    ![Příkaz Kopírovat](./media/setup_rsa_tool_92.png)

11. <span data-ttu-id="6349d-524">V podokně je přidán nový krok.</span><span class="sxs-lookup"><span data-stu-id="6349d-524">A new step is added in the pane.</span></span> <span data-ttu-id="6349d-525">Poznamenejte si hodnotu v poli **Číslo produktu**, protože ji budete potřebovat později.</span><span class="sxs-lookup"><span data-stu-id="6349d-525">Make a note of the value in the **Product number** field, because you will need it later.</span></span>

    ![Přidán nový krok](./media/setup_rsa_tool_93.png)

12. <span data-ttu-id="6349d-527">Vyberte **Dokončeny úpravy**.</span><span class="sxs-lookup"><span data-stu-id="6349d-527">Select **Done editing**.</span></span>
13. <span data-ttu-id="6349d-528">Vyberte **Uložit do Lifecycle Services** a přiřaďte nový záznam úkolu ke stejné knihovně BPM a obchodnímu procesu, k němuž byl záznam původního úkolu přidružen.</span><span class="sxs-lookup"><span data-stu-id="6349d-528">Select **Save to Lifecycle Services**, and associate the new task recording with the same BPM library and business process that the original task recording was associated with.</span></span> <span data-ttu-id="6349d-529">Další informace naleznete v tématu [Vytvoření záznamu úlohy a jeho uložení do oddílu knihovny BPM](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="6349d-529">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>
14. <span data-ttu-id="6349d-530">Přejděte do knihovny BPM a vyberte **Synchronizovat testovací případy** pro přepis záznamu úloh připojenému k testovacímu případu v Azure DevOps, jak je popsáno v části [Testování synchronizace z BPM do Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="6349d-530">Go to the BPM library, and select **Sync testcases** to overwrite the task recording that is attached to the test case in Azure DevOps, as described in the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>
15. <span data-ttu-id="6349d-531">Otevřete RSAT a vyberte **Načíst**, chcete-li znovu načíst všechny testovací případy v sadě testů.</span><span class="sxs-lookup"><span data-stu-id="6349d-531">Open RSAT, and select **Load** to reload all the test cases in the test suite.</span></span> <span data-ttu-id="6349d-532">Je nutné znovu vygenerovat soubory automatizace a parametrů pro příslušný testovací případ výběrem testovacího případu a následným výběrem **Nový \> Generovat soubory spuštění testu a parametry**, jak je popsáno v části [Načtení a spuštění testovacích případů](#load-and-run-test-cases).</span><span class="sxs-lookup"><span data-stu-id="6349d-532">You must regenerate the automation and parameter files for the appropriate test case by selecting the test case and then selecting **New \> Generate Test Execution and Parameter files**, as described in the [Load and run test cases](#load-and-run-test-cases) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="6349d-533">Pokud byl soubor parametrů aplikace Excel ponechán otevřený, obnovení se nezdaří.</span><span class="sxs-lookup"><span data-stu-id="6349d-533">If the Excel parameter file was left open, regeneration will fail.</span></span> <span data-ttu-id="6349d-534">Proto se ujistěte, že soubor parametrů aplikace Excel pro testovací případ je uzavřen před tím, než vygenerujete nový soubor parametrů aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-534">Therefore, make sure that the Excel parameter file for the test case is closed before you generate the new Excel parameter file.</span></span>

16. <span data-ttu-id="6349d-535">Vyberte **Upravit** k otevření nového souboru parametrů aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-535">Select **Edit** to open the new Excel parameter file.</span></span> <span data-ttu-id="6349d-536">Bude zobrazena nová **uložená proměnná** položka na řádku 9.</span><span class="sxs-lookup"><span data-stu-id="6349d-536">You will see a new **Saved variable** entry on line 9.</span></span> <span data-ttu-id="6349d-537">Tato proměnná **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, je uložena v souboru XML záznamu úkolu a lze ji použít v následných testech.</span><span class="sxs-lookup"><span data-stu-id="6349d-537">This variable, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, is saved in the task recording's XML file and can be used in subsequent tests.</span></span>

    ![Záznam uložené proměnné](./media/setup_rsa_tool_94.png)

#### <a name="create-a-new-test-case"></a><span data-ttu-id="6349d-539">Vytvoření nového testovacího případu</span><span class="sxs-lookup"><span data-stu-id="6349d-539">Create a new test case</span></span>

1. <span data-ttu-id="6349d-540">Přejděte do knihovny **RSAT** BPM.</span><span class="sxs-lookup"><span data-stu-id="6349d-540">Go to the **RSAT** BPM library.</span></span>
2. <span data-ttu-id="6349d-541">Vyberte **příklad obchodního procesu podpory** a poté na pravé straně vyberte **režim úprav**.</span><span class="sxs-lookup"><span data-stu-id="6349d-541">Select the **Sample Support Business Process** process, and then, on the right, select **Edit mode**.</span></span>
3. <span data-ttu-id="6349d-542">Změňte hodnotu v poli **Název** a v poli **Popis** na **Vydat produkt**.</span><span class="sxs-lookup"><span data-stu-id="6349d-542">Change the value of both the **Name** field and the **Description** field to **Release a product**.</span></span> <span data-ttu-id="6349d-543">Pak vyberte **Uložit**.</span><span class="sxs-lookup"><span data-stu-id="6349d-543">Then select **Save**.</span></span>

    ![Název a popis se změnil na Uvolnění produktu.](./media/setup_rsa_tool_95.png)

#### <a name="create-a-new-task-recording-that-has-a-validate-function"></a><span data-ttu-id="6349d-545">Vytvoření nového záznamu úlohy s funkcí ověření</span><span class="sxs-lookup"><span data-stu-id="6349d-545">Create a new task recording that has a Validate function</span></span>

- <span data-ttu-id="6349d-546">Vytvořte záznam úloh pro uvolnění produktu, který byl vytvořen dříve, do právnické osoby USRT.</span><span class="sxs-lookup"><span data-stu-id="6349d-546">Create a task recording to release the product that was created earlier to the USRT legal entity.</span></span> <span data-ttu-id="6349d-547">Další informace naleznete v tématu [Vytvoření záznamu úlohy a jeho uložení do oddílu knihovny BPM](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="6349d-547">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="6349d-548">U zřetězených testovacích případů doporučujeme, abyste vyhledali nebo vyfiltroval záznam, který požadujete, *ručním zadáním hodnoty pole*.</span><span class="sxs-lookup"><span data-stu-id="6349d-548">For chained test cases, we always recommend that you find or filter for the record that you require by *manually typing the value of the field*.</span></span> <span data-ttu-id="6349d-549">Tímto způsobem může nástroj určit záznam, proti němuž je akce provedena v následném testovacím případu.</span><span class="sxs-lookup"><span data-stu-id="6349d-549">In that way, the tool can determine the record that the action must be taken against in the subsequent test case.</span></span>

    ![Nový záznam úlohy s funkcí ověření](./media/setup_rsa_tool_96.png)

    <span data-ttu-id="6349d-551">Jak ukazuje předchozí ilustrace, po nalezení produktu pomocí rychlého filtru, ale před tím, než vyberete **uvolněné produkty**, ověřte hodnotu pole **číslo produktu**, abyste se ujistili, že ID produktu je ID produktu, které bylo vytvořeno dříve.</span><span class="sxs-lookup"><span data-stu-id="6349d-551">As the preceding illustration shows, after the product is found by using the Quick Filter, but before you select **Release products**, you validate the value of the **Product number** field to make sure that the product ID is the product ID that was created earlier.</span></span> <span data-ttu-id="6349d-552">Chcete-li hodnotu ověřit, klepněte pravým tlačítkem myši na pole **Číslo produktu** a vyberte **Záznamník úloh \> Ověřit \> Aktuální hodnota**.</span><span class="sxs-lookup"><span data-stu-id="6349d-552">To validate the value, right-click the **Product number** field, and then select **Task recorder \> Validate \> Current Value**.</span></span>

    ![Ověření aktuální hodnoty](./media/setup_rsa_tool_97.png)

#### <a name="save-the-task-recording-to-bpm"></a><span data-ttu-id="6349d-554">Uložení záznamu úlohy do BPM</span><span class="sxs-lookup"><span data-stu-id="6349d-554">Save the task recording to BPM</span></span>

1. <span data-ttu-id="6349d-555">Po dokončení záznamu vyberte možnost **Uložit do Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="6349d-555">After the task recording is completed, select **Save to Lifecycle Services**.</span></span>

    ![Uložení dokončeného záznamu úlohy do služby Lifecycle Services](./media/setup_rsa_tool_98.png)

2. <span data-ttu-id="6349d-557">Informace o knihovně jsou načítány z LCS.</span><span class="sxs-lookup"><span data-stu-id="6349d-557">Library information is loaded from LCS.</span></span>

    ![Načítání informací o knihovně z LCS](./media/setup_rsa_tool_99.png)

3. <span data-ttu-id="6349d-559">Vyberte knihovnu BPM pro přidružení k záznamu úlohy.</span><span class="sxs-lookup"><span data-stu-id="6349d-559">Select the BPM library to associate the task recording with.</span></span> <span data-ttu-id="6349d-560">V tomto kurzu vyberte knihovnu **RSAT** BPM, která byla vytvořena dříve, a vytvořte v ní obchodní proces **Uvolnit produkt**.</span><span class="sxs-lookup"><span data-stu-id="6349d-560">For this tutorial, select the **RSAT** BPM library that was created earlier and the **Release a product** business process under it.</span></span> <span data-ttu-id="6349d-561">Pak vyberte **OK**.</span><span class="sxs-lookup"><span data-stu-id="6349d-561">Then select **OK**.</span></span>

#### <a name="sync-bpm-to-azure-devops"></a><span data-ttu-id="6349d-562">Synchronizace BPM v Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="6349d-562">Sync BPM to Azure DevOps</span></span>

1. <span data-ttu-id="6349d-563">Přejděte do knihovny BPM a otevřete knihovnu **RSAT**.</span><span class="sxs-lookup"><span data-stu-id="6349d-563">Go to the BPM library, and open the **RSAT** library.</span></span>
2. <span data-ttu-id="6349d-564">Vyberte **Synchronizace VSTS** a **Synchronizovat testovací případy**.</span><span class="sxs-lookup"><span data-stu-id="6349d-564">Select **VSTS sync** and then **Sync test cases**.</span></span> <span data-ttu-id="6349d-565">Další informace naleznete v tématu [Testování synchronizace z BPM do Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="6349d-565">For more information, see the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>

    <span data-ttu-id="6349d-566">Po dokončení synchronizace se nová položka práce a odpovídající testovací případ pro obchodní proces **Uvolnit produkt** objeví v Azure DevOps v části **Vývěsky \> Položky práce**.</span><span class="sxs-lookup"><span data-stu-id="6349d-566">After the synchronization is completed, the new work item and the corresponding test case for the **Release a product** business process appear in Azure DevOps at **Boards \> Work Items**.</span></span>

#### <a name="add-the-new-test-case-to-the-existing-test-suite"></a><span data-ttu-id="6349d-567">Přidání novéno testovacího případu do existující sady testů</span><span class="sxs-lookup"><span data-stu-id="6349d-567">Add the new test case to the existing test suite</span></span>

1. <span data-ttu-id="6349d-568">Přejděte na **Testovací plány \> Testovací plány** a vyberte plán **RSAT Test Plan**.</span><span class="sxs-lookup"><span data-stu-id="6349d-568">Go to **Test plans \> Test plans**, and select the **RSAT Test Plan** plan.</span></span>
2. <span data-ttu-id="6349d-569">Vyberte **Přidat existující**.</span><span class="sxs-lookup"><span data-stu-id="6349d-569">Select **Add existing**.</span></span>
3. <span data-ttu-id="6349d-570">Na stránce **Přidat testovací případy do sady** vyberte možnost **spustit dotaz**.</span><span class="sxs-lookup"><span data-stu-id="6349d-570">On the **Add test cases to suite** page, select **Run query**.</span></span>
4. <span data-ttu-id="6349d-571">Vyberte nový testovací případ, který byl vytvořen pro **uvolnění produktu** a poté vyberte možnost **přidat testovací případy** v pravém dolním rohu stránky (toto tlačítko není zobrazeno na následujícím obrázku).</span><span class="sxs-lookup"><span data-stu-id="6349d-571">Select the new test case that was created for **Release a product**, and then select **Add test cases** in the lower-right corner of the page (this button isn't shown in the following illustration).</span></span>

    ![Stránka Přidat testovací případy do sady](./media/setup_rsa_tool_100.png)

    <span data-ttu-id="6349d-573">Sada testů nyní obsahuje dva testovací případy.</span><span class="sxs-lookup"><span data-stu-id="6349d-573">The test suite now has two test cases.</span></span>

    ![Dva testovací případy v sadě testů](./media/setup_rsa_tool_101.png)

#### <a name="load-test-cases-into-rsat"></a><span data-ttu-id="6349d-575">Nahrání testovacích případů do RSAT</span><span class="sxs-lookup"><span data-stu-id="6349d-575">Load test cases into RSAT</span></span>

1. <span data-ttu-id="6349d-576">Otevřete okno RSAT a vyberte **Načíst**.</span><span class="sxs-lookup"><span data-stu-id="6349d-576">Open RSAT, and select **Load**.</span></span>
2. <span data-ttu-id="6349d-577">Dojde k načtení testovacích případů a obdržíte upozornění, že tato akce přepíše soubory testovacích dat aplikace Excel, místní změny budou ztraceny.</span><span class="sxs-lookup"><span data-stu-id="6349d-577">The test cases are loaded, and you receive a warning that states, "This action will overwrite Excel test data files, local changes will be lost.</span></span> <span data-ttu-id="6349d-578">Chcete pokračovat?</span><span class="sxs-lookup"><span data-stu-id="6349d-578">Do you want to proceed?"</span></span> <span data-ttu-id="6349d-579">Výběrem **Ano** aktualizujete soubory parametrů aplikace Excel v místním systému, ale nepoužijete soubory parametrů aplikace Excel v Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="6349d-579">Select **Yes** to update the Excel parameter files in the local system but not the Excel parameter files that were uploaded to Azure DevOps.</span></span>

    ![Tato akce přepíše testovací datové soubory aplikace Excel](./media/setup_rsa_tool_102.png)

    <span data-ttu-id="6349d-581">Oba testovací případy se načítají spolu se souborem parametrů aplikace Excel pro první testovací případ.</span><span class="sxs-lookup"><span data-stu-id="6349d-581">Both the test cases are loaded, together with the Excel parameter file for the first test case.</span></span> <span data-ttu-id="6349d-582">Protože jste vybrali **Odeslat** při posledním spuštění, budou soubory parametrů načteny z Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="6349d-582">Because you selected **Upload** in the last run, the parameter files are pulled from Azure DevOps.</span></span>

    ![Testovací případy odeslány](./media/setup_rsa_tool_103.png)

3. <span data-ttu-id="6349d-584">Vyberte pouze druhý testovací případ a poté vyberte **Nový \> Generovat soubory parametrů spuštění testu a parametrů**.</span><span class="sxs-lookup"><span data-stu-id="6349d-584">Select only the second test case, and then select **New \> Generate test execution and parameter files**.</span></span>

#### <a name="edit-the-excel-parameter-file"></a><span data-ttu-id="6349d-585">Úprava souboru parametrů aplikace Excel</span><span class="sxs-lookup"><span data-stu-id="6349d-585">Edit the Excel parameter file</span></span>

1. <span data-ttu-id="6349d-586">Vyberte jenom druhý testovací případ a poté výběrem možnosti **Upravit** otevřete odpovídající soubor parametrů aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-586">Select only the second test case, and then select **Edit** to open the corresponding Excel parameter file.</span></span>
2. <span data-ttu-id="6349d-587">Zkopírujte uloženou proměnnou **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** (viz [Modifikace existujícího záznamu úlohy k vytvoření uložené proměnné](#modify-an-existing-task-recording-to-create-a-saved-variable)) z prvního testovacího případu do všech polí, kde se používá číslo produktu.</span><span class="sxs-lookup"><span data-stu-id="6349d-587">Copy the **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** saved variable (see the [Modify an existing task recording to create a saved variable](#modify-an-existing-task-recording-to-create-a-saved-variable) section) from the first test case into all the fields where the product number is used.</span></span> <span data-ttu-id="6349d-588">V takovém případě zkopírujte proměnnou do polí **Číslo produktu** a **Ověřit číslo produktu** v listu **EcoResProductListPage**.</span><span class="sxs-lookup"><span data-stu-id="6349d-588">In this case, you copy the variable into the **Product number** and **Validate Product number** fields on the **EcoResProductListPage** sheet.</span></span>

    ![Pole Číslo produktu a Ověřit číslo produktu](./media/setup_rsa_tool_104.png)

    > [!NOTE]
    > <span data-ttu-id="6349d-590">Proměnné lze mezi testy přenášet pouze během jednoho testovacího běhu.</span><span class="sxs-lookup"><span data-stu-id="6349d-590">Variables can be passed between tests only during the same test run.</span></span> <span data-ttu-id="6349d-591">Názvy proměnných se musí přesně shodovat.</span><span class="sxs-lookup"><span data-stu-id="6349d-591">The names of the variables must match exactly.</span></span>

3. <span data-ttu-id="6349d-592">Zvolte **Uložit** a pak zavřete sešit aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-592">Select **Save**, and then close the Excel workbook.</span></span>
4. <span data-ttu-id="6349d-593">Vyberte **Nahrát** pro uložení provedených změn do souboru parametrů aplikace Excel.</span><span class="sxs-lookup"><span data-stu-id="6349d-593">Select **Upload** to save the changes that you made to the Excel parameter file.</span></span>

#### <a name="run-the-chained-test-cases"></a><span data-ttu-id="6349d-594">Spuštění zřetězených testovacích případů</span><span class="sxs-lookup"><span data-stu-id="6349d-594">Run the chained test cases</span></span>

1. <span data-ttu-id="6349d-595">Vyberte oba testovací případy a pak vyberte **Spustit**.</span><span class="sxs-lookup"><span data-stu-id="6349d-595">Select both the test cases, and then select **Run**.</span></span>
2. <span data-ttu-id="6349d-596">Ověřte, zda oba testovací případy byly úspěšné.</span><span class="sxs-lookup"><span data-stu-id="6349d-596">Verify that both test cases have passed.</span></span>
