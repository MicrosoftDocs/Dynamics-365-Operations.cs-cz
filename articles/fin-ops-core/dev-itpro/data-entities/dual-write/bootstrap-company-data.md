...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="0731d-103">Často kladené dotazy ke zavádění s daty společnosti</span><span class="sxs-lookup"><span data-stu-id="0731d-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="0731d-104">Proč potřebuji zavádění?</span><span class="sxs-lookup"><span data-stu-id="0731d-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="0731d-105">Je možné, že máte stávající Common Data Service nebo jinou instanci aplikace Dynamics 365 s obchodními daty a chcete pro ni povolit propojení s dvojím zápisem.</span><span class="sxs-lookup"><span data-stu-id="0731d-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="0731d-106">V tomto případě musíte zavést Common Data Service nebo jiná data aplikace Dynamics 365 s informacemi o společnosti před povolením propojení s dvojím zápisem.</span><span class="sxs-lookup"><span data-stu-id="0731d-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="0731d-107">Kdy mám použít spuštění?</span><span class="sxs-lookup"><span data-stu-id="0731d-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="0731d-108">Zavádění byste měli použít před povolením mapování entit dvojího zápisu (během kroku 5).</span><span class="sxs-lookup"><span data-stu-id="0731d-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="0731d-109">Chcete-li nastavit propojení s dvojí zápisem mezi instancemi aplikace Finance and Operations a Common Data Service neb jinou aplikací Dynamics 365, přihlaste se k aplikaci Finance and Operations jako správce.</span><span class="sxs-lookup"><span data-stu-id="0731d-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="0731d-110">Přejděte na modul **Správa dat** a klikněte na **Dvojí zápis**.</span><span class="sxs-lookup"><span data-stu-id="0731d-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="0731d-111">Tím se spustí **Integrátor dat**.</span><span class="sxs-lookup"><span data-stu-id="0731d-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="0731d-112">Vytvořte propojení s dvojím zápisem pro jednu nebo více společností.</span><span class="sxs-lookup"><span data-stu-id="0731d-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="0731d-113">![Vytvoření propojení s dvojím zápisem](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="0731d-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="0731d-114">Povolte mapování entit **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="0731d-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="0731d-115">Tato možnost synchronizuje společnosti z aplikace Finance and Operations s aplikací Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="0731d-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="0731d-116">![Povolení mapování entit](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="0731d-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="0731d-117">Spusťte ukázkový zaváděcí kód na instanci Common Data Service nebo jiné aplikace Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="0731d-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="0731d-118">Když je zavádění hotovo a systém je připraven na živou synchronizaci, povolte mapování entit.</span><span class="sxs-lookup"><span data-stu-id="0731d-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="0731d-119">Povolením mapování entit se spustí počáteční synchronizace dat pro povolené mapování entit.</span><span class="sxs-lookup"><span data-stu-id="0731d-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="0731d-120">Data odpovídající společnostem vybraným v propojení s duálním zápisem jsou synchronizována mezi aplikací Finance and Operations a Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="0731d-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="0731d-121">Jak použiji ukázku kódu?</span><span class="sxs-lookup"><span data-stu-id="0731d-121">How to I use the code sample?</span></span>
<span data-ttu-id="0731d-122">Ukázkový kód je aplikace v jazyce C#, kterou lze načíst v aplikaci Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="0731d-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="0731d-123">Použije závislosti balíčku NuGet v Common Data Service, které lze aktualizovat pomocí standardních nástrojů Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="0731d-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="0731d-124">Po rozbalení a otevření řešení v aplikaci Visual Studio a obnovení balíčků NuGet vyhledejte **TODO** v kódu.</span><span class="sxs-lookup"><span data-stu-id="0731d-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="0731d-125">Každé rozhodnutí o způsobu, jakým chcete zavádět informace o společnosti, je zaznamenáno pomocí **TODO** s ukázkovým kódem pro kanonickou implementaci.</span><span class="sxs-lookup"><span data-stu-id="0731d-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="0731d-126">Ukázkový kód zobrazuje pouze jeden z mnoha způsobů, jak můžete kategorizovat záznamy entit podle společnosti.</span><span class="sxs-lookup"><span data-stu-id="0731d-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="0731d-127">Změnou logiky v částech **TODO** můžete vytvořit vlastní kategorizaci.</span><span class="sxs-lookup"><span data-stu-id="0731d-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="0731d-128">Co mám očekávat?</span><span class="sxs-lookup"><span data-stu-id="0731d-128">What should I expect?</span></span>
<span data-ttu-id="0731d-129">Ve výchozím nastavení umožňuje ukázková aplikace poskytnout slovník mapování kódů obchodní jednotky na společnost.</span><span class="sxs-lookup"><span data-stu-id="0731d-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="0731d-130">Každá entita, kterou zavádíte pomoc pole **OwningBusinessUnit**, je automaticky nastavena k použití určené společnosti.</span><span class="sxs-lookup"><span data-stu-id="0731d-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="0731d-131">Jakákoliv entita bez pole **OwningBusinessUnit**, například produkt, nastaví společnost na základě mapování s prázdnou hodnotou obchodní jednotky.</span><span class="sxs-lookup"><span data-stu-id="0731d-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="0731d-132">Aplikace konzoly očekává jeden parametr, buď **–simulate** nebo **–apply**.</span><span class="sxs-lookup"><span data-stu-id="0731d-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="0731d-133">Pokud použijete parametr příkazového řádku **-simulovat** , nebudou aktualizována žádná data.</span><span class="sxs-lookup"><span data-stu-id="0731d-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="0731d-134">Pouze soubory **simulation_<entityname>.csv** jsou generovány ve stejném adresáři jako nástroj, jeden pro každou entitu, která by byla aktualizována.</span><span class="sxs-lookup"><span data-stu-id="0731d-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="0731d-135">Tyto soubory můžete při práci iterativně zkontrolovat, abyste zajistili, že kód bude aktualizovat hodnoty společnosti očekávaným způsobem.</span><span class="sxs-lookup"><span data-stu-id="0731d-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="0731d-136">Po dokončení simulovaných aktualizací použijte parametr **-apply**.</span><span class="sxs-lookup"><span data-stu-id="0731d-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="0731d-137">Tato možnost aktualizuje všechny záznamy, které mají aktuálně nesprávnou hodnotu společnosti, v dávkách po 1000 záznamech (ve výchozím nastavení).</span><span class="sxs-lookup"><span data-stu-id="0731d-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="0731d-138">Kód je idempotentní jak je poskytnut, což znamená, že ho můžete znovu spustit a budou aktualizovány pouze nesprávně přiřazené společnosti.</span><span class="sxs-lookup"><span data-stu-id="0731d-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="0731d-139">Při spuštění pomocí **–apply** kód vygeneruje CSV soubory s provedenými změnami s názvem **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="0731d-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
