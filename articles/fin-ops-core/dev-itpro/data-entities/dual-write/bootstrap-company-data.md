...
Je možné, že máte stávající Common Data Service nebo jinou instanci aplikace Dynamics 365 s obchodními daty a chcete pro ni povolit propojení s dvojím zápisem. V tomto případě musíte zavést Common Data Service nebo jiná data aplikace Dynamics 365 s informacemi o společnosti před povolením propojení s dvojím zápisem.  
 
## <a name="when-should-i-use-bootstrapping"></a>Kdy mám použít spuštění? 
Zavádění byste měli použít před povolením mapování entit dvojího zápisu (během kroku 5).  
1. Chcete-li nastavit propojení s dvojí zápisem mezi instancemi aplikace Finance and Operations a Common Data Service neb jinou aplikací Dynamics 365, přihlaste se k aplikaci Finance and Operations jako správce. 
...
4. Povolte mapování entit **Cdm_companies**. Tato možnost synchronizuje společnosti z aplikace Finance and Operations s aplikací Common Data Service.  
    > [!div class="mx-imgBorder"]
    > ![Povolení mapování entit](media/dual-write-boot-2.png)
5. Spusťte ukázkový zaváděcí kód na instanci Common Data Service nebo jiné aplikace Dynamics 365.  
6. Když je zavádění hotovo a systém je připraven na živou synchronizaci, povolte mapování entit.  

    Povolením mapování entit se spustí počáteční synchronizace dat pro povolené mapování entit. Data odpovídající společnostem vybraným v propojení s duálním zápisem jsou synchronizována mezi aplikací Finance and Operations a Common Data Service. 
 
## <a name="how-to-i-use-the-code-sample"></a>Jak použiji ukázku kódu?
Ukázkový kód je aplikace v jazyce C#, kterou lze načíst v aplikaci Visual Studio. Použije závislosti balíčku NuGet v Common Data Service, které lze aktualizovat pomocí standardních nástrojů Visual Studio. 
