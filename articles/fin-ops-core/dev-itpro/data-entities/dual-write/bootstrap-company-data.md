...
## <a name="why-do-i-need-bootstrapping"></a>Proč potřebuji zavádění? 
Je možné, že máte stávající Dataverse nebo jinou instanci aplikace Dynamics 365 s obchodními daty a chcete pro ni povolit propojení s dvojím zápisem. V tomto případě musíte zavést Dataverse nebo jiná data aplikace Dynamics 365 s informacemi o společnosti před povolením propojení s dvojím zápisem.  
 
## <a name="when-should-i-use-bootstrapping"></a>Kdy mám použít spuštění? 
Zavádění byste měli použít před povolením mapování tabulek dvojího zápisu (během kroku 5).  
1. Chcete-li nastavit propojení s dvojí zápisem mezi instancemi aplikace Finance and Operations a Dataverse neb jinou aplikací Dynamics 365, přihlaste se k aplikaci Finance and Operations jako správce. 
2. Přejděte na modul **Správa dat** a klikněte na **Dvojí zápis**. Tím se spustí **Integrátor dat**. 
3. Vytvořte propojení s dvojím zápisem pro jednu nebo více společností.  
    > [!div class="mx-imgBorder"]
    > ![Vytvoření propojení s dvojím zápisem](media/dual-write-boot-1.png)
4. Povolte mapování tabulek **Cdm_companies**. Tato možnost synchronizuje společnosti z aplikace Finance and Operations s aplikací Dataverse.  
    > [!div class="mx-imgBorder"]
    > ![Povolení mapování tabulek](media/dual-write-boot-2.png)
5. Spusťte ukázkový zaváděcí kód na instanci Dataverse nebo jiné aplikace Dynamics 365.  
6. Když je zavádění hotovo a systém je připraven na živou synchronizaci, povolte mapování tabulek.  

    Povolením mapování tabulek se spustí počáteční synchronizace dat pro povolené mapování tabulek. Data odpovídající společnostem vybraným v propojení s duálním zápisem jsou synchronizována mezi aplikací Finance and Operations a Dataverse. 
 
## <a name="how-to-i-use-the-code-sample"></a>Jak použiji ukázku kódu?
Ukázkový kód je aplikace v jazyce C#, kterou lze načíst v aplikaci Visual Studio. Použije závislosti balíčku NuGet v Dataverse, které lze aktualizovat pomocí standardních nástrojů Visual Studio. 

Po rozbalení a otevření řešení v aplikaci Visual Studio a obnovení balíčků NuGet vyhledejte **TODO** v kódu. Každé rozhodnutí o způsobu, jakým chcete zavádět informace o společnosti, je zaznamenáno pomocí **TODO** s ukázkovým kódem pro kanonickou implementaci. 

Ukázkový kód zobrazuje pouze jeden z mnoha způsobů, jak můžete kategorizovat řádky entit podle společnosti. Změnou logiky v částech **TODO** můžete vytvořit vlastní kategorizaci. 
...
Po dokončení simulovaných aktualizací použijte parametr **-apply**. Tato možnost aktualizuje všechny řádky, které mají aktuálně nesprávnou hodnotu společnosti, v dávkách po 1000 řádcích (ve výchozím nastavení). Kód je idempotentní jak je poskytnut, což znamená, že ho můžete znovu spustit a budou aktualizovány pouze nesprávně přiřazené společnosti. Při spuštění pomocí **–apply** kód vygeneruje CSV soubory s provedenými změnami s názvem **applied_<entityname>.csv**. 
