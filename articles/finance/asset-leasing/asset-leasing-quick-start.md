...
- **Přírůstková výpůjční úroková sazba** – Je to úroková sazba, která bude použita k výpočtu současné hodnoty. K výpočtu současné hodnoty leasingových splátek systém použije implicitní sazbu, pokud je definována v datech leasingu. Pokud implicitní sazba není definována, systém použije přírůstkovou výpůjční úrokovou sazbu.
...
- **Složený interval** – Ten představuje počet období, za které je úrok složen za rok. Může to být měsíčně (12 období ročně), čtvrtletně (4 období ročně), pololetně (2 období ročně) nebo ročně (1 období ročně). Počet období bude zohledněn při výpočtu současné hodnoty.
...
- **Platební kalendář** – Jde o vypočtenou současnou hodnotu založenou na době pokryté leasingovými splátkami, výši splátek, složeném období a typu anuity.
...
- **Měsíce** – Udává počet kalendářních měsíců po dobu trvání leasingu. Částka platby je splatná částka, jak je definována ve frekvenci plateb. Vypočítaná současná hodnota je leasingová splátka založená na současné hodnotě za období, složené intervaly a přírůstkovou výpůjční úrokovou sazbu.

> [!NOTE] 
> Současná hodnota se počítá na základě rovnice diskontovaného peněžního toku.
...
- **Prahové hodnoty** – Slouží v klasifikačních testech leasingu k určení, jestli je majetek klasifikován jako jedno z následujících:
  ...
  - **Současná hodnota** – Toto je procento reálné hodnoty majetku, které se má použít při klasifikačním testu. Systém klasifikuje leasing jako finanční, pokud je typ leasingu nastaven na automatický a je-li současná hodnota budoucích leasingových splátek na reálnou hodnotu majetku větší než nebo rovna procentu zde definovanému.
...
Klasifikační testy zahrnují převod vlastnictví, možnost nákupu, dobu trvání leasingu, současnou hodnotu a unikátní majetek. Následující diagram ilustruje klasifikační testy leasingu.
...
|     Finanční leasing podle IFRS a US GAAP        |     Používaný majetek      |     Závazek z operativního leasingu       |
...
|     Typ                                          |     Debet                     |     Kredit                            |
|-----------------------------------------------    |-----------------------------  |------------------------------------   |
|     Položka závazku z operativního leasingu podle US GAAP ASC 842    |     Výdaje na úroky          |     Závazek z operativního leasingu         |
...
Používaný majetek se odepisuje podle toho, co je menší – očekávaná doba použitelnosti majetku nebo doba trvání leasingu. Metoda výpočtu odpisů pro US GAAP (ASC 842) je založena na rozdílu mezi lineárním nákladem na leasing a částkou úroku. Úroky z finančního leasingu se počítají standardní lineární metodou. Odpisy leasingu ovlivňují výkaz zisků a ztrát odečtením úrokových nákladů. Rozvaha je ovlivněna připsáním na naakumulovaný účet používaného majetku pro finanční leasing. U operativního leasingu se odpisy připisují na účet nákladů na leasing. Pokud je leasing spojen s dlouhodobým majetkem, odpisové transakce budou provedeny pouze z modulu dlouhodobého majetku. 
